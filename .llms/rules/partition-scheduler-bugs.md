...
- **Fix**: Replace the degenerate guard with a dedicated single-buffered **cross-partition** reuse token (mirrors the TLX persistent `k_empties` pattern). The load task `producer_acquire`s it at the top of the persistent outer loop (before the operand loads); the staging task `consumer_release`s it at the bottom (after the staging stores have drained). A separate token avoids the `consumerWarps == nWarps` assert (MMA = 1 warp vs staging = 8 warps). The acquire phase is a 0-based iteration counter normalized as `(iv - lb)/step` (folds to the IV for the normalized persistent loop) so parity is correct regardless of loop bounds/step. The coarse token (acquire-before-all-loads / release-after-all-stores) covers every aliasing pair that shares one outer loop + load task + staging task; the shared staging-reuse protocol planner verifies that invariant and Step 7.5 skips (with an `LDBG`) rather than emitting a barrier that would guard only one pair.
...
### 18. Packed qK/dQ TMEM reuse loses the dQ EMPTY acquire (2026-07-31, fixed)
- **Symptom**: BM128 2-CTA FA-bwd at `Z=4, H=48, N_CTX=4096, D=128` intermittently produces a large dQ error after qK and dQ are pinned to the same TMEM buffer at offset 0. Synccheck is clean because every barrier operation is individually legal.
- **Root cause**: qK is a whole-allocation `useD=false` overwrite, while dQ is a narrower temporal-reuse view of the same TMEM allocation and starting column. A6 code partitioning treated every inner-cadence sibling as self-ordered and skipped it. dQ's EMPTY acquire therefore remained only at the later dQ producer, so the next qK could overwrite TMEM before the previous dQ consumer released it. The late `InterleaveTMem` repair in `de0c84ec4` reconstructed this lost reuse relationship after outlining and modulo expansion, when the original channels and cadence were already obscured.
- **Fix**: preserve the dependency while the reuse group is explicit. `WSCodePartition` recognizes an inner-loop sibling whose TMEM range starts with the whole-allocation owner and is narrower, then emits the missing EMPTY acquire immediately before qK. The acquire and its phase arithmetic use qK's `loop.stage`/`loop.cluster`. At that stage, code partitioning reconstructs dQ's phase from qK's accumulation count `A` and the loop-carried MMA `useAccumulator` flag: `S = useAccumulator ? A - 1 : A`, followed by the ordinary single-buffer EMPTY phase `(S & 1) ^ 1`. Standard software-pipeline expansion then emits `S=C_t` in the prologue and `S=C_t+j-1` in steady state, without a mixed operand-stage API. This removes the owner/sibling markers and the post-expansion `materializeWholeOverwriteReuseWaits` repair. The late, optional `InterleaveTMem` repair and its synthetic test remain removed.
- **Scope check**: the 2-CTA config introduced a spatial-packing group `{qK@0, P@64, dQ@0}` with the same-start narrower dQ hazard. The supported 1-CTA three-member reuse group `{dpT, dsT, dQ}` is full-overlap temporal reuse handled outside A6 and remains correct; three-member reuse alone is not the trigger.
- **Tests**: `ws_code_partition_tmem_packed_reuse_backward.mlir` checks the 1-CTA owner-stage acquire and stage-local phase reconstruction, proving detection is not cluster-gated. `pipeline_bwd_bm128_gemm_only.mlir` checks the qK-positioned prologue/steady waits and reconstructed dQ phase. `test_bwd_bm128_2cta_packed_dq`, `test_bwd_bm128_2cta_nonpersistent`, and both `test_bwd_tmem_dsT_reuse_3group{,_persistent}` paths pass. At the production shape (`Z=4,H=48,N_CTX=4096,D=128`, FP16, seed 0), persistent AutoWS passes 10/10 runs with all gradients below 0.01 error.
...
### 35. Jagged CLC hole exposes premature K/V EMPTY release under an outer `scf.while` (2026-09-12, fixed; T288013887)
- **Symptom**: HSTU self-attention backward with a rectangular CLC grid deadlocks when a reused CTA sees `valid -> hole -> valid`. Single-wave jagged launches pass, and compact jagged scheduling avoids the hang.
- **Root cause**: `WSCodePartition.cpp` predicated an outer-produced K/V buffer's MMAv5 EMPTY completion on the last inner iteration only when the producer had an enclosing `scf.for`. In the CLC kernel the producer is inside a collective `scf.if` in the after region of an outer `scf.while`, so every inner MMA released the single-copy K/V buffer. The channel counter still advanced once per valid outer transaction. A skipped iteration gave the extra asynchronous barrier flips time to overtake that preserved counter, and the next valid tile waited on an already-passed phase.
...
- **Fix**: `doCodePartition` builds a normalized protocol graph after memory
  ...
  from inner entry through inner drain. `allocation.reuseTarget` aliases add
  the same coalesced cross-tile WAR edge that Step 7.5 emits, derived by a
  shared protocol planner. The post-insertion conformance builder remains a
  follow-up in Phase 10 of
