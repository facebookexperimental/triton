...
```json
{
  "schema_version": "paper-joint-manual-cuda-memory-v2",
...
- owner CTA and physical warp, and allowed remote access;
...
- `planned.register_words`, plus `planned.per_warp_register_words`: a map keyed
  by integer physical warp ID that covers every mapped warp, respects the
  per-warp register limit, and sums to `planned.register_words`;
- static and dynamic SMEM, including `planned.barrier_smem_bytes` for barrier
  storage, and descriptors;
- TMEM columns and any CTA-group constraints;
- `planned.physical_warps`, equal to the number of warps in the IR, plus CTAs
  per cluster and expected occupancy limits;
...
Do not silently reduce stages, reassign a scheduled warp, spill a scheduled
value, or alias buffers merely to make the kernel launch.
