...
This workflow starts with a validated `twill-pipelined-warp-ir-v1` JSON
...
as automatic code generation, a paper-generated kernel, or proof that Twill
...
2. Verify `schema_version == "twill-pipelined-warp-ir-v1"`.
3. Recompute the IR hash and compare it with the
   `twill-manual-cuda-handoff-v1` manifest.
4. Record the solution hash, source-program hash, machine-model provenance,
   kernel geometry, `ii`, `length`, `copies`, `horizon`, and region bounds.
5. Reject legacy or partial artifacts. Do not infer missing cycles, groups,
   group widths, lane masks, dependencies, or region boundaries.
...
the Twill artifact and public B200 specifications with a reviewer who can
verify the boundary.
...
```json
{
  "schema_version": "twill-agent-assisted-manual-cuda-v1",
  "authoring_mode": "agent-assisted-manual-cuda",
...
7. **Compile and audit.** Inspect diagnostics, resources, and disassembly for
   ...
   implementation, not permission to mutate the Twill schedule silently.
...
contradicts a Twill cycle, issue trace, lane assignment, or dependency, obtain
a new Twill artifact or label the result as a non-faithful experiment.
