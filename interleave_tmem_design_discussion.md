...
`WSCodePartition`, adds an explicit proof of the post-pipeline order, and emits
neither endpoint of the redundant empty edge only when both proofs succeed:

```text
whole-overwrite reuse proof succeeds
  and QK_load(i) precedes P_store(i) after expansion
  and PV_MMA(i) precedes QK_MMA(i+1) after expansion
  -> no P producer-acquire wait
  -> no matching PV completion arrival
  -> keep the PV MMA asynchronous for its other channels
```

For `before(i) -> after(i+d)`, an op at stage `S` maps to logical iteration
`kernelIter + maxStage - S`; therefore the proof compares
`d + after.stage - before.stage`.  A positive value is ordered, zero falls back
to cluster then source order, and a negative value is unsafe.  Missing schedule
metadata also fails closed.  Thus a future scheduler can change these
coordinates without silently inheriting the old assumption: code partitioning
will retain the original synchronization edge.
...
reuse group, precise TMEM ranges, task identity, program order, loop identity,
and serialized stage/cluster coordinates needed for the redundancy proof. Once
that proof succeeds, emitting a hardware synchronization edge and asking a
later unrelated pass to erase it is unnecessary on the current pipeline. If a
future schedule no longer preserves either required order, the proof fails and
the original edge remains.
