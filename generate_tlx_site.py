...
def read_readme() -> str:
    return subprocess.check_output(
        ["git", "show", "main:README.md"], cwd=ROOT, text=True
    )
...
def page_links(page: Page) -> str:
    ...
    links = []
    if previous:
        href = "./" if previous.slug == "overview" else f"{previous.slug}.html"
        links.append(f'<a href="{href}">← {html.escape(previous.title)}</a>')
    if following:
        links.append(
            f'<a class="next" href="{following.slug}.html">{html.escape(following.title)} →</a>'
        )
    return "".join(links)
...
<head>
  ...
  <link rel="stylesheet" href="assets/tlx.css">
</head>
<body>
  <header class="topbar">
    <a class="brand" href="./"><span>TLX</span> Triton Low-level Language Extensions</a>
    <a class="repo-link" href="{REPOSITORY_URL}" target="_blank" rel="noreferrer">View on GitHub ↗</a>
  </header>
  <div class="shell">
    <aside class="sidebar">
      <p class="eyebrow">Documentation</p>
      <nav>{navigation(page.slug)}</nav>
    </aside>
    <main>
      ...
      <footer class="pager">{page_links(page)}</footer>
    </main>
  </div>
</body>
</html>
...
def main() -> None:
    ...
    for page, source in chunks:
        ...
        output = ROOT / (
            "index.html" if page.slug == "overview" else f"{page.slug}.html"
        )
