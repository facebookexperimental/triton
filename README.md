...
four AMD TDM descriptor loads behind one static hardware TDM instruction. Each
member is a `(positioned_desc, destination, warp_used_hint)` tuple:

| Member field | Description |
|--------------|-------------|
| `positioned_desc` | Tensor descriptor carrying the member's offsets, bounds, and optional predicate. |
| `destination` | Local buffer or local view receiving the member. |
| `warp_used_hint` | Bitmask indicating the waves that use the member. |

Position each descriptor with `tlx.update_tensor_descriptor` before issuing the
fused load. The warp-used hints must be non-empty, pairwise disjoint, and legal
axis-aligned subsets of the CTA's waves; they do not need to cover every wave.
Members must have the same descriptor rank, but may have different shapes and
element types. The fused operation currently requires one CTA, uses one shared
cache modifier, and requires shared layouts supported by AMD TDM lowering. This
is useful for kernels where different wave groups load different inputs, such
as A/B GEMM tiles or A/B plus scale tiles in MXFP GEMM, while keeping the
assembly to one TDM instruction per fused load.
