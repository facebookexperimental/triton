...
// Verify that getTotalElemsPerThread (used by the LLVM struct type converter)
// agrees with getInDimSize("register") of the LinearLayout (used by
// emitIndices to produce values).  A mismatch causes a fatal
// "size mismatch when packing elements for LLVM struct" error.
TEST_F(LinearEncodingTest, NpotStructPackingConsistency) {
  // Shapes that exercise modular reduction in combineCtaCgaWithShape.
  // Includes the originally-reported 1023.
  SmallVector<SmallVector<int64_t>> npotShapes = {
      {33},
      {1023},
  };

  // Create a BlockedEncoding: sizePerThread=1, threadsPerWarp=32,
  // warpsPerCTA=4, order=[0] -- the configuration from the bug report.
  auto blocked1D = triton::gpu::BlockedEncodingAttr::get(
      &ctx, /*sizePerThread=*/{1}, /*threadsPerWarp=*/{32},
      /*warpsPerCTA=*/{4}, /*order=*/{0},
      triton::gpu::CGAEncodingAttr::get1CTALayout(&ctx, /*rank=*/1));

  StringAttr kRegister = StringAttr::get(&ctx, "register");

  for (auto &shape : npotShapes) {
    // Get the LinearLayout that emitIndices would use.
    auto ll = blocked1D.toLinearLayout(shape);
    unsigned llRegisterCount = ll.getInDimSize(kRegister);

    // Get what the type converter would compute for the struct size.
    auto linearEncoding = triton::gpu::LinearEncodingAttr::get(&ctx, ll);
    unsigned structSize = linearEncoding.getTotalElemsPerThread(shape);

    ASSERT_EQ(llRegisterCount, structSize)
        << "Struct packing mismatch for NPOT shape [" << shape[0]
        << "]: emitIndices produces " << llRegisterCount
        << " values but struct expects " << structSize;
  }

  // 2D NPOT shapes: both dims are non-power-of-2.
  // Exercises combineCtaCgaWithShape with multi-dim modular reduction.
  SmallVector<SmallVector<int64_t>> npot2DShapes = {
      {33, 48},
  };

  auto blocked2D = triton::gpu::BlockedEncodingAttr::get(
      &ctx, /*sizePerThread=*/{1, 1}, /*threadsPerWarp=*/{1, 32},
      /*warpsPerCTA=*/{4, 1}, /*order=*/{1, 0},
      triton::gpu::CGAEncodingAttr::get1CTALayout(&ctx, /*rank=*/2));

  for (auto &shape : npot2DShapes) {
    auto ll = blocked2D.toLinearLayout(shape);
    unsigned llRegisterCount = ll.getInDimSize(kRegister);
    ...
    ASSERT_EQ(llRegisterCount, structSize)
        << "Struct packing mismatch for 2D NPOT shape [" << shape[0] << ", "
        << shape[1] << "]: emitIndices produces " << llRegisterCount
        << " values but struct expects " << structSize;
  }
}

// Verify struct packing consistency for SliceEncoding with NPOT shapes.
TEST_F(LinearEncodingTest, NpotStructPackingSlice) {
  auto blocked2D = triton::gpu::BlockedEncodingAttr::get(
      &ctx, {1, 1}, {1, 32}, {4, 1}, {1, 0},
      triton::gpu::CGAEncodingAttr::get1CTALayout(&ctx, 2));
  auto sliced = triton::gpu::SliceEncodingAttr::get(&ctx, 1, blocked2D);

  StringAttr kRegister = StringAttr::get(&ctx, "register");

  for (int64_t npotSize : {33, 48}) {
    auto ll = sliced.toLinearLayout({npotSize});
    unsigned llRegisterCount = ll.getInDimSize(kRegister);

    auto linearEncoding = triton::gpu::LinearEncodingAttr::get(&ctx, ll);
    unsigned structSize = linearEncoding.getTotalElemsPerThread({npotSize});

    ASSERT_EQ(llRegisterCount, structSize)
        << "Struct packing mismatch for sliced NPOT shape [" << npotSize << "]";
  }
}

// Verify basesPerDim reports correct NPOT lane counts (not rounded to pow2).
TEST_F(LinearEncodingTest, NpotBasesPerDimLaneCounts) {
  auto blocked = triton::gpu::BlockedEncodingAttr::get(
      &ctx, /*sizePerThread=*/{1, 1}, /*threadsPerWarp=*/{4, 8},
      /*warpsPerCTA=*/{1, 1}, /*order=*/{1, 0},
      triton::gpu::CGAEncodingAttr::get1CTALayout(&ctx, 2));

  // Dim 0: shape 3 < threadsPerWarp 4 → expect 3, not 4.
  {
    auto ll = blocked.toLinearLayout({3, 8});
    auto linearEncoding = triton::gpu::LinearEncodingAttr::get(&ctx, ll);
    auto tpw = linearEncoding.getThreadsPerWarp();
    ASSERT_EQ(tpw[0], 3u) << "Expected 3 threads in dim 0, got " << tpw[0];
    ASSERT_EQ(tpw[1], 8u) << "Expected 8 threads in dim 1, got " << tpw[1];
  }

  // Both dims NPOT: shape {3, 5}.
  {
    auto ll = blocked.toLinearLayout({3, 5});
    auto linearEncoding = triton::gpu::LinearEncodingAttr::get(&ctx, ll);
    auto tpw = linearEncoding.getThreadsPerWarp();
    ASSERT_EQ(tpw[0], 3u) << "Expected 3 threads in dim 0, got " << tpw[0];
