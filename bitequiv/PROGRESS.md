...
- [WIP]  M1 (Wk3-5): STATIC TTGIR reduction-equivalence checker + autotuner integration DONE (initial): bitequiv/equivalence.py (reduction_signature/classify) + autotuner `equivalence_fn` hook (prune to reference config's order, no launch). detect (unordered→N classes) + enforce-verify (inner_tree→1 class) validated. TODO: PTX/FMA backstop, multi-reduce/MMA (#mma layouts), design doc + progress post.
...
- [DONE] T3: autotuner correctness-check hook — GPU-validated 2026-06-04 (GB300): 9/9 constraint-prune tests + regression clean. correctness_fn/correctness_prune on @triton.autotune. (see bitequiv/reports/starter-t3-t4-report.md)
- [DONE] T4: IR/PTX pruning — GPU-validated 2026-06-05 (GB300), 11/11 tests. artifact_config_prune in prune_configs_by; all 4 example filters fire a real keep+drop: vectorization (PTX), AutoWS keep-path (TTGIR; persistent TMA matmul + tl.range(warp_specialize=True) + knobs.nvidia.use_meta_ws; FLATTEN=False kept / True dropped), TMEM_LOAD prune (TTGIR; fp32 input_precision tf32→tmem_load dropped / ieee kept). ONLY remaining: real TMEM_LOAD *accuracy-bug* repro (needs Nick/collab-day).
