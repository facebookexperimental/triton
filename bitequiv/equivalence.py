...
class _IRBasedPrune:
    ...
    def _anchor_key_of(self, ref):
        ...
        if isinstance(ref, str):  # raw IR text (the checker reads the level it needs)
            return self.key_fn(None, {"ttgir": ref, "ptx": ref}, None)
...
def reduction_equivalence_prune(level="ttgir", reference=None):
    ...
    matches the reference at *every* level. ``reference`` chooses the anchor order:
    ``None`` (the first config, supplied by the autotuner) or an external pre-compiled
    kernel / asm dict / IR text. Built on :func:`ir_based_prune_configs`; returns a
    predicate exposing ``.classes`` / ``.pruned``.
    """
