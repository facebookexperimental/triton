...
autotuner ``ir_config_prune`` hook):

  * ``reduction_descriptor(ttgir) -> tuple[str, ...]`` — one canonical signature per
  ...
  * ``reductions_equivalent(a, b) -> bool``.

The human-readable design is in ``bitequiv/design-doc.md``. This module is pure
Python but requires the built ``triton`` (it calls into ``libtriton``); parsing a
TTGIR string is CPU-only (no GPU, no kernel launch).
"""
