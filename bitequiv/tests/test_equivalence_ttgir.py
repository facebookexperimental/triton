...
MMA) is covered in `test_ttgir_reduction.py`. Tests that exercise the real checker
...
The PTX-level checker is the in-tree sibling `bitequiv/ptx_reduction.py`."""
import os
import sys
...
from bitequiv.equivalence_ttgir import (  # noqa: E402
    ttgir_reduction_signature, classify, _ttgir_equivalence_key, CHECKERS, ir_based_prune_configs,
...
def test_equivalence_key_adapter_reads_ttgir():
    key_a = _ttgir_equivalence_key(None, {"ttgir": _fx("sum_uno_nw4")}, None)
    key_b = _ttgir_equivalence_key(None, {"ttgir": _fx("sum_uno_nw8")}, None)
    assert key_a == ttgir_reduction_signature(_fx("sum_uno_nw4"))
    assert key_a != key_b


def test_no_reduction_yields_empty_signature():
    assert ttgir_reduction_signature("") == ()
