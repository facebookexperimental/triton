"""Unit tests for the MLIR-native reduction-order checker (`bitequiv.reduction_tree`).
...
from bitequiv.reduction_tree import reduction_descriptor, reductions_equivalent  # noqa: E402
...
def _eq(a, b):
    return reductions_equivalent(_load(a), _load(b))
...
def test_descriptor_one_per_reduce_and_empty_for_none():
    assert len(reduction_descriptor(_load("sum_uno_nw4"))) == 1  # one tt.reduce
    assert reduction_descriptor("") == ()  # no reduction-like op
...
def test_multi_operand_argmin_sound():
    ...
    assert reduction_descriptor(_load("argmin_nw2")) != ()
...
def test_gemm_mma_sound():
    ...
    assert reduction_descriptor(_load("gemm_ieee")) != ()
