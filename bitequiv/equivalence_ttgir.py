...
the in-tree PTX/``--fmad`` backstop (``bitequiv/ptx_reduction.py``) is also needed.
Signatures are conservative — they never call non-equivalent configs equivalent
(a textual layout difference yields a different signature), so the kept set is always
a safe subset.

The reduction-order engine now lives in :mod:`bitequiv.ttgir_reduction` (a logical
mixed-radix / GF(2) thread<->data descriptor + a canonical-tree test oracle); this
module keeps the autotuner-facing API (signature/prune/registry). Names mirror the PTX
checker's :mod:`bitequiv.equivalence_ptx` (``ttgir_reduction_signature`` ↔
``ptx_reduction_signature``). The human-readable design is in ``bitequiv/CLAUDE.md`` /
``bitequiv/PROGRESS.md``.
"""
from collections import OrderedDict

from .ttgir_reduction import ttgir_reduction_descriptor, ttgir_reductions_equivalent


def ttgir_reduction_signature(ttgir):
    """Static reduction-order signature of a TTGIR module (one entry per
    ``tt.reduce``).

    Thin alias for :func:`bitequiv.ttgir_reduction.ttgir_reduction_descriptor` — the
    ...
    reduction. See ``bitequiv/CLAUDE.md`` / ``bitequiv/PROGRESS.md``
    for how the descriptor encodes the thread<->data map and how equality is decided.
    """
    return ttgir_reduction_descriptor(ttgir)


def same_reduction_order(ttgir_a, ttgir_b):
    """True iff the two TTGIRs reduce in the same (bitwise-equivalent) order.
    Thin alias for :func:`bitequiv.ttgir_reduction.ttgir_reductions_equivalent`."""
    return ttgir_reductions_equivalent(ttgir_a, ttgir_b)


def _ttgir_equivalence_key(config, asm, metadata):
    ...
    """
    return ttgir_reduction_signature(asm.get("ttgir", ""))
...
# bitwise-equivalent at that level. This registry ships the TTGIR level; the PTX
# backstop (``bitequiv/ptx_reduction.py``) is the in-tree sibling checker.
CHECKERS = {
    "ttgir": _ttgir_equivalence_key,
}
...
def reduction_equivalence_prune(level="ttgir", reference=None):
    ...
    ``level`` is ``"ttgir"`` (default) or an ordered list of level names (this module
    registers ``"ttgir"``; the PTX backstop is the in-tree sibling
    ``bitequiv/ptx_reduction.py``). Multiple levels are combined into a single key (the per-level
...
def classify(named_ttgirs):
    ...
    for label, ttgir in named_ttgirs.items():
        classes.setdefault(ttgir_reduction_signature(ttgir), []).append(label)
    return classes
