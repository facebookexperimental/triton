#!/bin/bash
# llvm_bisect.sh - Bisect LLVM commits within a compatible Triton range
#
...
#     $ git bisect bad
#     $ git bisect run bash /path/to/llvm_bisect.sh
#
# For help: bash llvm_bisect.sh --help
...
Example:
  ...
  cd llvm-project
  TRITON_DIR=/path/to/triton TEST_SCRIPT=/path/to/test.py \
    git bisect run bash llvm_bisect.sh
...
if [ -z "$TRITON_DIR" ]; then
  ...
  echo "Run 'bash llvm_bisect.sh --help' for usage information"
  exit 128
fi
