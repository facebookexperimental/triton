...
tt.func public @reinterpret_propagate_to_make_desc(%arg0: !tt.ptr<i8> {tt.divisibility = 16 : i32}, %arg1: i32, %arg2: i32) {
  ...
  // CHECK: tt.make_tensor_descriptor {{.*}} descPtr = {{.*}} : !tt.ptr<i8> : !tt.ptr<i8>, !tt.tensordesc<128x64xi8, #[[SHARED]]>
  %1 = tt.make_tensor_descriptor %arg0, [%arg1, %arg2], [%0, %c1_i64], descPtr = %desc_ptr : !tt.ptr<i8> : !tt.ptr<i8>, !tt.tensordesc<128x64xi8>

  // Fence and reinterpret the pointer as a tensor descriptor
  ttng.tensormap_fenceproxy_acquire %desc_ptr : !tt.ptr<i8>
  // CHECK: ttng.reinterpret_tensor_descriptor {{.*}} : !tt.ptr<i8> to !tt.tensordesc<128x64xi8, #[[SHARED]]>
  %2 = ttng.reinterpret_tensor_descriptor %desc_ptr : !tt.ptr<i8> to !tt.tensordesc<128x64xi8>
  ...
  // CHECK: ttng.async_tma_copy_global_to_local {{.*}} : !tt.tensordesc<128x64xi8, #[[SHARED]]>
  ttng.async_tma_copy_global_to_local %2[%c0_i32, %c0_i32] %buf, %bar, %true : !tt.tensordesc<128x64xi8>, !ttg.memdesc<1xi64, #shared1, #smem, mutable> -> !ttg.memdesc<128x64xi8, #shared, #smem, mutable>
