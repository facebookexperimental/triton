...
// TMEM uses shape from the largest allocation, with blockN scaled up as needed
// to ensure blockM × blockN × sizeof(i32) >= total buffer size in bytes.
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @alloc_tmem_buffer() {
    ...
    // Allocation uses shape from user: blockM=128, blockN=64 (scaled up if needed)
    // CHECK: ttng.tmem_alloc : () -> !ttg.memdesc<128x64xi32
    // CHECK: tlx.local_alias
