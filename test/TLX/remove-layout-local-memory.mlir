...
// CHECK-NEXT: ttg.local_store %[[RESHAPE]], %{{.*}} {loop.cluster = 2 : i32, loop.stage = 3 : i32, ttg.partition = array<i32: 7>} : tensor<32x128xf32, #[[$DIRECT]]> -> !ttg.memdesc<32x128xf32, #{{.*}}, #{{.*}}, mutable>
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32, ttg.target = "cuda:100"} {
  tt.func @local_store_reshape_convert(
    ...
    ttg.local_store %reshape, %arg1 {ttg.partition = array<i32: 7>, loop.cluster = 2 : i32, loop.stage = 3 : i32} : tensor<32x128xf32, #linear_cvt_flat> -> !ttg.memdesc<32x128xf32, #shared_store, #smem_store, mutable>
    tt.return
  }
}
