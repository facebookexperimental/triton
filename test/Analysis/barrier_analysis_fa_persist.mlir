...
// Test barrier analysis on FlashAttention persistent kernel (fa_persist.mlir)
// This is a comprehensive test that exercises:
// 1. Cross-region barrier tracing through WarpSpecializeOp
// 2. Multiple barrier allocations with memdesc_index
// 3. Complex producer/consumer patterns across warp groups
// 4. Both mbarrier and gpu.barrier operations

// CHECK: === Barrier Analysis for function: _attn_fwd_persist ===
// CHECK: Barrier Execution Order Analysis

// Main region should have all init_barrier operations (bar=0 through bar=47)
// CHECK: [main] (Producer)
// CHECK-DAG: init_barrier (bar=0)
// CHECK-DAG: init_barrier (bar=1)
// CHECK-DAG: init_barrier (bar=2)
// CHECK-DAG: init_barrier (bar=3)
// CHECK-DAG: init_barrier (bar=4)
// CHECK-DAG: init_barrier (bar=5)
// CHECK-DAG: init_barrier (bar=10)
// CHECK-DAG: init_barrier (bar=20)
// CHECK-DAG: init_barrier (bar=30)
// CHECK-DAG: init_barrier (bar=40)

// WG0 (default region) should have wait/arrive operations with resolved barrier IDs
// CHECK: [WG0] (Producer) (Consumer)
// Verify barrier IDs are resolved (not bar=-1)
// CHECK-DAG: wait_barrier (bar={{[0-9]+}}
// CHECK-DAG: arrive_barrier (bar={{[0-9]+}}

// Partition regions should have their barriers traced through block arguments
// CHECK: [WG1]
// CHECK-DAG: wait_barrier (bar={{[0-9]+}}

// CHECK: [WG2]
// CHECK-DAG: wait_barrier (bar={{[0-9]+}}
// CHECK-DAG: barrier_expect (bar={{[0-9]+}}
...
// CHECK: Dependencies:
// Verify init->use dependencies are detected
// CHECK-DAG: init_barrier{{.*}} --[init->use]-->
// Verify arrive->wait dependencies are detected
// CHECK-DAG: arrive_barrier{{.*}} --[arrive->wait]-->

// CHECK: Cross-Warp-Group Dependencies:
// The fa_persist kernel has cross-warp-group synchronization

// CHECK: Execution Trace Visualization
// CHECK: Legend:

// CHECK: digraph BarrierDependencies
...
#tmem3 = #ttng.tensor_memory_encoding<blockM = 128, blockN = 128, unpacked = false>
