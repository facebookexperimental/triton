...
// CHECK-LABEL: external_argument vs external_argument: alias=true
// CHECK: external_argument vs external_local: alias=true, lhs_contains_rhs=false, rhs_contains_lhs=false
// CHECK: external_local vs external_local: alias=true
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.shared = 64 : i32, ttg.target = "cuda:90", ttg.tensor_memory_size = 0 : i32, "ttg.threads-per-warp" = 32 : i32, "ttg.total-num-warps" = 1 : i32} {
  tt.func public @external_descriptor_is_unknown(
      %incoming: !ttg.memdesc<16xi32, #shared, #smem, mutable>) {
    %local = ttg.local_alloc {allocation.offset = 0 : i32} : () -> !ttg.memdesc<16xi32, #shared, #smem, mutable>
    %0 = ttg.local_load %incoming {test.region_name = "external_argument"} : !ttg.memdesc<16xi32, #shared, #smem, mutable> -> tensor<16xi32>
    %1 = ttg.local_load %local {test.region_name = "external_local"} : !ttg.memdesc<16xi32, #shared, #smem, mutable> -> tensor<16xi32>
    tt.return
  }
}
