...
// CHECK:      ttng.wait_barrier %[[TMEM_FULL:.*]], %{{.*}}
// CHECK-NEXT: %[[TV:.*]] = ttng.tmem_load
// CHECK-NEXT: ttng.arrive_barrier %[[TMEM_EMPTY:.*]], 1
// CHECK-NEXT: %[[TC:.*]] = ttg.convert_layout %[[TV]]
// CHECK-NEXT: ttng.wait_barrier %[[LOCAL_FULL:.*]], %{{.*}}
// CHECK-NEXT: %[[LV:.*]] = ttg.local_load
// CHECK-NEXT: ttng.arrive_barrier %[[LOCAL_EMPTY:.*]], 1
// CHECK-NEXT: %[[LE:.*]] = arith.extf %[[LV]]
// CHECK-NEXT: %[[SUM:.*]] = arith.addf %[[TC]], %[[LE]]
...
// The TMEM-consumer hoist must survive TRITON_DISABLE_WSBARRIER_REORDER=1.
// prioritizeTMemOperand was gated on that knob, and the FA kernels set it by
// default, so the hoist silently never ran in production and FA backward lost
// ~25%. The knob still disables the global sinkWSArrives/raiseWSWaits
// normalization; it no longer disables this operand-ordering step.
// TARGETED-LABEL: @prioritize_tmem_operand_reorder_disabled
// TARGETED:      ttng.wait_barrier %[[TFULL:.*]], %{{.*}}
// TARGETED-NEXT: %[[TV:.*]] = ttng.tmem_load
// TARGETED-NEXT: ttng.arrive_barrier %[[TEMPTY:.*]], 1
// TARGETED-NEXT: %[[TC:.*]] = ttg.convert_layout %[[TV]]
// TARGETED-NEXT: ttng.wait_barrier %[[LFULL:.*]], %{{.*}}
// TARGETED-NEXT: %[[LV:.*]] = ttg.local_load
// TARGETED-NEXT: ttng.arrive_barrier %[[LEMPTY:.*]], 1
tt.func @prioritize_tmem_operand_reorder_disabled(
    %smem: !ttg.memdesc<128x64xf16, #shared, #smem, mutable>,
    %tmem: !ttg.memdesc<128x64xf32, #tmem, #ttng.tensor_memory, mutable, 128x128>,
    %local_full: !ttg.memdesc<1xi64, #barrier_shared, #smem, mutable>,
    %local_empty: !ttg.memdesc<1xi64, #barrier_shared, #smem, mutable>,
...
// release. prioritizeTMemOperand used to reject the whole move whenever the
// adjacent barriers failed a barrier-map identity check; that rejection also
// suppressed the profitable hoist above, so it was removed. This pins the
// behaviour that replaced it: the foreign channel's arrive must NOT be dragged
// ahead of the TMEM load with the operand group -- it stays after the
// local_load that precedes it in program order.
// CHECK-LABEL: @prioritize_tmem_operand_foreign_barrier
// CHECK:      %[[TV2:.*]] = ttng.tmem_load
// CHECK:      %[[LV2:.*]] = ttg.local_load
// CHECK:      ttng.arrive_barrier %[[FOREIGN:.*]], 1
