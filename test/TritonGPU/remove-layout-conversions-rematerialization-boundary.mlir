...
// An identity conversion carrying the boolean coordinate-rematerialization
// request is a semantic backend boundary. Its block-argument source has no producer to
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func @identity_rematerialization_boundary(
    ...
    // CHECK: %[[BOUNDARY:.*]] = ttg.convert_layout %arg0 {tlx.rematerialize_coordinates} : tensor<64x2xf32, #{{.*}}> -> tensor<64x2xf32, #{{.*}}>
    %boundary = ttg.convert_layout %src {tlx.rematerialize_coordinates} : tensor<64x2xf32, #blocked> -> tensor<64x2xf32, #blocked>
