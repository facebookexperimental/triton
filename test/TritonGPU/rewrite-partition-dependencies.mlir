...
// -tritongpu-rewrite-partition-dependencies -verify-diagnostics -canonicalize |
// FileCheck %s
...
!ty = tensor<1xi32, #blocked>
  ...
  {ttg.partition.stages = [ 0, 2 ], ttg.warp_specialize.tag = 0 : i32} tt.return
  }
...
!ty = tensor<1xi32, #blocked>
  ...
  tt.func @invalid_attribute(% lb : i32, % ub : i32, % step : i32) {
    // expected-error @below {{partition stages attribute 'ttg.partition.stages'
    // has invalid element "a"}}
  scf.for %i = %lb to %ub step %step : i32 {
    scf.yield
