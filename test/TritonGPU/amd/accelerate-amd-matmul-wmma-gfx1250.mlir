...
// Carrying independent A/B operands alongside an accumulator must not classify
// a peeled GEMM as a chained dot or change its warp layout at loop boundaries.
...
// CHECK: #[[$PEELED_MMA:.+]] = #ttg.amd_wmma<{version = 3, isTranspose = true, ctaLayout = {warp = {{\[\[0, 1\], \[1, 0\]\]}}}, instrShape = [16, 16, 32]}>
// CHECK-NOT: #ttg.amd_wmma
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx1250", "ttg.threads-per-warp" = 32 : i32} {
  // CHECK-LABEL: wmma_peeled_gemm_independent_loop_results
  tt.func public @wmma_peeled_gemm_independent_loop_results(%a: tensor<256x32xf16, #a>, %b: tensor<32x256xf16, #b>, %count: index) -> tensor<256x256xf32, #blocked> {
    %zero = arith.constant dense<0.0> : tensor<256x256xf32, #blocked>
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    // CHECK: tt.dot {{.*}} -> tensor<256x256xf32, #[[$PEELED_MMA]]>
    %first = tt.dot %a, %b, %zero : tensor<256x32xf16, #a> * tensor<32x256xf16, #b> -> tensor<256x256xf32, #blocked>
    %result:3 = scf.for %i = %c0 to %count step %c1 iter_args(%acc = %first, %next_a = %a, %next_b = %b) -> (tensor<256x256xf32, #blocked>, tensor<256x32xf16, #a>, tensor<32x256xf16, #b>) {
      // CHECK: tt.dot {{.*}} -> tensor<256x256xf32, #[[$PEELED_MMA]]>
      %dot = tt.dot %next_a, %next_b, %acc : tensor<256x32xf16, #a> * tensor<32x256xf16, #b> -> tensor<256x256xf32, #blocked>
      scf.yield %dot, %next_a, %next_b : tensor<256x256xf32, #blocked>, tensor<256x32xf16, #a>, tensor<32x256xf16, #b>
    }
    // CHECK: tt.dot {{.*}} -> tensor<256x256xf32, #[[$PEELED_MMA]]>
    %last = tt.dot %result#1, %result#2, %result#0 : tensor<256x32xf16, #a> * tensor<32x256xf16, #b> -> tensor<256x256xf32, #blocked>
...
// A loop result that actually carries a dot into operand A remains a chain.
#blocked = #ttg.blocked<{sizePerThread = [1, 8], threadsPerWarp = [8, 4], warpsPerCTA = [4, 1], order = [1, 0]}>
#a = #ttg.dot_op<{opIdx = 0, parent = #blocked}>
#b = #ttg.dot_op<{opIdx = 1, parent = #blocked}>
// CHECK: #[[$CHAIN_MMA:.+]] = #ttg.amd_wmma<{version = 3, isTranspose = true, ctaLayout = {warp = {{\[\[1, 0\], \[2, 0\]\]}}}, instrShape = [16, 16, 32]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx1250", "ttg.threads-per-warp" = 32 : i32} {
  // CHECK-LABEL: wmma_chain_through_loop_result
  tt.func public @wmma_chain_through_loop_result(%a: tensor<256x256xf16, #a>, %b: tensor<256x256xf16, #b>, %count: index) -> tensor<256x256xf32, #blocked> {
    %zero = arith.constant dense<0.0> : tensor<256x256xf32, #blocked>
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    // CHECK: tt.dot {{.*}} -> tensor<256x256xf32, #[[$CHAIN_MMA]]>
    %first = tt.dot %a, %b, %zero : tensor<256x256xf16, #a> * tensor<256x256xf16, #b> -> tensor<256x256xf32, #blocked>
    %half = arith.truncf %first : tensor<256x256xf32, #blocked> to tensor<256x256xf16, #blocked>
    %next_a = ttg.convert_layout %half : tensor<256x256xf16, #blocked> -> tensor<256x256xf16, #a>
    %result = scf.for %i = %c0 to %count step %c1 iter_args(%carried_a = %next_a) -> tensor<256x256xf16, #a> {
      scf.yield %carried_a : tensor<256x256xf16, #a>
    }
    // CHECK: tt.dot {{.*}} -> tensor<256x256xf32, #[[$CHAIN_MMA]]>
    %last = tt.dot %result, %b, %zero : tensor<256x256xf16, #a> * tensor<256x256xf16, #b> -> tensor<256x256xf32, #blocked>
    tt.return %last : tensor<256x256xf32, #blocked>
  }
}
