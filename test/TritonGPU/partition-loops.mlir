...
// -tritongpu-partition-loops -verify-diagnostics -canonicalize | FileCheck %s
...
!ty = tensor<1xi32, #blocked>
  ...
  tt.func @multiple_partitions_two_loops(% lb : i32, % ub : i32, % step : i32,
    ...
    // CHECK: partition0 num_warps(4)
    // CHECK-NEXT: op_00b
    // CHECK-NEXT: [[RET:%.*]]:3 = scf.for [[I:%.*]] = [[LB:%.*]] to [[UB:%.*]]
    // step [[STEP:%.*]] iter_args([[ARG0:%.*]] = {{.*}}, [[ARG1:%.*]] = {{.*}},
    // [[ARG2:%.*]] = {{.*}}) -> (i32, i32, i32) : i32 { CHECK-NEXT:   [[X:%.*]]
    // = "op_a" CHECK-NEXT:   "op_b"([[ARG0]]) CHECK-NEXT:   "op_b"([[X]])
    ...
    // CHECK-NEXT: [[RET:%.*]]:3 = scf.for [[I:%.*]] = [[LB:%.*]] to [[UB:%.*]]
    // step [[STEP:%.*]] iter_args([[ARG0:%.*]] = {{.*}}, [[ARG1:%.*]] = {{.*}},
    // [[ARG2:%.*]] = {{.*}}) -> (i32, i32, i32) : i32 { CHECK-NEXT:   [[Y:%.*]]
    // = arith.addi [[I]], [[I]] CHECK-NEXT:   [[X:%.*]] = "op_a"([[Y]])
    ...
    // step [[STEP:%.*]] iter_args([[ARG0:%.*]] = {{.*}}, [[ARG1:%.*]] = {{.*}},
    // [[ARG2:%.*]] = {{.*}}) -> (i32, i32, i32) : i32 { CHECK-NEXT:   [[Y:%.*]]
    // = arith.addi [[I]], [[I]] CHECK-NEXT:   [[Z:%.*]] = arith.addi [[I]],
    // [[Y]] CHECK-NEXT:   [[X:%.*]] = "op_a"([[Z]]) CHECK-NEXT:
    // "op_b"([[ARG2]]) CHECK-NEXT:   "op_b"([[X]]) CHECK-NEXT:   arith.addi
    // CHECK-NEXT:   arith.addi
    // CHECK-NEXT:   arith.addi
    // CHECK-NEXT:   scf.yield
    // CHECK-NEXT: }
  ...
  tt.func @partition_outputs(% lb : i32, % ub : i32, % step : i32)
      ->(!ty, !ty, !ty) {
  ...
  tt.func @future_conditional_self_use(% lb : i32, % ub : i32, % step : i32,
                                       % cond : i1) {
  %c0_i32 = arith.constant 0 : i32
  scf.for %i = %lb to %ub step %step iter_args(%k = %c0_i32) -> i32 : i32 {
    % 0 = "op_a"(){ttg.partition = array < i32 : 0 > } : ()->i32 scf.if %
          cond{"use"(% k) : (i32)->()} {ttg.partition = array <
                                                        i32 : 0 > } scf.yield %
          0 : i32
  }
  {ttg.partition.stages = [0], ttg.warp_specialize.tag = 0 : i32} tt.return
  }

  // CHECK-LABEL: @trivial_tensor_captures
  tt.func @trivial_tensor_captures(% arg0 : f16, % lb : i32, % ub : i32,
                                   % step : i32) {
  ...
  scf.for %i = %lb to %ub step %step iter_args(%idxs = %cst) -> tensor<128xi32, #blocked> : i32 {
    % do_prologue = "prologue_cond"(% i)
        : (i32)->i1 %
                    0 = scf.if % do_prologue->tensor<128xi32, #blocked> {
      % 1 = tt.splat % i : i32->tensor<128xi32, #blocked> % 2 = arith.addi % 1,
        % idxs : tensor<128xi32, #blocked> scf.yield %
            2 : tensor<128xi32, #blocked>
    }
    else {
      scf.yield % idxs : tensor<128xi32, #blocked>
    }
    "op_a"(% 0){ttg.partition = array < i32 : 0 > }
        : (tensor<128xi32, #blocked>)
              ->() "op_b"(% i){ttg.partition = array < i32 : 1 > } : (i32)
              ->() "op_c"(% 0){ttg.partition = array < i32 : 2 > }
        : (tensor<128xi32, #blocked>)
              ->() scf.yield %
        0 : tensor<128xi32, #blocked>
  }
