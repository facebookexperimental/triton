...
// --nvws-assign-stage-phase  -canonicalize | FileCheck %s
...
module attributes{
  ...
  tt.func @aref_lowering(% d : !ttg.memdesc<3x64x16xf16, #shared0, #smem>,
    ...
    scf.for %i = %lb to %ub step %c1_i32 : i32{
      ...
      % 1 : 3 = nvws.aref.put.enter %
                aref0[% c0_i32, % c0_i32]{ttg.partition = array < i32 : 0 > }
          : !nvws.aref<[
            !ttg.memdesc<3x64x16xf16, #shared0, #smem>,
            !ttg.memdesc<3x16x32xf16, #shared0, #smem>
          ]> -> !ttg.memdesc<64x16xf16, #shared0, #smem>,
            !ttg.memdesc<16x32xf16, #shared0, #smem>,
            !ttg.async
                    .token "op1"(% 1 #0){ttg.partition = array < i32 : 0 > }
          : (!ttg.memdesc<64x16xf16, #shared0, #smem>)
                    ->() "op2"(% 1 #1){ttg.partition = array < i32 : 0 > }
          : (!ttg.memdesc<16x32xf16, #shared0, #smem>)
                    ->()
            // CHECK: op2
            ...
            nvws.aref.put.exit %
                aref0[% c0_i32],
            % 1 #2 [#nvws.async_op<tma_load>,
                    #nvws.async_op<none>] { ttg.partition = array < i32 : 0 > }
          : !nvws.aref<[
            !ttg.memdesc<3x64x16xf16, #shared0, #smem>,
            !ttg.memdesc<3x16x32xf16, #shared0, #smem>
          ]>,
            !ttg.async.token
                ...
                // CHECK-NEXT: [[S1a:%.*]] = arith.addi [[S1]], [[C1]]
                // CHECK-NEXT: [[CMP:%.*]] = arith.cmpi eq, [[S1a]], [[C3]]
                // CHECK-NEXT: [[S1b:%.*]] = arith.select [[CMP]], [[C0]],
                // [[S1a]] CHECK-NEXT: [[P1a:%.*]] = arith.xori [[P1]], [[C1]]
                // CHECK-NEXT: [[P1b:%.*]] = arith.select [[CMP]], [[P1a]],
                // [[P1]] CHECK-NEXT: {{.*}}, [[TOK1:%.*]] = nvws.aref.get.enter
                // [[AREF0]][[[S1b]], [[P1b]]] {ttg.partition = array<i32: 1>}
                % 2 : 3 =
                nvws.aref.get.enter %
                aref0[% c0_i32, % c0_i32]{ttg.partition = array < i32 : 1 > }
          : !nvws.aref<[
            !ttg.memdesc<3x64x16xf16, #shared0, #smem>,
            !ttg.memdesc<3x16x32xf16, #shared0, #smem>
          ]> -> !ttg.memdesc<64x16xf16, #shared0, #smem>,
            ...
            !ttg.async
                    .token
                "op3"(% 2 #0, % 2 #1){ttg.partition = array < i32 : 1 > }
          : (!ttg.memdesc<64x16xf16, #shared0, #smem>,
             !ttg.memdesc<16x32xf16, #shared0, #smem>)
                    ->()
            // CHECK: op3
            // CHECK-NEXT: get.exit [[AREF0]][[[S1b]]], [[TOK1]]
            // [#nvws.async_op<tc5mma>] {ttg.partition = array<i32: 1>}
            nvws.aref.get.exit %
                aref0[% c0_i32],
            % 2 #2
                [#nvws.async_op<tc5mma>] { ttg.partition = array < i32 : 1 > }
          : !nvws.aref<[
            ...
            !ttg.async
                ...
                cond{
                  // CHECK-NEXT: [[S2a:%.*]] = arith.addi [[S2]], [[C1]]
                  // CHECK-NEXT: [[CMP:%.*]] = arith.cmpi eq, [[S2a]], [[C3]]
                  // CHECK-NEXT: [[S2b:%.*]] = arith.select [[CMP]], [[C0]],
                  // [[S2a]]
                  // CHECK-NEXT: [[P2a:%.*]] = arith.xori [[P2]], [[C1]]
                  // CHECK-NEXT: [[P2b:%.*]] = arith.select [[CMP]], [[P2a]],
                  // [[P2]]
                  // CHECK-NEXT: {{.*}}, [[TOK2:%.*]] = nvws.aref.put.enter
                  // [[AREF1]][[[S2b]], [[P2b]]] {ttg.partition = array<i32: 0>}
                  // CHECK-NEXT: op4
                  // CHECK-NEXT: put.exit [[AREF1]][[[S2b]]]
                  % 4 : 3 = nvws.aref.put.enter %
                            aref1[% c0_i32, % c0_i32]{
                  ...
                  !ttg.async.token
                      ...
                      // CHECK-NEXT: [[CMP:%.*]] = arith.cmpi eq, [[S3a]],
                      // [[C3]]
                      // CHECK-NEXT: [[S3b:%.*]] = arith.select [[CMP]], [[C0]],
                      // [[S3a]]
                      // CHECK-NEXT: [[P3a:%.*]] = arith.xori [[P3]], [[C1]]
                      // CHECK-NEXT: [[P3b:%.*]] = arith.select [[CMP]],
                      // [[P3a]], [[P3]]
                      // CHECK-NEXT: {{.*}}, [[TOK3:%.*]] = nvws.aref.get.enter
                      // [[AREF1]][[[S3b]], [[P3b]]] {ttg.partition = array<i32:
                      // 1>}
                      // CHECK-NEXT: op5
                      // CHECK-NEXT: get.exit [[AREF1]][[[S3b]]], [[TOK3]]
                      // [#nvws.async_op<tc5mma>] {ttg.partition = array<i32:
                      // 1>}
                      % 5 : 3 =
                      nvws.aref.get.enter %
                      aref1[% c0_i32, % c0_i32]{
      ...
      // CHECK-NEXT: }
      // CHECK: scf.yield [[S0b]], [[P0b]], [[S1b]], [[P1b]], [[IDX1]]#0,
      // [[IDX1]]#1, [[IDX1]]#2, [[IDX1]]#3
    }
    {ttg.warp_specialize.tag = 0 : i32} tt.return
  }
}
