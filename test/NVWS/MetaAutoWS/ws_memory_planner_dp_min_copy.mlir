// RUN: triton-opt %s --nvws-memory-planner=num-buffers=2 --mlir-print-debuginfo --mlir-print-local-scope | FileCheck %s
...
// CHECK: ttng.tmem_alloc {{.*}}buffer.copy = 2 : i32, buffer.id = [[ACC1:[0-9]+]] : i32, buffer.offset = 0 : i32
// CHECK: ttng.tmem_alloc {{.*}}buffer.copy = 2 : i32, buffer.id = [[ACC0:[0-9]+]] : i32, buffer.offset = 0 : i32
