...
// Insertion: accesses, ownership, buffer reuse, structured regions, and diagnostics.
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @circular_tutorial_1_1_to_2_2(%lb: i32, %ub: i32, %step: i32) {
    ...
    // CHECK: [[BASE:%[-A-Za-z0-9_.$#]+]] = ttg.local_alloc {buffer.circular, buffer.copy = 2 : i32, buffer.id = 301 : i32, buffer.start = 0 : i32} : () -> !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>
    // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
    // CHECK-NEXT: [[EMPTY:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.create [[BASE]] released = 3 {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
    // CHECK-NEXT: [[FULL:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.create [[BASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    ...
    %r = scf.for %iv = %lb to %ub step %step iter_args(%i = %c0) -> (i32) : i32 {
      // CHECK: [[K_EMPTY_STAGE:%[-A-Za-z0-9_.$#]+]] = arith.constant {ttg.partition = array<i32: 1>} 0 : i32
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[K_EMPTY_TOK:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.acquire [[EMPTY]]{{\[}}[[K_EMPTY_STAGE]]{{\]}} {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[K_EMPTY_BUF:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.buffer [[EMPTY]]{{\[}}[[K_EMPTY_STAGE]]{{\]}}, [[K_EMPTY_TOK]] {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      ...
      // CHECK: [[V_EMPTY_TOK:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.acquire [[EMPTY]]{{\[}}[[K_EMPTY_STAGE]]{{\]}} {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[V_EMPTY_BUF:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.buffer [[EMPTY]]{{\[}}[[K_EMPTY_STAGE]]{{\]}}, [[V_EMPTY_TOK]] {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      ...
      // CHECK: [[K_FULL_STAGE:%[-A-Za-z0-9_.$#]+]] = arith.constant {ttg.partition = array<i32: 2>} -1 : i32
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[K_FULL_TOK:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.acquire [[FULL]]{{\[}}[[K_FULL_STAGE]]{{\]}} {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[K_FULL_BUF:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.buffer [[FULL]]{{\[}}[[K_FULL_STAGE]]{{\]}}, [[K_FULL_TOK]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[KVAL:%[-A-Za-z0-9_.$#]+]] = ttg.local_load [[K_FULL_BUF]] {ttg.partition = array<i32: 2>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> tensor<128x128xf16, #blocked>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      ...
      // CHECK: [[V_FULL_STAGE:%[-A-Za-z0-9_.$#]+]] = arith.constant {ttg.partition = array<i32: 2>} 0 : i32
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[V_FULL_TOK:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.acquire [[FULL]]{{\[}}[[V_FULL_STAGE]]{{\]}} {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[V_FULL_BUF:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.buffer [[FULL]]{{\[}}[[V_FULL_STAGE]]{{\]}}, [[V_FULL_TOK]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[VVAL:%[-A-Za-z0-9_.$#]+]] = ttg.local_load [[V_FULL_BUF]] {ttg.partition = array<i32: 2>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> tensor<128x128xf16, #blocked>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @circular_tutorial_1_2_to_3_4(%lb: i32, %ub: i32, %step: i32) {
    ...
    // CHECK: [[BASE:%[-A-Za-z0-9_.$#]+]] = ttg.local_alloc {buffer.circular, buffer.copy = 2 : i32, buffer.id = 302 : i32, buffer.start = 0 : i32} : () -> !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>
    // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
    // CHECK-NEXT: [[EMPTY:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.create [[BASE]] released = 3 {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
    // CHECK-NEXT: [[FULL:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.create [[BASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    ...
    %r = scf.for %iv = %lb to %ub step %step iter_args(%i = %c0) -> (i32) : i32 {
      ...
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      ttg.local_store %payload_k, %k {ttg.partition = array<i32: 1>} : !ty -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK: [[V_EMPTY_STAGE:%[-A-Za-z0-9_.$#]+]] = arith.constant {ttg.partition = array<i32: 2>} 0 : i32
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[V_EMPTY_TOK:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.acquire [[EMPTY]]{{\[}}[[V_EMPTY_STAGE]]{{\]}} {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[V_EMPTY_BUF:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.buffer [[EMPTY]]{{\[}}[[V_EMPTY_STAGE]]{{\]}}, [[V_EMPTY_TOK]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      ...
      // CHECK: [[K_FULL_STAGE:%[-A-Za-z0-9_.$#]+]] = arith.constant {ttg.partition = array<i32: 3>} -1 : i32
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[K_FULL_TOK:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.acquire [[FULL]]{{\[}}[[K_FULL_STAGE]]{{\]}} {ttg.partition = array<i32: 3>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[K_FULL_BUF:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.buffer [[FULL]]{{\[}}[[K_FULL_STAGE]]{{\]}}, [[K_FULL_TOK]] {ttg.partition = array<i32: 3>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[KVAL:%[-A-Za-z0-9_.$#]+]] = ttg.local_load [[K_FULL_BUF]] {ttg.partition = array<i32: 3>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> tensor<128x128xf16, #blocked>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      ...
      // CHECK: [[V_FULL_STAGE:%[-A-Za-z0-9_.$#]+]] = arith.constant {ttg.partition = array<i32: 4>} 0 : i32
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[V_FULL_TOK:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.acquire [[FULL]]{{\[}}[[V_FULL_STAGE]]{{\]}} {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[V_FULL_BUF:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.buffer [[FULL]]{{\[}}[[V_FULL_STAGE]]{{\]}}, [[V_FULL_TOK]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[VVAL:%[-A-Za-z0-9_.$#]+]] = ttg.local_load [[V_FULL_BUF]] {ttg.partition = array<i32: 4>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> tensor<128x128xf16, #blocked>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @circular_tutorial_1_1_to_2_3(%lb: i32, %ub: i32, %step: i32) {
    ...
    %payload_k = "make_k"() {ttg.partition = array<i32: 1>} : () -> !ty
    %payload_v = "make_v"() {ttg.partition = array<i32: 1>} : () -> !ty
    // CHECK: [[BASE:%[-A-Za-z0-9_.$#]+]] = ttg.local_alloc {buffer.circular, buffer.copy = 2 : i32, buffer.id = 303 : i32, buffer.start = 0 : i32} : () -> !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>
    // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
    // CHECK-NEXT: [[EMPTY:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.create [[BASE]] released = 3 {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
    // CHECK-NEXT: [[FULL:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.create [[BASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    ...
    %r = scf.for %iv = %lb to %ub step %step iter_args(%i = %c0) -> (i32) : i32 {
      ...
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      %kval = ttg.local_load %k {ttg.partition = array<i32: 2>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> !ty
      // CHECK: [[V_FULL_STAGE:%[-A-Za-z0-9_.$#]+]] = arith.constant {ttg.partition = array<i32: 3>} 0 : i32
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[V_FULL_TOK:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.acquire [[FULL]]{{\[}}[[V_FULL_STAGE]]{{\]}} {ttg.partition = array<i32: 3>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[V_FULL_BUF:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.buffer [[FULL]]{{\[}}[[V_FULL_STAGE]]{{\]}}, [[V_FULL_TOK]] {ttg.partition = array<i32: 3>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
      // CHECK: [[VVAL:%[-A-Za-z0-9_.$#]+]] = ttg.local_load [[V_FULL_BUF]] {ttg.partition = array<i32: 3>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> tensor<128x128xf16, #blocked>
      // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @circular_tutorial_1_2_to_3_3(%lb: i32, %ub: i32, %step: i32) {
    ...
    %payload_k = "make_k"() {ttg.partition = array<i32: 1>} : () -> !ty
    %payload_v = "make_v"() {ttg.partition = array<i32: 2>} : () -> !ty
    // CHECK: [[BASE:%[-A-Za-z0-9_.$#]+]] = ttg.local_alloc {buffer.circular, buffer.copy = 2 : i32, buffer.id = 304 : i32, buffer.start = 0 : i32} : () -> !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>
    // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
    // CHECK-NEXT: [[EMPTY:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.create [[BASE]] released = 3 {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    // CHECK-NOT: {{ttg\.local_alloc|nvws\.semaphore\.create}}
    // CHECK-NEXT: [[FULL:%[-A-Za-z0-9_.$#]+]] = nvws.semaphore.create [[BASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
...
// SMEM buffer reuse: buffer.id groups the local allocations, and buffer.offset
// determines which member footprints overlap.
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  // (extent 128 each). Two partitions
...
module attributes {"ttg.num-warps" = 4 : i32} {
  tt.func @unsupported_local_memdesc_forwarding(%lb: i32, %ub: i32, %step: i32) {
    ...
    scf.for %i = %lb to %ub step %step : i32 {
      ...
      // expected-error @below {{nvws-insert-semas: unsupported memdesc alias use test.memdesc_view}}
...
module attributes {"ttg.num-warps" = 4 : i32} {
  tt.func @reject_zero_buffer_copy(%lb: i32, %ub: i32, %step: i32) {
    // expected-error @below {{nvws-insert-semas: buffer.copy must be in [1, 32], got 0}}
...
module attributes {"ttg.num-warps" = 4 : i32} {
  tt.func @reject_buffer_copy_above_mask_width() {
    // expected-error @below {{nvws-insert-semas: buffer.copy must be in [1, 32], got 33}}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @tokenless_copy_one_alias(%lb: i32, %ub: i32, %step: i32,
    ...
    // Both logical allocations use the same physical view after CSE.
    // CHECK-NOT: nvws.semaphore
    ...
    scf.for %i = %lb to %ub step %step : i32 {
      ...
      // CHECK-NEXT: %[[LOADED:.*]] = ttg.local_load %[[A_VIEW]] {ttg.partition = array<i32: 0>}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  tt.func @disjoint_members_reject(%lb: i32, %ub: i32, %step: i32) {
    ...
    %cst1 = arith.constant dense<1.000000e+00> : tensor<128x128xf16, #blocked>
    %r = scf.for %iv = %lb to %ub step %step iter_args(%i = %c0) -> (i32) : i32 {
      // expected-error @below {{nvws-insert-semas: buffer.id group has disjoint pieces (more than one connected component); InsertSemas requires one component per group}}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @invalid_loop_carried_schedule(%lb: i32, %ub: i32, %step: i32) {
    ...
    scf.for %iv = %lb to %ub step %step : i32 {
      ...
      // expected-note @below {{handoff {1} -> {3} has producer loop.stage 2, consumer loop.stage 0, loop-carried dependency distance 1, and required delay 1}}
      ...
      // expected-note @below {{handoff {3} -> {1} has producer loop.stage 0, consumer loop.stage 0, loop-carried dependency distance 0, and required delay 0}}
      ...
      // expected-error @below {{nvws-insert-semas: fixed loop.stage assignments form an unsatisfiable semaphore handoff cycle (cycle requires 1 additional pipeline iteration)}}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @fused_alias_depth_two(%lb: i32, %ub: i32, %step: i32) {
    ...
    // CHECK: [[BASE:%.*]] = ttg.local_alloc {buffer.copy = 2 : i32, buffer.id = 500 : i32} : () -> !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>
    // CHECK: [[ENTRY:%.*]] = nvws.semaphore.create [[BASE]], [[BASE]] released = 3 {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    // CHECK: [[FULL0:%.*]] = nvws.semaphore.create [[BASE]], [[BASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    // CHECK: [[EMPTY1:%.*]] = nvws.semaphore.create [[BASE]], [[BASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    // CHECK: [[FULL1:%.*]] = nvws.semaphore.create [[BASE]], [[BASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>
    ...
    scf.for %iv = %lb to %ub step %step : i32 {
      ...
      // CHECK: [[W0_ZERO:%.*]] = arith.constant {ttg.partition = array<i32: 4>} 0 : i32
      // CHECK: [[W0_TOK:%.*]] = nvws.semaphore.acquire [[ENTRY]][[[W0_ZERO]]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: [[W0_BUF:%.*]]:2 = nvws.semaphore.buffer [[ENTRY]], [[W0_TOK]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<128x128xf16, #shared, #smem, mutable, 2x128x128>
      // CHECK: ttg.local_store %{{[-A-Za-z0-9_.$#]+}}, [[W0_BUF]]#0 {ttg.partition = array<i32: 4>} : tensor<128x128xf16, #blocked> -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK: nvws.semaphore.release [[FULL0]][[[W0_ZERO]]], [[W0_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[R0_ZERO:%.*]] = arith.constant {ttg.partition = array<i32: 2>} 0 : i32
      // CHECK: [[R0_TOK:%.*]] = nvws.semaphore.acquire [[FULL0]][[[R0_ZERO]]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: [[R0_BUF:%.*]]:2 = nvws.semaphore.buffer [[FULL0]], [[R0_TOK]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<128x128xf16, #shared, #smem, mutable, 2x128x128>
      // CHECK: ttg.local_load [[R0_BUF]]#0 {ttg.partition = array<i32: 2>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> tensor<128x128xf16, #blocked>
      // CHECK: [[TO_M1:%.*]] = arith.constant {ttg.partition = array<i32: 2>} 1 : i32
      // CHECK: nvws.semaphore.release [[EMPTY1]][[[TO_M1]]], [[R0_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[W1_TOK:%.*]] = nvws.semaphore.acquire [[EMPTY1]][[[W0_ZERO]]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: [[W1_BUF:%.*]]:2 = nvws.semaphore.buffer [[EMPTY1]], [[W1_TOK]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable, 2x128x128>, !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK: ttg.local_store %{{[-A-Za-z0-9_.$#]+}}, [[W1_BUF]]#1 {ttg.partition = array<i32: 4>} : tensor<128x128xf16, #blocked> -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK: nvws.semaphore.release [[FULL1]][[[W0_ZERO]]], [[W1_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[R1_TOK:%.*]] = nvws.semaphore.acquire [[FULL1]][[[R0_ZERO]]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: [[R1_BUF:%.*]]:2 = nvws.semaphore.buffer [[FULL1]], [[R1_TOK]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable, 2x128x128>, !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
      // CHECK: ttg.local_load [[R1_BUF]]#1 {ttg.partition = array<i32: 2>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> tensor<128x128xf16, #blocked>
      // CHECK: nvws.semaphore.release [[ENTRY]][[[TO_M1]]], [[R1_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<2x128x128xf16, #shared, #smem, mutable>]>, !ttg.async.token
  ...
  tt.func @fused_partial_alias_depth_three(%lb: i32, %ub: i32, %step: i32) {
    ...
    // CHECK: [[PLARGE:%.*]] = ttg.local_alloc {buffer.copy = 3 : i32, buffer.id = 502 : i32} : () -> !ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>
    // CHECK: [[PSMALL:%.*]] = ttg.local_alloc {buffer.copy = 3 : i32, buffer.id = 502 : i32} : () -> !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>
    // CHECK: [[PENTRY:%.*]] = nvws.semaphore.create [[PLARGE]], [[PSMALL]] released = 5 {pending_count = 1 : i32} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>
    // CHECK: [[PFULL0:%.*]] = nvws.semaphore.create [[PLARGE]], [[PSMALL]] {pending_count = 1 : i32} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>
    // CHECK: [[PHANDOFF:%.*]] = nvws.semaphore.create [[PLARGE]], [[PSMALL]] {pending_count = 1 : i32} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>
    // CHECK: [[PFULL1:%.*]] = nvws.semaphore.create [[PLARGE]], [[PSMALL]] {pending_count = 1 : i32} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>
    ...
    scf.for %iv = %lb to %ub step %step : i32 {
      ...
      // CHECK: [[PW0_ZERO:%.*]] = arith.constant {ttg.partition = array<i32: 4>} 0 : i32
      // CHECK: [[PW0_TOK:%.*]] = nvws.semaphore.acquire [[PENTRY]][[[PW0_ZERO]]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: [[PW0_BUF:%.*]]:2 = nvws.semaphore.buffer [[PENTRY]], [[PW0_TOK]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<256x64xf16, #shared, #smem, mutable, 3x256x64>, !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
      // CHECK: ttg.local_store %{{[-A-Za-z0-9_.$#]+}}, [[PW0_BUF]]#1 {ttg.partition = array<i32: 4>} : tensor<128x64xf16, #blocked1> -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
      // CHECK: nvws.semaphore.release [[PFULL0]][[[PW0_ZERO]]], [[PW0_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 4>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[PR0_ZERO:%.*]] = arith.constant {ttg.partition = array<i32: 2>} 0 : i32
      // CHECK: [[PR0_TOK:%.*]] = nvws.semaphore.acquire [[PFULL0]][[[PR0_ZERO]]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: [[PR0_BUF:%.*]]:2 = nvws.semaphore.buffer [[PFULL0]], [[PR0_TOK]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<256x64xf16, #shared, #smem, mutable, 3x256x64>, !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
      // CHECK: ttg.local_load [[PR0_BUF]]#1 {ttg.partition = array<i32: 2>} : !ttg.memdesc<128x64xf16, #shared, #smem, mutable> -> tensor<128x64xf16, #blocked1>
      // CHECK: [[TO_LARGE:%.*]] = arith.constant {ttg.partition = array<i32: 2>} 1 : i32
      // CHECK: nvws.semaphore.release [[PHANDOFF]][[[TO_LARGE]]], [[PR0_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[PW1_TOK:%.*]] = nvws.semaphore.acquire [[PHANDOFF]][[[PW0_ZERO]]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: [[PW1_BUF:%.*]]:2 = nvws.semaphore.buffer [[PHANDOFF]], [[PW1_TOK]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<128x64xf16, #shared, #smem, mutable, 3x128x64>
      // CHECK: ttg.local_store %{{[-A-Za-z0-9_.$#]+}}, [[PW1_BUF]]#0 {ttg.partition = array<i32: 4>} : tensor<256x64xf16, #blocked1> -> !ttg.memdesc<256x64xf16, #shared, #smem, mutable>
      // CHECK: nvws.semaphore.release [[PFULL1]][[[PW0_ZERO]]], [[PW1_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 4>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[PR1_TOK:%.*]] = nvws.semaphore.acquire [[PFULL1]][[[PR0_ZERO]]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: [[PR1_BUF:%.*]]:2 = nvws.semaphore.buffer [[PFULL1]], [[PR1_TOK]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<128x64xf16, #shared, #smem, mutable, 3x128x64>
      // CHECK: ttg.local_load [[PR1_BUF]]#0 {ttg.partition = array<i32: 2>} : !ttg.memdesc<256x64xf16, #shared, #smem, mutable> -> tensor<256x64xf16, #blocked1>
      // CHECK: nvws.semaphore.release [[PENTRY]][[[PR0_ZERO]]], [[PR1_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<3x256x64xf16, #shared, #smem, mutable>, !ttg.memdesc<3x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token
  ...
  tt.func @tmem_fused_alias_depth_two(%lb: i32, %ub: i32, %step: i32) {
    ...
    // CHECK: [[TBASE:%.*]] = ttng.tmem_alloc {buffer.copy = 2 : i32, buffer.id = 501 : i32, buffer.offset = 0 : i32} : () -> !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>
    // CHECK: [[TENTRY:%.*]] = nvws.semaphore.create [[TBASE]], [[TBASE]] released = 3 {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>
    // CHECK: [[TFULL0:%.*]] = nvws.semaphore.create [[TBASE]], [[TBASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>
    // CHECK: [[TEMPTY1:%.*]] = nvws.semaphore.create [[TBASE]], [[TBASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>
    // CHECK: [[TFULL1:%.*]] = nvws.semaphore.create [[TBASE]], [[TBASE]] {pending_count = 1 : i32} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>
    // CHECK: scf.for
    scf.for %iv = %lb to %ub step %step : i32 {
      // Member 0 write: the value-carrying tmem_alloc becomes a tmem_store
      // through view #0 with no token bracket.
      // CHECK: [[TW0_ZERO:%.*]] = arith.constant {ttg.partition = array<i32: 4>} 0 : i32
      // CHECK: [[TW0_TOK:%.*]] = nvws.semaphore.acquire [[TENTRY]][[[TW0_ZERO]]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]> -> !ttg.async.token
      // CHECK: [[TW0_BUF:%.*]]:2 = nvws.semaphore.buffer [[TENTRY]], [[TW0_TOK]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>
      // CHECK: ttng.tmem_store %{{[-A-Za-z0-9_.$#]+}}, [[TW0_BUF]]#0, %{{[-A-Za-z0-9_.$#]+}} {ttg.partition = array<i32: 4>} : tensor<128x128xf32, #blocked> -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>
      // CHECK: nvws.semaphore.release [[TFULL0]][[[TW0_ZERO]]], [[TW0_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[TR0_ZERO:%.*]] = arith.constant {ttg.partition = array<i32: 2>} 0 : i32
      // CHECK: [[TR0_TOK:%.*]] = nvws.semaphore.acquire [[TFULL0]][[[TR0_ZERO]]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]> -> !ttg.async.token
      // CHECK: [[TR0_BUF:%.*]]:2 = nvws.semaphore.buffer [[TFULL0]], [[TR0_TOK]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>
      // CHECK: %{{[-A-Za-z0-9_.$#]+}}, %{{[-A-Za-z0-9_.$#]+}} = ttng.tmem_load [[TR0_BUF]]#0[] {ttg.partition = array<i32: 2>} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128> -> tensor<128x128xf32, #blocked>
      // CHECK: [[T_TO_M1:%.*]] = arith.constant {ttg.partition = array<i32: 2>} 1 : i32
      // CHECK: nvws.semaphore.release [[TEMPTY1]][[[T_TO_M1]]], [[TR0_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token
      ...
      // TFULL1.
      // CHECK: [[TW1_TOK:%.*]] = nvws.semaphore.acquire [[TEMPTY1]][[[TW0_ZERO]]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]> -> !ttg.async.token
      // CHECK: [[TW1_BUF:%.*]]:2 = nvws.semaphore.buffer [[TEMPTY1]], [[TW1_TOK]] {ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>
      // CHECK: ttng.tmem_store %{{[-A-Za-z0-9_.$#]+}}, [[TW1_BUF]]#1, %{{[-A-Za-z0-9_.$#]+}} {ttg.partition = array<i32: 4>} : tensor<128x128xf32, #blocked> -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>
      // CHECK: nvws.semaphore.release [[TFULL1]][[[TW0_ZERO]]], [[TW1_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 4>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[TR1_TOK:%.*]] = nvws.semaphore.acquire [[TFULL1]][[[TR0_ZERO]]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]> -> !ttg.async.token
      // CHECK: [[TR1_BUF:%.*]]:2 = nvws.semaphore.buffer [[TFULL1]], [[TR1_TOK]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128>
      // CHECK: %{{[-A-Za-z0-9_.$#]+}}, %{{[-A-Za-z0-9_.$#]+}} = ttng.tmem_load [[TR1_BUF]]#1[] {ttg.partition = array<i32: 2>} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 2x128x128> -> tensor<128x128xf32, #blocked>
      // CHECK: nvws.semaphore.release [[TENTRY]][[[T_TO_M1]]], [[TR1_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<2x128x128xf32, #tmem, #ttng.tensor_memory, mutable>]>, !ttg.async.token
  ...
  tt.func @case_a(%lb: i32, %ub: i32, %step: i32,
    ...
    // CHECK: [[A_INIT_TOK:%.*]] = nvws.semaphore.acquire [[A_ENTRY]] {ttg.partition = array<i32: 0>, ttg.warp_specialize.tag = 10 : i32} :
    %acc, %tok = ttng.tmem_alloc {buffer.copy = 3 : i32} : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
    // CHECK: scf.for {{.*}} iter_args([[A_W0_TOK:%.*]] = [[A_INIT_TOK]])
    %outer = scf.for %i = %lb to %ub step %step iter_args(%carry = %tok) -> (!ttg.async.token) : i32 {
      // CHECK: [[A_W0_BUF:%.*]] = nvws.semaphore.buffer [[A_ENTRY]], [[A_W0_TOK]] {ttg.partition = array<i32: 0>}
      // CHECK: ttng.tmem_store {{%.*}}, [[A_W0_BUF]][], {{%.*}} {ttg.partition = array<i32: 0>}
      // CHECK: nvws.semaphore.release [[A_NEXT]], [[A_W0_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
      ...
      // CHECK: [[A_W1_TOK:%.*]] = nvws.semaphore.acquire [[A_NEXT]] {ttg.partition = array<i32: 1>}
      // CHECK: [[A_W1_BUF:%.*]] = nvws.semaphore.buffer [[A_NEXT]], [[A_W1_TOK]] {ttg.partition = array<i32: 1>}
      // CHECK: ttng.tc_gen5_mma {{%.*}}, {{%.*}}, [[A_W1_BUF]][], {{%.*}}, {{%.*}} {ttg.partition = array<i32: 1>}
      // CHECK: nvws.semaphore.release [[A_ENTRY]], [[A_W1_TOK]] [#nvws.async_op<tc5mma>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[A_READ_TOK:%.*]] = nvws.semaphore.acquire [[A_ENTRY]] {ttg.partition = array<i32: 0>}
      // CHECK: [[A_READ_BUF:%.*]] = nvws.semaphore.buffer [[A_ENTRY]], [[A_READ_TOK]] {ttg.partition = array<i32: 0>}
  ...
  tt.func @case_b(%lb: i32, %ub: i32, %step: i32,
    ...
    // CHECK: [[B_INIT_TOK:%.*]] = nvws.semaphore.acquire [[B_ENTRY]] :
    %acc, %tok = ttng.tmem_alloc {buffer.copy = 3 : i32} : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
    // CHECK: [[B_W0_BUF:%.*]] = nvws.semaphore.buffer [[B_ENTRY]], [[B_INIT_TOK]]
    ...
    // CHECK: scf.for {{.*}} iter_args([[B_MMA_TOK:%.*]] = [[B_INIT_TOK]])
    %outer = scf.for %i = %lb to %ub step %step iter_args(%carry = %w0) -> (!ttg.async.token) : i32 {
      // CHECK: [[B_MMA_BUF:%.*]] = nvws.semaphore.buffer [[B_ENTRY]], [[B_MMA_TOK]] {ttg.partition = array<i32: 1>}
      // CHECK: ttng.tc_gen5_mma {{%.*}}, {{%.*}}, [[B_MMA_BUF]][], {{%.*}}, {{%.*}} {ttg.partition = array<i32: 1>}
      // CHECK: nvws.semaphore.release [[B_FULL]], [[B_MMA_TOK]] [#nvws.async_op<tc5mma>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[B_READ_TOK:%.*]] = nvws.semaphore.acquire [[B_FULL]] {ttg.partition = array<i32: 0>}
      // CHECK: [[B_READ_BUF:%.*]] = nvws.semaphore.buffer [[B_FULL]], [[B_READ_TOK]] {ttg.partition = array<i32: 0>}
      // CHECK: {{%.*}}, {{%.*}} = ttng.tmem_load [[B_READ_BUF]][] {ttg.partition = array<i32: 0>}
      // CHECK: nvws.semaphore.release [[B_ENTRY]], [[B_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
      %value, %read = ttng.tmem_load %acc[%w1] {ttg.partition = array<i32: 0>} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<128x128xf32, #blocked>
      "use_b"(%value) {ttg.partition = array<i32: 0>} : (tensor<128x128xf32, #blocked>) -> ()
      // CHECK: [[B_NEXT_TOK:%.*]] = nvws.semaphore.acquire [[B_ENTRY]] {ttg.partition = array<i32: 1>}
      scf.yield {ttg.partition = array<i32: 0, 1>} %read : !ttg.async.token
  ...
  tt.func @case_c(%lb: i32, %ub: i32, %step: i32,
    ...
    %outer = scf.for %i = %lb to %ub step %step iter_args(%outer_token = %tok) -> (!ttg.async.token) : i32 {
      // CHECK: [[C_W0_TOK:%.*]] = nvws.semaphore.acquire [[C_ENTRY]] {ttg.partition = array<i32: 0>}
      // CHECK: [[C_W0_BUF:%.*]] = nvws.semaphore.buffer [[C_ENTRY]], [[C_W0_TOK]] {ttg.partition = array<i32: 0>}
      // CHECK: ttng.tmem_store {{%.*}}, [[C_W0_BUF]][], {{%.*}} {ttg.partition = array<i32: 0>}
      // CHECK: nvws.semaphore.release [[C_FREE]], [[C_W0_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
      ...
      %inner = scf.for %j = %lb to %ub step %step iter_args(%inner_token = %w0) -> (!ttg.async.token) : i32 {
        // CHECK: [[C_W1_TOK:%.*]] = nvws.semaphore.acquire [[C_FREE]] {ttg.partition = array<i32: 1>}
        // CHECK: [[C_W1_BUF:%.*]] = nvws.semaphore.buffer [[C_FREE]], [[C_W1_TOK]] {ttg.partition = array<i32: 1>}
        // CHECK: ttng.tc_gen5_mma {{%.*}}, {{%.*}}, [[C_W1_BUF]][], {{%.*}}, {{%.*}} {ttg.partition = array<i32: 1>}
        // CHECK: nvws.semaphore.release [[C_FULL]], [[C_W1_TOK]] [#nvws.async_op<tc5mma>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
        ...
        // CHECK: [[C_READ_TOK:%.*]] = nvws.semaphore.acquire [[C_FULL]] {ttg.partition = array<i32: 0>}
        // CHECK: [[C_READ_BUF:%.*]] = nvws.semaphore.buffer [[C_FULL]], [[C_READ_TOK]] {ttg.partition = array<i32: 0>}
        // CHECK: {{%.*}}, {{%.*}} = ttng.tmem_load [[C_READ_BUF]][] {ttg.partition = array<i32: 0>}
        // CHECK: nvws.semaphore.release [[C_FREE]], [[C_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
        %value, %read = ttng.tmem_load %acc[%w1] {ttg.partition = array<i32: 0>} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<128x128xf32, #blocked>
      ...
      // CHECK: [[C_ACQUIRE_AUTHORED_OFFSET_ONE:%.*]] = arith.constant {ttg.partition = array<i32: 1>} 1 : i32
      // CHECK: [[C_EXIT_TOK:%.*]] = nvws.semaphore.acquire [[C_FREE]][[[C_ACQUIRE_AUTHORED_OFFSET_ONE]]] {ttg.partition = array<i32: 1>}
      // CHECK: nvws.semaphore.release [[C_ENTRY]][[[C_ACQUIRE_AUTHORED_OFFSET_ONE]]], [[C_EXIT_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
  ...
  tt.func @case_d(%lb: i32, %ub: i32, %step: i32,
    ...
    // CHECK: [[D_INIT_TOK:%.*]] = nvws.semaphore.acquire [[D_ENTRY]] {ttg.partition = array<i32: 0>, ttg.warp_specialize.tag = 13 : i32} :
    %acc, %tok = ttng.tmem_alloc {buffer.copy = 3 : i32} : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
    // CHECK: scf.for {{.*}} iter_args([[D_W0_TOK:%.*]] = [[D_INIT_TOK]])
    %outer = scf.for %i = %lb to %ub step %step iter_args(%outer_token = %tok) -> (!ttg.async.token) : i32 {
      // CHECK: [[D_W0_BUF:%.*]] = nvws.semaphore.buffer [[D_ENTRY]], [[D_W0_TOK]] {ttg.partition = array<i32: 0>}
      // CHECK: ttng.tmem_store {{%.*}}, [[D_W0_BUF]][], {{%.*}} {ttg.partition = array<i32: 0>}
      // CHECK: nvws.semaphore.release [[D_NEXT]], [[D_W0_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
      %w0 = ttng.tmem_store %zero, %acc[%outer_token], %true {ttg.partition = array<i32: 0>} : tensor<128x128xf32, #blocked> -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
      // CHECK: [[D_W1_TOK:%.*]] = nvws.semaphore.acquire [[D_NEXT]] {ttg.partition = array<i32: 1>}
      // CHECK: scf.for
      %inner = scf.for %j = %lb to %ub step %step iter_args(%inner_token = %w0) -> (!ttg.async.token) : i32 {
        // CHECK: [[D_W1_BUF:%.*]] = nvws.semaphore.buffer [[D_NEXT]], [[D_W1_TOK]] {ttg.partition = array<i32: 1>}
      ...
      // CHECK: nvws.semaphore.release [[D_ENTRY]], [[D_W1_TOK]] [#nvws.async_op<tc5mma>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
      // CHECK: [[D_READ_TOK:%.*]] = nvws.semaphore.acquire [[D_ENTRY]] {ttg.partition = array<i32: 0>}
      // CHECK: [[D_READ_BUF:%.*]] = nvws.semaphore.buffer [[D_ENTRY]], [[D_READ_TOK]] {ttg.partition = array<i32: 0>}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @depth5_regular_atomic_if(%lb: i32, %ub: i32, %step: i32,
                                    %cond: i1) {
    // CHECK: [[A_BASE:%.*]] = ttg.local_alloc {buffer.copy = 5 : i32, buffer.id = 900 : i32}
    // CHECK: [[B_BASE:%.*]] = ttg.local_alloc {buffer.copy = 5 : i32, buffer.id = 900 : i32}
    // CHECK: [[C_BASE:%.*]] = ttg.local_alloc {buffer.copy = 5 : i32, buffer.id = 900 : i32}
    // CHECK: [[ENTRY:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] released = 21 {pending_count = 1 : i32}
    // CHECK: [[A_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] {pending_count = 1 : i32}
    // CHECK: [[A_TO_B:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] {pending_count = 1 : i32}
    // CHECK: [[B_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] {pending_count = 1 : i32}
    // CHECK: [[C_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] {pending_count = 1 : i32}
    // CHECK: [[IF_BACK:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] {pending_count = 1 : i32}
    ...
    // CHECK: scf.for
    scf.for %iv = %lb to %ub step %step : i32 {
      // CHECK: [[ZA:%.*]] = arith.constant {{.*}} 0 : i32
      // CHECK: [[A_TOK:%.*]] = nvws.semaphore.acquire [[ENTRY]][[[ZA]]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>}
      // CHECK: [[A_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[ENTRY]], [[A_TOK]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>}
      // CHECK: ttg.local_store {{.*}}, [[A_VIEWS]]#0 {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>}
      // CHECK: nvws.semaphore.release [[A_FULL]][[[ZA]]], [[A_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[ZAR:%.*]] = arith.constant {{.*}} 0 : i32
      // CHECK: [[A_READ_TOK:%.*]] = nvws.semaphore.acquire [[A_FULL]][[[ZAR]]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
      // CHECK: [[A_READ_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[A_FULL]], [[A_READ_TOK]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
      // CHECK: ttg.local_load [[A_READ_VIEWS]]#0 {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
      // CHECK: [[TO_B:%.*]] = arith.constant {{.*}} 1 : i32
      // CHECK: nvws.semaphore.release [[A_TO_B]][[[TO_B]]], [[A_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
      ...
      // B starts a fresh write after A's read releases the next slot.
      // CHECK: [[B_TOK:%.*]] = nvws.semaphore.acquire [[A_TO_B]][[[ZA]]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>}
      // CHECK: [[B_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[A_TO_B]], [[B_TOK]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>}
      // CHECK: ttg.local_store {{.*}}, [[B_VIEWS]]#1 {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>}
      // CHECK: nvws.semaphore.release [[B_FULL]][[[ZA]]], [[B_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[B_READ_TOK:%.*]] = nvws.semaphore.acquire [[B_FULL]][[[ZAR]]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
      // CHECK: [[B_READ_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[B_FULL]], [[B_READ_TOK]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
      ...
      // The scf.if yields its owner-0 token.
      // CHECK: [[IF_TOKEN:%.*]] = scf.if
      scf.if %cond {
        ...
        // CHECK: [[C_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[B_FULL]], [[B_READ_TOK]] {ttg.partition = array<i32: 0>}
        // CHECK: ttg.local_store {{.*}}, [[C_VIEWS]]#2 {ttg.partition = array<i32: 0>}
        // CHECK: nvws.semaphore.release [[C_FULL]], [[B_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
        ...
        // CHECK: [[C_READ_TOK:%.*]] = nvws.semaphore.acquire [[C_FULL]] {ttg.partition = array<i32: 1>}
        // CHECK: [[C_READ_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[C_FULL]], [[C_READ_TOK]] {ttg.partition = array<i32: 1>}
        // CHECK: ttg.local_load [[C_READ_VIEWS]]#2 {ttg.partition = array<i32: 1>}
        // CHECK: nvws.semaphore.release [[IF_BACK]], [[C_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
        // CHECK: [[IF_THEN_TOKEN:%.*]] = nvws.semaphore.acquire [[IF_BACK]] {ttg.partition = array<i32: 0>}
        // CHECK: scf.yield {{.*}}[[IF_THEN_TOKEN]] : !ttg.async.token
        ...
        // CHECK: scf.yield {{.*}}[[B_READ_TOK]] : !ttg.async.token
      } {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 0, 1>, ttg.partition.outputs = []}
      // CHECK: } {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 0, 1>, ttg.partition.outputs = [array<i32: 0>]}
      // CHECK: nvws.semaphore.release [[ENTRY]][{{%.*}}], [[IF_TOKEN]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
  ...
  tt.func @depth3_same_owner_atomic_if(%lb: i32, %ub: i32, %step: i32,
                                       %cond: i1) {
    // CHECK: [[A_BASE:%.*]] = ttg.local_alloc {buffer.copy = 3 : i32, buffer.id = 901 : i32}
    // CHECK: [[B_BASE:%.*]] = ttg.local_alloc {buffer.copy = 3 : i32, buffer.id = 901 : i32}
    // CHECK: [[C_BASE:%.*]] = ttg.local_alloc {buffer.copy = 3 : i32, buffer.id = 901 : i32}
    // CHECK: [[ENTRY:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] released = 7 {pending_count = 1 : i32}
    // CHECK: [[AB_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] {pending_count = 1 : i32}
    // CHECK: [[C_FULL:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] {pending_count = 1 : i32}
    // CHECK: [[IF_BACK:%.*]] = nvws.semaphore.create [[A_BASE]], [[B_BASE]], [[C_BASE]] {pending_count = 1 : i32}
    ...
    scf.for %iv = %lb to %ub step %step : i32 {
      ...
      // CHECK: [[AB_TOK:%.*]] = nvws.semaphore.acquire [[ENTRY]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
      // CHECK: [[AB_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[ENTRY]], [[AB_TOK]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
      ...
      // CHECK: nvws.semaphore.release [[AB_FULL]], [[AB_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>}
      ...
      // CHECK: [[AB_READ_TOK:%.*]] = nvws.semaphore.acquire [[AB_FULL]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
      // CHECK: [[AB_READ_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[AB_FULL]], [[AB_READ_TOK]] {loop.cluster = 5 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 0>}
      ...
      scf.if %cond {
        ...
        // CHECK: [[C_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[AB_FULL]], [[AB_READ_TOK]] {ttg.partition = array<i32: 0>}
        // CHECK: ttg.local_store {{.*}}, [[C_VIEWS]]#2 {ttg.partition = array<i32: 0>}
        // CHECK: nvws.semaphore.release [[C_FULL]], [[AB_READ_TOK]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
        ttg.local_store %c_value, %c {ttg.partition = array<i32: 0>} : tensor<128x128xf16, #blocked128> -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
        // The C read hands ownership back to owner 0 before the branch yields.
        // CHECK: [[C_READ_TOK:%.*]] = nvws.semaphore.acquire [[C_FULL]] {ttg.partition = array<i32: 1>}
        // CHECK: [[C_READ_VIEWS:%.*]]:3 = nvws.semaphore.buffer [[C_FULL]], [[C_READ_TOK]] {ttg.partition = array<i32: 1>}
        ...
        // CHECK: scf.yield {{.*}}[[AB_READ_TOK]] : !ttg.async.token
      } {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 0, 1>, ttg.partition.outputs = []}
      // CHECK: } {loop.cluster = 6 : i32, loop.stage = 2 : i32, ttg.partition = array<i32: 0, 1>, ttg.partition.outputs = [array<i32: 0>]}
      // CHECK: nvws.semaphore.release [[ENTRY]], [[IF_TOKEN]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
...
// Check the semaphore pending_count and the scaled release arrive_count in
// the emitted IR.
...
module attributes {"ttg.num-warps" = 4 : i32} {
  ...
  tt.func @release_multiplicity_unified_fanin_regain(%lb: i32, %ub: i32, %step: i32) {
    // CHECK: [[V1:%.*]] = ttg.local_alloc {buffer.id = 700 : i32} : () -> !ttg.memdesc<1x1xi32, #shared, #smem, mutable>
    ...
    // CHECK: [[V4:%.*]] = nvws.semaphore.create [[V1]] {pending_count = 1 : i32} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>
    // CHECK: [[V5:%.*]] = nvws.semaphore.create [[V1]] {pending_count = 1 : i32} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>
    // CHECK: [[V6:%.*]] = nvws.semaphore.create [[V1]] {pending_count = 2 : i32} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>
    // CHECK-NOT: iter_args
    scf.for %i = %lb to %ub step %step : i32 {
      ...
      // CHECK: [[V7:%.*]] = nvws.semaphore.acquire [[V2]] {ttg.partition = array<i32: 3>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: [[V8:%.*]] = nvws.semaphore.buffer [[V2]], [[V7]] {ttg.partition = array<i32: 3>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<1xi32, #shared, #smem, mutable>
      // CHECK: ttg.local_store %{{[-A-Za-z0-9_.$#]+}}, [[V8]] {ttg.partition = array<i32: 3>} : tensor<1xi32, #blocked> -> !ttg.memdesc<1xi32, #shared, #smem, mutable>
      // CHECK: nvws.semaphore.release [[V6]], [[V7]] [#nvws.async_op<none>] {arrive_count = 2 : i32, ttg.partition = array<i32: 3>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token
      ttg.local_store %v, %alloc {ttg.partition = array<i32: 3>} : !ty -> !ttg.memdesc<1xi32, #shared, #smem, mutable>
      // CHECK-NOT: iter_args
      scf.for %j = %lb to %ub step %step : i32 {
        // CHECK: [[V9:%.*]] = nvws.semaphore.acquire [[V6]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]> -> !ttg.async.token
        // CHECK: nvws.semaphore.release [[V3]], [[V9]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token
        // CHECK: [[V10:%.*]] = nvws.semaphore.buffer [[V6]], [[V9]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<1xi32, #shared, #smem, mutable>
        // CHECK: ttg.local_load [[V10]] {ttg.partition = array<i32: 2>} : !ttg.memdesc<1xi32, #shared, #smem, mutable> -> tensor<1xi32, #blocked>
        // CHECK: nvws.semaphore.release [[V4]], [[V9]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token
        %l2 = ttg.local_load %alloc {ttg.partition = array<i32: 2>} : !ttg.memdesc<1xi32, #shared, #smem, mutable> -> !ty
        "use2"(%l2) {ttg.partition = array<i32: 2>} : (!ty) -> ()
        // CHECK: [[V11:%.*]] = nvws.semaphore.acquire [[V3]] {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]> -> !ttg.async.token
        // CHECK: [[V12:%.*]] = nvws.semaphore.buffer [[V3]], [[V11]] {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<1xi32, #shared, #smem, mutable>
        // CHECK: ttg.local_load [[V12]] {ttg.partition = array<i32: 1>} : !ttg.memdesc<1xi32, #shared, #smem, mutable> -> tensor<1xi32, #blocked>
        %l1 = ttg.local_load %alloc {ttg.partition = array<i32: 1>} : !ttg.memdesc<1xi32, #shared, #smem, mutable> -> !ty
        %c = "correct"(%l1) {ttg.partition = array<i32: 1>} : (!ty) -> !ty
        // CHECK: [[V13:%.*]] = nvws.semaphore.acquire [[V4]] {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]> -> !ttg.async.token
        // CHECK: [[V14:%.*]] = nvws.semaphore.buffer [[V4]], [[V13]] {ttg.partition = array<i32: 1>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<1xi32, #shared, #smem, mutable>
        // CHECK: ttg.local_store %{{[-A-Za-z0-9_.$#]+}}, [[V14]] {ttg.partition = array<i32: 1>} : tensor<1xi32, #blocked> -> !ttg.memdesc<1xi32, #shared, #smem, mutable>
        // CHECK: nvws.semaphore.release [[V5]], [[V13]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token
        // CHECK: nvws.semaphore.release [[V6]], [[V13]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token
        ttg.local_store %c, %alloc {ttg.partition = array<i32: 1>} : !ty -> !ttg.memdesc<1xi32, #shared, #smem, mutable>
        // CHECK: [[V15:%.*]] = nvws.semaphore.acquire [[V5]] {ttg.partition = array<i32: 0>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]> -> !ttg.async.token
        // CHECK: [[V16:%.*]] = nvws.semaphore.buffer [[V5]], [[V15]] {ttg.partition = array<i32: 0>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<1xi32, #shared, #smem, mutable>
        // CHECK: ttg.local_load [[V16]] {ttg.partition = array<i32: 0>} : !ttg.memdesc<1xi32, #shared, #smem, mutable> -> tensor<1xi32, #blocked>
        %l0 = ttg.local_load %alloc {ttg.partition = array<i32: 0>} : !ttg.memdesc<1xi32, #shared, #smem, mutable> -> !ty
        // CHECK: nvws.semaphore.release [[V6]], [[V15]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token
        "use0"(%l0) {ttg.partition = array<i32: 0>} : (!ty) -> ()
      } {ttg.partition = array<i32: 0, 1, 2>}
      ...
      // CHECK: [[V17:%.*]] = nvws.semaphore.acquire [[V6]] {ttg.partition = array<i32: 2>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]> -> !ttg.async.token
      // CHECK: nvws.semaphore.release [[V2]], [[V17]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]>, !ttg.async.token
    } {tt.warp_specialize, ttg.partition = array<i32: 0, 1, 2, 3>, ttg.partition.stages = [0 : i32, 0 : i32, 0 : i32, 1 : i32], ttg.warp_specialize.tag = 0 : i32}
    tt.return
  }
}
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  // CHECK-LABEL: @direct_descriptor_store_completion
  tt.func @direct_descriptor_store_completion(%desc: !tt.tensordesc<128x64xf16, #shared>, %i: i32, %lb: i32, %ub: i32, %step: i32) {
    // CHECK: [[V1:%.*]] = ttg.local_alloc {buffer.id = 600 : i32} : () -> !ttg.memdesc<1x128x64xf16, #shared, #smem, mutable>
    // CHECK: [[EMPTY:%.*]] = nvws.semaphore.create [[V1]] released = 1 {pending_count = 1 : i32}
    // CHECK-NEXT: [[FULL:%.*]] = nvws.semaphore.create [[V1]] {pending_count = 1 : i32}
    %alloc = ttg.local_alloc {buffer.id = 600 : i32} : () -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
    // CHECK: [[ENTRY:%.*]] = nvws.semaphore.acquire [[EMPTY]]
    // CHECK-NEXT: [[LOOP:%.*]] = scf.for %{{.*}} = %{{.*}} to %{{.*}} step %{{.*}} iter_args([[WRITE:%.*]] = [[ENTRY]]) -> (!ttg.async.token)  : i32 {
    scf.for %iv = %lb to %ub step %step : i32 {
      %first = "producer"() {ttg.partition = array<i32: 0>} : () -> tensor<128x64xf16, #blocked>
      // CHECK: [[WRITE_BUF:%.*]] = nvws.semaphore.buffer [[EMPTY]], [[WRITE]] {ttg.partition = array<i32: 0>}
      // CHECK-NEXT: ttg.local_store %{{.*}}, [[WRITE_BUF]] {ttg.partition = array<i32: 0>}
      // CHECK-NEXT: nvws.semaphore.release [[FULL]], [[WRITE]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 0>}
      ...
      // CHECK-NEXT: [[READ_BUF:%.*]] = nvws.semaphore.buffer [[FULL]], [[READ]] {ttg.partition = array<i32: 1>}
      // CHECK-NEXT: [[LOADED:%.*]] = ttg.local_load [[READ_BUF]] {ttg.partition = array<i32: 1>}
      ...
      // CHECK-NEXT: tt.descriptor_store %{{.*}}[%{{.*}}, %{{.*}}], [[LOADED]] {ttg.partition = array<i32: 1>}
      // CHECK-NEXT: nvws.semaphore.release [[EMPTY]], [[READ]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
      ...
      // CHECK: [[NEXT_WRITE:%.*]] = nvws.semaphore.acquire [[EMPTY]] {ttg.partition = array<i32: 0>}
      // CHECK-NEXT: [[NEXT_BUF:%.*]] = nvws.semaphore.buffer [[EMPTY]], [[NEXT_WRITE]] {ttg.partition = array<i32: 0>}
      // CHECK-NEXT: ttg.local_store %{{.*}}, [[NEXT_BUF]] {ttg.partition = array<i32: 0>}
      ...
      // CHECK-NEXT: scf.yield {ttg.partition = array<i32: 0, 1>} [[NEXT_WRITE]] : !ttg.async.token
    } {tt.warp_specialize, ttg.partition = array<i32: 0, 1>, ttg.warp_specialize.tag = 0 : i32}
    // CHECK: } {tt.warp_specialize, ttg.partition = array<i32: 0, 1>, ttg.partition.outputs = [array<i32: 0>], ttg.warp_specialize.tag = 0 : i32}
    tt.return
  }
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  // CHECK-LABEL: @already_lowered_tma_store_handoffs
  tt.func @already_lowered_tma_store_handoffs(
    ...
    // CHECK: [[BASE:%.*]] = ttg.local_alloc
    // CHECK: [[ENTRY:%.*]] = nvws.semaphore.create [[BASE]], [[BASE]] released = 3
    // CHECK-NEXT: [[COPY_READY:%.*]] = nvws.semaphore.create [[BASE]], [[BASE]]
    // CHECK-NEXT: [[M1_READY:%.*]] = nvws.semaphore.create [[BASE]], [[BASE]]
    // CHECK-NEXT: [[REDUCE_READY:%.*]] = nvws.semaphore.create [[BASE]], [[BASE]]
    ...
    scf.for %i = %lb to %ub step %step : i32 {
      // CHECK: [[ZERO_P0:%.*]] = arith.constant {ttg.partition = array<i32: 0>} 0 : i32
      // CHECK-NEXT: [[W0_TOKEN:%.*]] = nvws.semaphore.acquire [[ENTRY]][[[ZERO_P0]]]
      // CHECK-NEXT: [[W0_BUFFER:%.*]]:2 = nvws.semaphore.buffer [[ENTRY]], [[W0_TOKEN]]
      // CHECK-NEXT: ttg.local_store %{{.*}}, [[W0_BUFFER]]#0
      // CHECK-NEXT: nvws.semaphore.release [[COPY_READY]][[[ZERO_P0]]], [[W0_TOKEN]]
      ...
      // CHECK-NEXT: [[ZERO_P1:%.*]] = arith.constant {ttg.partition = array<i32: 1>} 0 : i32
      // CHECK-NEXT: [[COPY_TOKEN:%.*]] = nvws.semaphore.acquire [[COPY_READY]][[[ZERO_P1]]]
      // CHECK-NEXT: [[COPY_BUFFER:%.*]]:2 = nvws.semaphore.buffer [[COPY_READY]], [[COPY_TOKEN]]
      // CHECK-NEXT: [[COPY:%.*]] = ttng.async_tma_copy_local_to_global %{{.*}} [[COPY_BUFFER]]#0
      // CHECK-NEXT: ttng.async_tma_store_token_wait [[COPY]]
      // CHECK-NEXT: [[TO_M1:%.*]] = arith.constant {{.*}} 1 : i32
      // CHECK-NEXT: nvws.semaphore.release [[M1_READY]][[[TO_M1]]], [[COPY_TOKEN]]
      ...
      // CHECK-NEXT: [[W1_TOKEN:%.*]] = nvws.semaphore.acquire [[M1_READY]][[[ZERO_P0]]]
      // CHECK-NEXT: [[W1_BUFFER:%.*]]:2 = nvws.semaphore.buffer [[M1_READY]], [[W1_TOKEN]]
      // CHECK-NEXT: ttg.local_store %{{.*}}, [[W1_BUFFER]]#1
      // CHECK-NEXT: nvws.semaphore.release [[REDUCE_READY]][[[ZERO_P0]]], [[W1_TOKEN]]
      ...
      // CHECK-NEXT: [[REDUCE_TOKEN:%.*]] = nvws.semaphore.acquire [[REDUCE_READY]][[[ZERO_P1]]]
      // CHECK-NEXT: [[REDUCE_BUFFER:%.*]]:2 = nvws.semaphore.buffer [[REDUCE_READY]], [[REDUCE_TOKEN]]
      // CHECK-NEXT: [[REDUCE:%.*]] = ttng.async_tma_reduce add, %{{.*}} [[REDUCE_BUFFER]]#1
      // CHECK-NEXT: ttng.async_tma_store_token_wait [[REDUCE]]
      // CHECK-NEXT: nvws.semaphore.release [[ENTRY]][[[TO_M1]]], [[REDUCE_TOKEN]]
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  // CHECK-LABEL: @converted_descriptor_store_completion
  tt.func @converted_descriptor_store_completion(%desc: !tt.tensordesc<128x64xf16, #shared>, %i: i32, %lb: i32, %ub: i32, %step: i32) {
    // CHECK: [[V1:%.*]] = ttg.local_alloc {buffer.id = 601 : i32} : () -> !ttg.memdesc<1x128x64xf16, #shared, #smem, mutable>
    // CHECK: [[EMPTY:%.*]] = nvws.semaphore.create [[V1]] released = 1 {pending_count = 1 : i32}
    // CHECK-NEXT: [[FULL:%.*]] = nvws.semaphore.create [[V1]] {pending_count = 1 : i32}
    ...
    scf.for %iv = %lb to %ub step %step : i32 {
      ...
      // CHECK-NEXT: [[CONVERTED:%.*]] = ttg.convert_layout [[LOADED]] {ttg.partition = array<i32: 1>}
      ...
      // CHECK-NEXT: tt.descriptor_store %{{.*}}[%{{.*}}, %{{.*}}], [[CONVERTED]] {ttg.partition = array<i32: 1>}
      // CHECK-NEXT: nvws.semaphore.release [[EMPTY]], [[READ]] [#nvws.async_op<none>] {arrive_count = 1 : i32, ttg.partition = array<i32: 1>}
...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @legal_cross_partition_backpressure(%lb: i32, %ub: i32, %step: i32) {
    ...
    scf.for %iv = %lb to %ub step %step : i32 {
      ...
      // CHECK: [[V7:%.*]] = nvws.semaphore.buffer [[V2]][[[V4]]], [[V5]] {loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<4x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
      ...
      // CHECK: nvws.semaphore.release [[V3]][[[V4]]], [[V5]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 2>} : <[!ttg.memdesc<4x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[V12:%.*]] = nvws.semaphore.buffer [[V2]][[[V9]]], [[V10]] {loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>} : <[!ttg.memdesc<4x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
      ...
      // CHECK: nvws.semaphore.release [[V3]][[[V9]]], [[V10]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = array<i32: 1>} : <[!ttg.memdesc<4x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[V17:%.*]] = nvws.semaphore.buffer [[V3]][[[V14]]], [[V15]] {loop.cluster = 2 : i32, loop.stage = 3 : i32, ttg.partition = array<i32: 0>} : <[!ttg.memdesc<4x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
      ...
      // CHECK: nvws.semaphore.release [[V2]][[[V14]]], [[V15]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 2 : i32, loop.stage = 3 : i32, ttg.partition = array<i32: 0>} : <[!ttg.memdesc<4x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token
      ...
      // CHECK: [[V23:%.*]] = nvws.semaphore.buffer [[V3]][[[V20]]], [[V21]] {loop.cluster = 2 : i32, loop.stage = 3 : i32, ttg.partition = array<i32: 0>} : <[!ttg.memdesc<4x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
      ...
      // CHECK: nvws.semaphore.release [[V2]][[[V20]]], [[V21]] [#nvws.async_op<none>] {arrive_count = 1 : i32, loop.cluster = 2 : i32, loop.stage = 3 : i32, ttg.partition = array<i32: 0>} : <[!ttg.memdesc<4x128x64xf16, #shared, #smem, mutable>]>, !ttg.async.token
...
// TMEM alias views under the default and Meta-partitioner configurations.
