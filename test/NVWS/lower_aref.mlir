...
// --nvws-lower-aref -canonicalize | FileCheck %s

#blocked =                                                                     \
    #ttg.blocked <                                                             \
    {sizePerThread = [1], threadsPerWarp = [32], warpsPerCTA = [4],            \
                                                                order = [0] }>
#shared = #ttg.swizzled_shared < {vec = 1, perPhase = 1, maxPhase = 1,         \
                                 order = [0] }>
...
module attributes{"ttg.num-warps" = 4 :i32} {
  ...
  tt.func @two_consumers(% arg0 : i32, % arg1 : i32, % arg2 : i32) {
    ...
    scf.for %arg3 = %arg0 to %arg1 step %arg2 : i32 {
      ...
      %buffers, %token = nvws.aref.put.enter %1[%c0_i32, %c0_i32] {
      loop.cluster = 1 : i32, loop.stage = 3 : i32,
      ttg.partition = array < i32 : 0 >} : <[!ttg.memdesc<3x1xi32, #shared, #smem, mutable>]> -> !ttg.memdesc<1xi32, #shared, #smem, mutable, 1x1>, !ttg.async.token
      ...
      %buffers_0, %token_1 = nvws.aref.get.enter %1[%c0_i32, %c0_i32] {
      loop.cluster = 2 : i32, loop.stage = 3 : i32,
      ttg.partition = array < i32 : 1 >} : <[!ttg.memdesc<3x1xi32, #shared, #smem, mutable>]> -> !ttg.memdesc<1xi32, #shared, #smem, mutable, 1x1>, !ttg.async.token
      ...
      %buffers_2, %token_3 = nvws.aref.get.enter %1[%c0_i32, %c0_i32] {
      loop.cluster = 3 : i32, loop.stage = 4 : i32,
      ttg.partition = array < i32 : 2 >} : <[!ttg.memdesc<3x1xi32, #shared, #smem, mutable>]> -> !ttg.memdesc<1xi32, #shared, #smem, mutable, 1x1>, !ttg.async.token
  ...
  tt.func @three_consumers(% arg0 : i32, % arg1 : i32, % arg2 : i32) {
    ...
    scf.for %arg3 = %arg0 to %arg1 step %arg2 : i32 {
      ...
      "op_c"(%20) {ttg.partition = array<i32: 2>} : (tensor<1xi32, #blocked>) -> ()
      "op_d"(%20) {ttg.partition = array<i32: 2>} : (tensor<1xi32, #blocked>) -> ()
      %buffers_4, %token_5 = nvws.aref.get.enter %1[%c0_i32, %c0_i32] {
      ttg.partition = array < i32 : 3 >} : <[!ttg.memdesc<3x1xi32, #shared, #smem, mutable>]> -> !ttg.memdesc<1xi32, #shared, #smem, mutable, 1x1>, !ttg.async.token
  ...
  tt.func @reuse_argument(% arg0 : i32, % arg1 : i32, % arg2 : i32) {
    ...
    scf.for %arg3 = %arg0 to %arg1 step %arg2 iter_args(%arg5 = %cst) -> (tensor<1xi32, #blocked>)  : i32 {
      ...
      // CHECK: op_a
      %buffers, %token = nvws.aref.put.enter %1[%c0_i32, %c0_i32] {
      ttg.partition = array < i32 : 0 >} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]> -> !ttg.memdesc<1xi32, #shared, #smem, mutable, 1x1>, !ttg.async.token
      ...
      // CHECK: op_d
      %buffers_1, %token_2 = nvws.aref.get.enter %1[%c0_i32, %c0_i32] {
      ttg.partition = array < i32 : 1 >} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]> -> !ttg.memdesc<1xi32, #shared, #smem, mutable, 1x1>, !ttg.async.token
      ...
      // CHECK-NEXT: ttng.arrive_barrier [[EMPTYMBAR1]], 1
      // CHECK: op_d
      %buffers_3, %token_4 = nvws.aref.get.enter %1[%c0_i32, %c0_i32] {
      ttg.partition = array < i32 : 2 >} : <[!ttg.memdesc<1x1xi32, #shared, #smem, mutable>]> -> !ttg.memdesc<1xi32, #shared, #smem, mutable, 1x1>, !ttg.async.token
...
#shared = #ttg.nvmma_shared < {swizzlingByteWidth = 128, transposed = false,   \
                              elementBitWidth = 16 }>
#shared1 = #ttg.swizzled_shared < {vec = 1, perPhase = 1, maxPhase = 1,        \
                                  order = [0] }>
#shared2 = #ttg.nvmma_shared < {swizzlingByteWidth = 128, transposed = true,   \
                               elementBitWidth = 16 }>
#smem = #ttg.shared_memory
#tmem = #ttng.tensor_memory_encoding < blockM = 128, blockN = 128,             \
                                       colStride = 1>
module attributes{"ttg.num-warps" = 4 :i32, ttg.target = "cuda:100"} {
  tt.func @warp_specialize_tma_matmul(
      % arg0 : i32, % arg1 : i32, % arg2 : i32,
      % arg3 : !tt.tensordesc<tensor<128x64xf16, #shared>>,
      % arg4 : !tt.tensordesc<tensor<128x64xf16, #shared>>) {
    ...
    %10 = scf.for %arg5 = %c0_i32 to %arg0 step %c1_i32 iter_args(%arg6 = %2) -> (!ttg.async.token)  : i32 {
      ...
      %buffers, %token_2 = nvws.aref.put.enter %4[%c0_i32, %c0_i32] {
      loop.cluster = 1 : i32, loop.stage = 0 : i32,
      ttg.partition = array < i32 : 2 >} : <[!ttg.memdesc<1x128x64xf16, #shared, #smem, mutable>]> -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable, 1x128x64>, !ttg.async.token
      ...
      %buffers_3, %token_4 = nvws.aref.get.enter %4[%c0_i32, %c0_i32] {
      loop.cluster = 0 : i32, loop.stage = 1 : i32,
      ttg.partition = array < i32 : 1 >} : <[!ttg.memdesc<1x128x64xf16, #shared, #smem, mutable>]> -> !ttg.memdesc<128x64xf16, #shared, #smem, 1x128x64>, !ttg.async.token
      %buffers_5, %token_6 = nvws.aref.put.enter %6[%c0_i32, %c0_i32] {
      loop.cluster = 1 : i32, loop.stage = 0 : i32,
      ttg.partition = array < i32 : 2 >} : <[!ttg.memdesc<1x128x64xf16, #shared, #smem, mutable>]> -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable, 1x128x64>, !ttg.async.token
      ...
      %buffers_7, %token_8 = nvws.aref.get.enter %6[%c0_i32, %c0_i32] {
      loop.cluster = 0 : i32, loop.stage = 1 : i32,
      ttg.partition = array < i32 : 1 >} : <[!ttg.memdesc<1x128x64xf16, #shared, #smem, mutable>]> -> !ttg.memdesc<128x64xf16, #shared, #smem, 1x128x64>, !ttg.async.token
      ...
      %14 = ttng.tc_gen5_mma %buffers_3, %12, %1[], %true, %true, %9[%13] {
      is_async, loop.cluster = 0 : i32, loop.stage = 1 : i32,
                tt.self_latency = 1 : i32, ttg.partition = array < i32 : 1 >} : !ttg.memdesc<128x64xf16, #shared, #smem, 1x128x64>, !ttg.memdesc<64x128xf16, #shared2, #smem, 1x64x128>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable, 1x128x128>, !ttg.memdesc<1xi64, #shared1, #smem, mutable, 1x1>
...
#smem = #ttg.shared_memory
module attributes{"ttg.num-warps" = 4 :i32, ttg.target = "cuda:100"} {
  tt.func @load_used_as_reg_and_smem(
      % arg0 : !tt.tensordesc<tensor<128x64xf16, #shared>>, % arg1 : i32) {
    %c1_i32 = arith.constant 1 : i32
    %c0_i32 = arith.constant 0 : i32
    ...
    scf.for %arg2 = %c0_i32 to %arg1 step %c1_i32  : i32 {
      ...
      %buffers_0, %token_1 = nvws.aref.get.enter %1[%c0_i32, %c0_i32] {
      loop.cluster = 0 : i32, loop.stage = 1 : i32,
      ttg.partition = array < i32 : 0 >} : <[!ttg.memdesc<1x128x64xf16, #shared, #smem, mutable>]> -> !ttg.memdesc<128x64xf16, #shared, #smem, 1x128x64>, !ttg.async.token
...
#smem = #ttg.shared_memory
module attributes{"ttg.num-warps" = 4 :i32, ttg.target = "cuda:100"} {
  tt.func @load_used_as_reg_and_smem_same_partition(
      % arg0 : !tt.tensordesc<tensor<128x64xf16, #shared>>, % arg1 : i32) {
    %c1_i32 = arith.constant 1 : i32
    %c0_i32 = arith.constant 0 : i32
    // CHECK: [[EMPTY:%.*]] = ttg.local_alloc : () -> !ttg.memdesc<3x1xi64
    ...
    scf.for %arg2 = %c0_i32 to %arg1 step %c1_i32  : i32 {
      %buffers, %token = nvws.aref.put.enter %1[%c0_i32, %c0_i32] {
      loop.cluster = 1 : i32, loop.stage = 0 : i32,
      ttg.partition = array < i32 : 1 >} : <[!ttg.memdesc<1x128x64xf16, #shared, #smem, mutable>]> -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable, 1x128x64>, !ttg.async.token
