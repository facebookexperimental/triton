...
// A shared-memory buffer written in one sibling region and read in another
// cannot be represented by the current forward-only sibling protocol. This
// must be rejected before buffer allocation mutates the function.
// CHECK-LABEL: @sibling_smem_channel
// CHECK-NOT: warp_specialize
// CHECK-NOT: async_task_id
// CHECK-NOT: ttg.partition
...
module attributes {"ttg.cluster-dim-x" = 2 : i32, "ttg.cluster-dim-y" = 2 : i32, "ttg.cluster-dim-z" = 1 : i32, ttg.early_tma_store_lowering = true, ttg.min_reg_auto_ws = 24 : i32, "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32, "ttng.two-ctas" = false} {
  ...
  tt.func public @sibling_smem_channel() {
    %c0_i32 = arith.constant 0 : i32
    %c1_i32 = arith.constant 1 : i32
    %true = arith.constant true
    %value = arith.constant dense<0.000000e+00> : tensor<128x64xf16, #blocked>
    %buffer = ttg.local_alloc : () -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
    scf.for %i = %c0_i32 to %c1_i32 step %c1_i32 : i32 {
      scf.if %true {
        %loaded = ttg.local_load %buffer {ttg.partition = array<i32: 0>} : !ttg.memdesc<128x64xf16, #shared, #smem, mutable> -> tensor<128x64xf16, #blocked>
      }
      scf.if %true {
        ttg.local_store %value, %buffer {ttg.partition = array<i32: 1>} : tensor<128x64xf16, #blocked> -> !ttg.memdesc<128x64xf16, #shared, #smem, mutable>
      }
    } {tt.warp_specialize, ttg.partition.stages = [0 : i32, 0 : i32], ttg.partition.types = ["epilogue", "gemm"], ttg.warp_specialize.tag = 2 : i32}
    tt.return
  }
}
