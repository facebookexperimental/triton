...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  // CHECK:       ttng.tc_gen5_mma {{.*}} {ttg.partition = array<i32: 0>}
  tt.func public @direct_tmem_d_init_store_gets_mma_task(%a: !ttg.memdesc<128x64xf16, #shared, #smem>, %b: !ttg.memdesc<64x128xf16, #shared1, #smem>, %n_tiles: i32) {
    ...
    %result = scf.for %iv = %c0 to %n_tiles step %c1 iter_args(%dep = %acc_token) -> (!ttg.async.token) : i32 {
      %mma_token = ttng.tc_gen5_mma %a, %b, %acc[%dep], %false, %true {ttg.partition = array<i32: 1>} : !ttg.memdesc<128x64xf16, #shared, #smem>, !ttg.memdesc<64x128xf16, #shared1, #smem>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @subslice_tmem_d_init_store_gets_mma_task(%a: !ttg.memdesc<128x64xf16, #shared, #smem>, %b: !ttg.memdesc<64x64xf16, #shared1, #smem>, %n_tiles: i32) {
    ...
    %result = scf.for %iv = %c0 to %n_tiles step %c1 iter_args(%dep = %acc_token) -> (!ttg.async.token) : i32 {
      %mma_token = ttng.tc_gen5_mma %a, %b, %acc[%dep], %false, %true {ttg.partition = array<i32: 1>} : !ttg.memdesc<128x64xf16, #shared, #smem>, !ttg.memdesc<64x64xf16, #shared1, #smem>, !ttg.memdesc<128x64xf32, #tmem1, #ttng.tensor_memory, mutable, 128x128>
      scf.yield %mma_token : !ttg.async.token
    }
    tt.return
  }
