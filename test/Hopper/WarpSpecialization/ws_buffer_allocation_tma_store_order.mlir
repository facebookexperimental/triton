...
module attributes {"ttg.cluster-dim-x" = 1 : i32, "ttg.cluster-dim-y" = 1 : i32, "ttg.cluster-dim-z" = 1 : i32, "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @tma_store_local_store_order(%c_desc: !tt.tensordesc<128x128xf16, #shared>) attributes {noinline = false} {
    ...
    %acc1 = arith.constant {ttg.partition = array<i32: 0>} dense<1.000000e+00> : tensor<128x256xf32, #linear1>
    %row1 = arith.addi %c0_i32, %c128_i32 {ttg.partition = array<i32: 2>} : i32
    %col1 = arith.addi %c0_i32, %c128_i32 {ttg.partition = array<i32: 2>} : i32
    %acc0_reshape = tt.reshape %acc0 {ttg.partition = array<i32: 0>} : tensor<128x256xf32, #linear1> -> tensor<128x2x128xf32, #linear2>
    %acc1_reshape = tt.reshape %acc1 {ttg.partition = array<i32: 0>} : tensor<128x256xf32, #linear1> -> tensor<128x2x128xf32, #linear2>
    %acc0_trans = tt.trans %acc0_reshape {ttg.partition = array<i32: 0>, order = array<i32: 0, 2, 1>} : tensor<128x2x128xf32, #linear2> -> tensor<128x128x2xf32, #linear3>
    %acc1_trans = tt.trans %acc1_reshape {ttg.partition = array<i32: 0>, order = array<i32: 0, 2, 1>} : tensor<128x2x128xf32, #linear2> -> tensor<128x128x2xf32, #linear3>
    %out0_lhs, %out0_rhs = tt.split %acc0_trans {ttg.partition = array<i32: 0>} : tensor<128x128x2xf32, #linear3> -> tensor<128x128xf32, #linear>
    %out1_lhs, %out1_rhs = tt.split %acc1_trans {ttg.partition = array<i32: 0>} : tensor<128x128x2xf32, #linear3> -> tensor<128x128xf32, #linear>
    %c0_a = arith.truncf %out0_lhs {ttg.partition = array<i32: 0>} : tensor<128x128xf32, #linear> to tensor<128x128xf16, #linear>
