...
//   flattened = accumCnt * numTiles(2) + tileIdx   (extui i32 -> i64)
//   bufferIdx = flattened % numBuffers(3)
//   view      = memdesc_index[base, bufferIdx]
// accumCnt and the representative multibuffer alloc (3x128x64) are threaded as
// shared args; the producer local_store dest / consumer async_tma_copy source are
// rewired to the in-body view; and the now-dead per-tile buffer positions are
// removed (SubtiledRegionOp::removePerTilePosition). columnOffset / data-leaf
// positions stay per-tile.
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  // In-body flattened count (accumCnt * numTiles(2) + tileIdx) % numBuffers(3),
  ...
  // CHECK:      arith.muli %{{[0-9a-z_]+}}, %c2_i64
