...
module attributes {"ttg.cluster-dim-x" = 1 : i32, "ttg.cluster-dim-y" = 1 : i32, "ttg.cluster-dim-z" = 1 : i32, "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @load_order_breaks_multibuffer_tie(
    ...
    scf.for %iv = %c0 to %c10 step %c1 : i32 {
      %x = tt.descriptor_load %x_desc[%c0, %c0] {ttg.partition = array<i32: 1>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : !tt.tensordesc<128x256xf16, #shared> -> tensor<128x256xf16, #blocked1>
      ...
      %w = tt.descriptor_load %w_desc[%c0, %c0] {ttg.partition = array<i32: 1>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : !tt.tensordesc<128x256xf16, #shared> -> tensor<128x256xf16, #blocked1>
