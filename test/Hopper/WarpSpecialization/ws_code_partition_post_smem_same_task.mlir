...
module attributes {"ttg.cluster-dim-x" = 1 : i32, "ttg.cluster-dim-y" = 1 : i32, "ttg.cluster-dim-z" = 1 : i32, "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @same_partition_smem_post_no_channel(%src: tensor<16xf32, #blocked>) attributes {noinline = false} {
    ...
    ttg.local_store %src, %alloc {ttg.partition = array<i32: 0>} : tensor<16xf32, #blocked> -> !ttg.memdesc<16xf32, #shared, #smem, mutable>
    %loaded = ttg.local_load %alloc {ttg.partition = array<i32: 0>} : !ttg.memdesc<16xf32, #shared, #smem, mutable> -> tensor<16xf32, #blocked>
    %used = arith.addf %loaded, %loaded {ttg.partition = array<i32: 0>} : tensor<16xf32, #blocked>
    tt.return
  }
}
