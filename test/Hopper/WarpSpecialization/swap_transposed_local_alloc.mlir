...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @swap_transposed_alloc(%desc_k: !tt.tensordesc<128x128xbf16, #shared>, %desc_q: !tt.tensordesc<128x128xbf16, #shared>) {
    ...
    %loop:4 = scf.for %iv = %c0_i32 to %c4_i32 step %c1_i32 iter_args(%use_d = %false, %dk_dep = %dk_token, %dq_dep = %dq_token, %prev = %true) -> (i1, !ttg.async.token, !ttg.async.token, i1) : i32 {
      ...
      %dq_alloc = ttg.local_alloc %dsT_val {ttg.partition = array<i32: 3>} : (tensor<128x128xbf16, #blocked>) -> !ttg.memdesc<128x128xbf16, #shared_T, #smem>
      %dq_trans = ttg.memdesc_trans %dq_alloc {ttg.partition = array<i32: 0>, order = array<i32: 1, 0>} : !ttg.memdesc<128x128xbf16, #shared_T, #smem> -> !ttg.memdesc<128x128xbf16, #shared, #smem>
      %dq_tok = ttng.tc_gen5_mma %dq_trans, %k_smem, %dq[%dq_dep], %use_d, %true {ttg.partition = array<i32: 0>} : !ttg.memdesc<128x128xbf16, #shared, #smem>, !ttg.memdesc<128x128xbf16, #shared, #smem>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @no_swap_operand_b(%desc_k: !tt.tensordesc<128x128xbf16, #shared_2>) {
    ...
    %loop:2 = scf.for %iv = %c0_i32 to %c4_i32 step %c1_i32 iter_args(%use_d = %false, %dep = %acc_token) -> (i1, !ttg.async.token) : i32 {
      ...
      %tok = ttng.tc_gen5_mma %a_smem, %b_trans, %acc[%dep], %use_d, %true {ttg.partition = array<i32: 0>} : !ttg.memdesc<128x128xbf16, #shared_2, #smem_2>, !ttg.memdesc<128x128xbf16, #shared_2, #smem_2>, !ttg.memdesc<128x128xf32, #tmem_2, #ttng.tensor_memory, mutable>
