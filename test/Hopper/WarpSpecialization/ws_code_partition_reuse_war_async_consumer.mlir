...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @reuse_war_async(%desc_k: !tt.tensordesc<tensor<128x128xf16, #shared>>, %desc_v: !tt.tensordesc<tensor<128x128xf16, #shared>>) attributes {noinline = false} {
    ...
    %c1024_i32 = arith.constant {async_task_id = array<i32: 0, 1, 2>} 1024 : i32
    ...
    %qload = tt.descriptor_load %desc_k[%c0_i32, %c0_i32] {async_task_id = array<i32: 1>} : !tt.tensordesc<tensor<128x128xf16, #shared>> -> tensor<128x128xf16, #blocked1>
    ttg.local_store %qload, %q {async_task_id = array<i32: 1>} : tensor<128x128xf16, #blocked1> -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
    scf.for %t = %c0_i32 to %c128_i32 step %c128_i32  : i32 {
      %r:4 = scf.for %i = %c0_i32 to %c1024_i32 step %c128_i32 iter_args(%off = %c0_i32, %uc = %false, %qkt = %qk_tok, %at = %acc_tok) -> (i32, i1, !ttg.async.token, !ttg.async.token)  : i32 {
        ...
        %kl = tt.descriptor_load %desc_k[%off, %c0_i32] {async_task_id = array<i32: 1>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : !tt.tensordesc<tensor<128x128xf16, #shared>> -> tensor<128x128xf16, #blocked1>
        ttg.local_store %kl, %k {async_task_id = array<i32: 1>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x128xf16, #blocked1> -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
        %kt = ttg.memdesc_trans %k {async_task_id = array<i32: 2>, loop.cluster = 1 : i32, loop.stage = 0 : i32, order = array<i32: 1, 0>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> !ttg.memdesc<128x128xf16, #shared1, #smem, mutable>
        %vl = tt.descriptor_load %desc_v[%off, %c0_i32] {async_task_id = array<i32: 1>, loop.cluster = 2 : i32, loop.stage = 1 : i32} : !tt.tensordesc<tensor<128x128xf16, #shared>> -> tensor<128x128xf16, #blocked1>
        ttg.local_store %vl, %v {async_task_id = array<i32: 1>, loop.cluster = 2 : i32, loop.stage = 1 : i32} : tensor<128x128xf16, #blocked1> -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
        // QK MMA: writes the reuse owner (operand D, useC=false => full overwrite), stage 0.
        %qkmma = ttng.tc_gen5_mma %q, %kt, %qk[%qkt], %false, %true {async_task_id = array<i32: 2>, loop.cluster = 1 : i32, loop.stage = 0 : i32, tt.autows = "{\22stage\22: \220\22, \22order\22: \220\22, \22channels\22: [\22opndB,smem,2,0\22]}", tt.self_latency = 0 : i32} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<128x128xf16, #shared1, #smem, mutable>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
        // computation reads the QK result (channel A consumer) and writes P into the
        // reuse sibling (channel B producer), stage 1.
        %qkres, %qkt2 = ttng.tmem_load %qk[%qkmma] {async_task_id = array<i32: 0>, loop.cluster = 2 : i32, loop.stage = 1 : i32} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<128x128xf32, #linear>
        %pf16 = arith.truncf %qkres {async_task_id = array<i32: 0>, loop.cluster = 2 : i32, loop.stage = 1 : i32} : tensor<128x128xf32, #linear> to tensor<128x128xf16, #linear>
        ttng.tmem_store %pf16, %p, %true {async_task_id = array<i32: 0>, loop.cluster = 2 : i32, loop.stage = 1 : i32} : tensor<128x128xf16, #linear> -> !ttg.memdesc<128x128xf16, #tmem, #ttng.tensor_memory, mutable>
        // PV MMA: reads P (operand A) async, same (gemm) partition as the QK MMA, stage 1.
        %pvmma = ttng.tc_gen5_mma %p, %v, %acc[%at], %uc, %true {async_task_id = array<i32: 2>, loop.cluster = 2 : i32, loop.stage = 1 : i32, tt.autows = "{\22stage\22: \221\22, \22order\22: \221\22, \22channels\22: [\22opndB,smem,2,1\22]}", tt.self_latency = 1 : i32} : !ttg.memdesc<128x128xf16, #tmem, #ttng.tensor_memory, mutable>, !ttg.memdesc<128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
        %off2 = arith.addi %off, %c128_i32 {async_task_id = array<i32: 1>, loop.cluster = 0 : i32, loop.stage = 1 : i32} : i32
        scf.yield {async_task_id = array<i32: 0>} %off2, %true, %qkt2, %pvmma : i32, i1, !ttg.async.token, !ttg.async.token
      } {async_task_id = array<i32: 0, 1, 2>, tt.data_partition_factor = 2 : i32, tt.merge_correction = true, tt.merge_epilogue = true, tt.scheduled_max_stage = 1 : i32}
      scf.yield {async_task_id = array<i32: 0, 1, 2>}
    } {async_task_id = array<i32: 0, 1, 2>, tt.data_partition_factor = 2 : i32, tt.merge_correction = true, tt.merge_epilogue = true, tt.warp_specialize, ttg.partition.stages = [0 : i32, 0 : i32, 1 : i32], ttg.partition.types = ["computation", "load", "gemm"], ttg.warp_specialize.tag = 0 : i32}
    tt.return
  }
}
