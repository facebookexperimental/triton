...
// CHECK: %[[LOCAL0:.*]] = ttg.local_load {{.*}}ttg.partition = array<i32: 1>
// CHECK: arith.addf %[[LOCAL0]], %[[LOCAL0]] {{.*}}ttg.partition = array<i32: 1>
// CHECK: ttng.wait_barrier {{.*}}ttg.partition = array<i32: 2>
// CHECK: %[[LOCAL1:.*]] = ttg.local_load {{.*}}ttg.partition = array<i32: 2>
// CHECK: arith.subf %[[LOCAL1]], %[[LOCAL1]] {{.*}}ttg.partition = array<i32: 2>
...
module attributes {"ttg.cluster-dim-x" = 1 : i32, "ttg.cluster-dim-y" = 1 : i32, "ttg.cluster-dim-z" = 1 : i32, "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @tma_multi_consumer_task_ids(%desc: !tt.tensordesc<128x64xf16, #shared>, %out0: !tt.ptr<f16>, %out1: !tt.ptr<f16>) attributes {noinline = false} {
    ...
    %ptrs0 = tt.splat %out0 {ttg.partition = array<i32: 1>} : !tt.ptr<f16> -> tensor<128x64x!tt.ptr<f16>, #blocked>
    %ptrs1 = tt.splat %out1 {ttg.partition = array<i32: 2>} : !tt.ptr<f16> -> tensor<128x64x!tt.ptr<f16>, #blocked>
