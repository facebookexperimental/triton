...
// RUN: triton-opt %s --nvgpu-test-ws-memory-planner="num-buffers=2 smem-budget=200000" --mlir-print-debuginfo --mlir-use-nameloc-as-prefix 2>&1 | FileCheck %s --check-prefix=PLANNER
// RUN: triton-opt %s --nvgpu-test-ws-memory-planner="num-buffers=2 smem-budget=200000" --nvgpu-test-annotate-tma-store-waits --mlir-print-debuginfo --mlir-use-nameloc-as-prefix 2>&1 | FileCheck %s --check-prefix=ANNOTATE
// RUN: triton-opt %s --nvgpu-test-ws-memory-planner="num-buffers=2 smem-budget=200000" --nvgpu-test-annotate-tma-store-waits --nvgpu-test-tma-store-token-wait-reorder --mlir-print-debuginfo --mlir-use-nameloc-as-prefix 2>&1 | FileCheck %s --check-prefix=REORDER
// RUN: triton-opt %s --nvgpu-test-ws-memory-planner="num-buffers=2 smem-budget=200000" --nvgpu-test-annotate-tma-store-waits --nvgpu-test-tma-store-token-wait-reorder --nvgpu-tma-store-token-wait-lowering --mlir-print-debuginfo --mlir-use-nameloc-as-prefix 2>&1 | FileCheck %s --check-prefix=LOWER
...
// ANNOTATE-COUNT-4: ttng.async_tma_store_token_wait {{.*}}can_rotate_by_buffer_count = 2 : i32{{.*}}planned_pending_count = 1 : i32
...
// Input descriptor loads must still feed their shared-memory stores before the
// corresponding consumers run.
// LOWER: %[[Q:.*]] = tt.descriptor_load %desc_q
// LOWER-NEXT: ttg.local_store %[[Q]], %q
// LOWER: %[[DO:.*]] = tt.descriptor_load %desc_do
// LOWER-NEXT: ttg.local_store %[[DO]], %do
