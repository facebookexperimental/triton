...
// RUN: triton-opt %S/reuse_group_2buffer.mlir --nvgpu-test-ws-code-partition="num-buffers=1 post-channel-creation=1" --debug-only=nvgpu-ws-utility 2>&1 | FileCheck %s --check-prefix=ACCEPT2
...
// RUN: triton-opt %S/reuse_group_2buffer_multibuf.mlir --nvgpu-test-ws-code-partition="num-buffers=1 post-channel-creation=1" --debug-only=nvgpu-ws-utility 2>&1 | FileCheck %s --check-prefix=MULTIBUF
...
// RUN: triton-opt %S/ws_code_partition_tmem_3group_chain.mlir --nvgpu-test-ws-code-partition="num-buffers=1 post-channel-creation=1" --debug-only=nvgpu-ws-utility 2>&1 | FileCheck %s --check-prefix=ACCEPTN
...
// RUN: not --crash triton-opt %S/ws_code_partition_tmem_3group_no_chain.mlir --nvgpu-test-ws-code-partition="num-buffers=1 post-channel-creation=1" 2>&1 | FileCheck %s --check-prefix=REJECTN
