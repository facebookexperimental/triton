// RUN: TRITON_USE_META_WS=1 triton-opt %s -split-input-file --nvgpu-test-taskid-propagate=num-warp-groups=2 | FileCheck %s

// Regression tests for B-20-F1 / T273501459.
//
// Existing Hopper-style `ttg.partition` anchors must contribute to the global
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:90", "ttg.threads-per-warp" = 32 : i32} {
  ...
  // CHECK-NEXT:  %[[C16:.*]] = arith.constant {ttg.partition = array<i32: 0, 1, 2>} 16 : i32
  // CHECK:       llvm.intr.assume {{.*}} : i1 {ttg.partition = array<i32: 0, 1, 2>}
  ...
  tt.func public @async_anchors_mark_assume_loop_and_bounds() {
    ...
    %pid = tt.get_program_id x {ttg.partition = array<i32: 0>} : i32
    %cmp = arith.cmpi slt, %pid, %c16 : i32
    llvm.intr.assume %cmp : i1
    scf.for %iv = %c0 to %c16 step %c1 : i32 {
      %producer = tt.get_program_id y {ttg.partition = array<i32: 1>} : i32
      %consumer = tt.get_program_id z {ttg.partition = array<i32: 2>} : i32
