...
module attributes {"ttg.cluster-dim-x" = 1 : i32, "ttg.cluster-dim-y" = 1 : i32, "ttg.cluster-dim-z" = 1 : i32, ttg.max_reg_auto_ws = 152 : i32, ttg.min_reg_auto_ws = 24 : i32, "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @_attn_bwd_persist(%desc_q: !tt.tensordesc<tensor<128x128xf16, #shared>>, %desc_q_0: i32, %desc_q_1: i32, %desc_q_2: i64, %desc_q_3: i64, %desc_k: !tt.tensordesc<tensor<128x128xf16, #shared>>, %desc_k_4: i32, %desc_k_5: i32, %desc_k_6: i64, %desc_k_7: i64, %desc_v: !tt.tensordesc<tensor<128x128xf16, #shared>>, %desc_v_8: i32, %desc_v_9: i32, %desc_v_10: i64, %desc_v_11: i64, %sm_scale: f32, %desc_do: !tt.tensordesc<tensor<128x128xf16, #shared>>, %desc_do_12: i32, %desc_do_13: i32, %desc_do_14: i64, %desc_do_15: i64, %desc_dq: !tt.tensordesc<tensor<128x32xf32, #shared1>>, %desc_dq_16: i32, %desc_dq_17: i32, %desc_dq_18: i64, %desc_dq_19: i64, %desc_dk: !tt.tensordesc<tensor<128x32xf16, #shared2>>, %desc_dk_20: i32, %desc_dk_21: i32, %desc_dk_22: i64, %desc_dk_23: i64, %desc_dv: !tt.tensordesc<tensor<128x32xf16, #shared2>>, %desc_dv_24: i32, %desc_dv_25: i32, %desc_dv_26: i64, %desc_dv_27: i64, %M: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %D: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %stride_z: i32 {tt.divisibility = 16 : i32}, %stride_h: i32 {tt.divisibility = 16 : i32}, %stride_tok: i32 {tt.divisibility = 16 : i32}, %BATCH: i32, %H: i32 {tt.divisibility = 16 : i32}, %N_CTX: i32 {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    %dq, %dq_28 = ttng.tmem_alloc {buffer.copy = 1 : i32, buffer.id = 8 : i32, buffer.offset = 0 : i32} : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
    %dsT = ttg.local_alloc {buffer.copy = 1 : i32, buffer.id = 0 : i32} : () -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
    %dpT, %dpT_29 = ttng.tmem_alloc {buffer.copy = 1 : i32, buffer.id = 8 : i32} : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
    %ppT = ttng.tmem_alloc {buffer.copy = 1 : i32, buffer.id = 7 : i32, buffer.offset = 0 : i32} : () -> !ttg.memdesc<128x128xf16, #tmem, #ttng.tensor_memory, mutable>
    %do = ttg.local_alloc {buffer.copy = 1 : i32, buffer.id = 1 : i32} : () -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
    %qkT, %qkT_30 = ttng.tmem_alloc {buffer.copy = 1 : i32, buffer.id = 7 : i32} : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
    %q = ttg.local_alloc {buffer.copy = 2 : i32, buffer.id = 2 : i32} : () -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
    %dv, %dv_31 = ttng.tmem_alloc {buffer.copy = 1 : i32, buffer.id = 6 : i32} : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
    %dk, %dk_32 = ttng.tmem_alloc {buffer.copy = 1 : i32, buffer.id = 5 : i32} : () -> (!ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>, !ttg.async.token)
    ...
    %tile_idx = scf.for %_ = %c0_i32 to %2 step %c1_i32 iter_args(%tile_idx_37 = %prog_id) -> (i32)  : i32 {
      ...
      %dk_53 = ttng.tmem_store %cst, %dk[%dk_32], %true {async_task_id = array<i32: 0>, tmem.start = array<i32: 10, 12>} : tensor<128x128xf32, #blocked> -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
      %dv_54 = ttng.tmem_store %cst, %dv[%dv_31], %true {async_task_id = array<i32: 0>, tmem.start = array<i32: 7, 9>} : tensor<128x128xf32, #blocked> -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
      %curr_m:7 = scf.for %curr_m_86 = %c0_i32 to %num_steps step %c1_i32 iter_args(%arg47 = %c0_i32, %arg48 = %false, %qkT_87 = %qkT_30, %dpT_88 = %dpT_29, %dv_89 = %dv_54, %dq_90 = %dq_28, %dk_91 = %dk_53) -> (i32, i1, !ttg.async.token, !ttg.async.token, !ttg.async.token, !ttg.async.token, !ttg.async.token)  : i32 {
        %q_92 = arith.extsi %arg47 {async_task_id = array<i32: 0, 2>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : i32 to i64
        %q_93 = arith.addi %off_bh_45, %q_92 {async_task_id = array<i32: 0, 2>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : i64
        %q_94 = arith.trunci %q_93 {async_task_id = array<i32: 0, 2>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : i64 to i32
        %q_95 = tt.descriptor_load %desc_q[%q_94, %c0_i32] {async_task_id = array<i32: 2>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : !tt.tensordesc<tensor<128x128xf16, #shared>> -> tensor<128x128xf16, #blocked3>
        ttg.local_store %q_95, %q {async_task_id = array<i32: 2>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : tensor<128x128xf16, #blocked3> -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
        %qT = ttg.memdesc_trans %q {async_task_id = array<i32: 1>, loop.cluster = 0 : i32, loop.stage = 0 : i32, order = array<i32: 1, 0>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> !ttg.memdesc<128x128xf16, #shared3, #smem, mutable>
        %offs_m_96 = tt.splat %arg47 {async_task_id = array<i32: 3>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : i32 -> tensor<128xi32, #blocked2>
        %offs_m_97 = arith.addi %offs_m_96, %offs_m {async_task_id = array<i32: 3>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : tensor<128xi32, #blocked2>
        %m_98 = tt.addptr %m, %offs_m_97 {async_task_id = array<i32: 3>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : tensor<128x!tt.ptr<f32>, #blocked2>, tensor<128xi32, #blocked2>
        %m_99 = tt.load %m_98 {async_task_id = array<i32: 3>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : tensor<128x!tt.ptr<f32>, #blocked2>
        %qkT_100 = ttng.tc_gen5_mma %k, %qT, %qkT[%qkT_87], %false, %true {async_task_id = array<i32: 1>, loop.cluster = 0 : i32, loop.stage = 0 : i32, tt.self_latency = 1 : i32} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<128x128xf16, #shared3, #smem, mutable>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
        %pT = ttg.convert_layout %m_99 {async_task_id = array<i32: 3>, loop.cluster = 2 : i32, loop.stage = 0 : i32} : tensor<128xf32, #blocked2> -> tensor<128xf32, #ttg.slice<{dim = 0, parent = #blocked}>>
        %pT_101 = tt.expand_dims %pT {async_task_id = array<i32: 3>, axis = 0 : i32, loop.cluster = 2 : i32, loop.stage = 0 : i32} : tensor<128xf32, #ttg.slice<{dim = 0, parent = #blocked}>> -> tensor<1x128xf32, #blocked>
        %pT_102 = tt.broadcast %pT_101 {async_task_id = array<i32: 3>, loop.cluster = 2 : i32, loop.stage = 0 : i32} : tensor<1x128xf32, #blocked> -> tensor<128x128xf32, #blocked>
        %qkT_103, %qkT_104 = ttng.tmem_load %qkT[%qkT_100] {async_task_id = array<i32: 3>, loop.cluster = 2 : i32, loop.stage = 0 : i32} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<128x128xf32, #blocked>
        %pT_105 = arith.subf %qkT_103, %pT_102 {async_task_id = array<i32: 3>, loop.cluster = 2 : i32, loop.stage = 0 : i32} : tensor<128x128xf32, #blocked>
        %pT_106 = math.exp2 %pT_105 {async_task_id = array<i32: 3>, loop.cluster = 2 : i32, loop.stage = 0 : i32} : tensor<128x128xf32, #blocked>
        %do_107 = tt.descriptor_load %desc_do[%q_94, %c0_i32] {async_task_id = array<i32: 2>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : !tt.tensordesc<tensor<128x128xf16, #shared>> -> tensor<128x128xf16, #blocked3>
        ttg.local_store %do_107, %do {async_task_id = array<i32: 2>, loop.cluster = 2 : i32, loop.stage = 0 : i32} : tensor<128x128xf16, #blocked3> -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
        %ppT_108 = arith.truncf %pT_106 {async_task_id = array<i32: 3>, loop.cluster = 2 : i32, loop.stage = 0 : i32} : tensor<128x128xf32, #blocked> to tensor<128x128xf16, #blocked>
        %dv_109 = arith.constant {async_task_id = array<i32: 3>, loop.cluster = 2 : i32, loop.stage = 0 : i32} true
        ttng.tmem_store %ppT_108, %ppT, %dv_109 {async_task_id = array<i32: 3>, loop.cluster = 2 : i32, loop.stage = 0 : i32} : tensor<128x128xf16, #blocked> -> !ttg.memdesc<128x128xf16, #tmem, #ttng.tensor_memory, mutable>
        %dpT_110 = ttg.memdesc_trans %do {async_task_id = array<i32: 1>, loop.cluster = 2 : i32, loop.stage = 0 : i32, order = array<i32: 1, 0>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> !ttg.memdesc<128x128xf16, #shared3, #smem, mutable>
        %dpT_111 = ttng.tc_gen5_mma %v, %dpT_110, %dpT[%dpT_88], %false, %true {async_task_id = array<i32: 1>, loop.cluster = 2 : i32, loop.stage = 0 : i32, tt.self_latency = 1 : i32} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<128x128xf16, #shared3, #smem, mutable>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
        %Di_112 = tt.addptr %Di, %offs_m_97 {async_task_id = array<i32: 3>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : tensor<128x!tt.ptr<f32>, #blocked2>, tensor<128xi32, #blocked2>
        %Di_113 = tt.load %Di_112 {async_task_id = array<i32: 3>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : tensor<128x!tt.ptr<f32>, #blocked2>
        ...
        %dsT_115 = ttg.convert_layout %Di_113 {async_task_id = array<i32: 3>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128xf32, #blocked2> -> tensor<128xf32, #ttg.slice<{dim = 0, parent = #blocked}>>
        %dsT_116 = tt.expand_dims %dsT_115 {async_task_id = array<i32: 3>, axis = 0 : i32, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128xf32, #ttg.slice<{dim = 0, parent = #blocked}>> -> tensor<1x128xf32, #blocked>
        %dsT_117 = tt.broadcast %dsT_116 {async_task_id = array<i32: 3>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<1x128xf32, #blocked> -> tensor<128x128xf32, #blocked>
        %dpT_118, %dpT_119 = ttng.tmem_load %dpT[%dpT_111] {async_task_id = array<i32: 3>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<128x128xf32, #blocked>
        %dsT_120 = arith.subf %dpT_118, %dsT_117 {async_task_id = array<i32: 3>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x128xf32, #blocked>
        %dsT_121 = arith.mulf %pT_106, %dsT_120 {async_task_id = array<i32: 3>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x128xf32, #blocked>
        %dsT_122 = arith.truncf %dsT_121 {async_task_id = array<i32: 3>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x128xf32, #blocked> to tensor<128x128xf16, #blocked>
        ttg.local_store %dsT_122, %dsT {async_task_id = array<i32: 3>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x128xf16, #blocked> -> !ttg.memdesc<128x128xf16, #shared, #smem, mutable>
        %dq_123 = ttg.memdesc_trans %dsT {async_task_id = array<i32: 1>, loop.cluster = 1 : i32, loop.stage = 0 : i32, order = array<i32: 1, 0>} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable> -> !ttg.memdesc<128x128xf16, #shared3, #smem, mutable>
        %dq_124 = ttng.tc_gen5_mma %dq_123, %k, %dq[%dq_90], %false, %true {async_task_id = array<i32: 1>, loop.cluster = 1 : i32, loop.stage = 1 : i32} : !ttg.memdesc<128x128xf16, #shared3, #smem, mutable>, !ttg.memdesc<128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
        %dk_125 = ttng.tc_gen5_mma %dsT, %q, %dk[%dk_91], %arg48, %true {async_task_id = array<i32: 1>, loop.cluster = 1 : i32, loop.stage = 1 : i32, tmem.end = array<i32: 10>, tmem.start = array<i32: 11>, tt.self_latency = 1 : i32} : !ttg.memdesc<128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<128x128xf16, #shared, #smem, mutable>, !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
        %dq_126, %dq_127 = ttng.tmem_load %dq[%dq_124] {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<128x128xf32, #blocked>
        %dqs = tt.reshape %dq_126 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x128xf32, #blocked> -> tensor<128x2x64xf32, #blocked4>
        %dqs_128 = tt.trans %dqs {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32, order = array<i32: 0, 2, 1>} : tensor<128x2x64xf32, #blocked4> -> tensor<128x64x2xf32, #blocked5>
        %dqs_129, %dqs_130 = tt.split %dqs_128 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x64x2xf32, #blocked5> -> tensor<128x64xf32, #blocked6>
        %dqs_131 = tt.reshape %dqs_129 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x64xf32, #blocked6> -> tensor<128x2x32xf32, #blocked7>
        %dqs_132 = tt.trans %dqs_131 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32, order = array<i32: 0, 2, 1>} : tensor<128x2x32xf32, #blocked7> -> tensor<128x32x2xf32, #blocked8>
        %dqs_133, %dqs_134 = tt.split %dqs_132 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32x2xf32, #blocked8> -> tensor<128x32xf32, #blocked1>
        %dqs_135 = tt.reshape %dqs_130 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x64xf32, #blocked6> -> tensor<128x2x32xf32, #blocked7>
        %dqs_136 = tt.trans %dqs_135 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32, order = array<i32: 0, 2, 1>} : tensor<128x2x32xf32, #blocked7> -> tensor<128x32x2xf32, #blocked8>
        %dqs_137, %dqs_138 = tt.split %dqs_136 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32x2xf32, #blocked8> -> tensor<128x32xf32, #blocked1>
        %dqN = arith.mulf %dqs_133, %cst_33 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32xf32, #blocked1>
        %dqN_139 = ttg.convert_layout %dqN {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32xf32, #blocked1> -> tensor<128x32xf32, #blocked9>
        tt.descriptor_reduce add, %desc_dq[%q_94, %c0_i32], %dqN_139 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : !tt.tensordesc<tensor<128x32xf32, #shared1>>, tensor<128x32xf32, #blocked9>
        %dqN_140 = arith.mulf %dqs_134, %cst_33 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32xf32, #blocked1>
        %dqN_141 = ttg.convert_layout %dqN_140 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32xf32, #blocked1> -> tensor<128x32xf32, #blocked9>
        tt.descriptor_reduce add, %desc_dq[%q_94, %c32_i32], %dqN_141 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : !tt.tensordesc<tensor<128x32xf32, #shared1>>, tensor<128x32xf32, #blocked9>
        %dqN_142 = arith.mulf %dqs_137, %cst_33 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32xf32, #blocked1>
        %dqN_143 = ttg.convert_layout %dqN_142 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32xf32, #blocked1> -> tensor<128x32xf32, #blocked9>
        tt.descriptor_reduce add, %desc_dq[%q_94, %c64_i32], %dqN_143 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : !tt.tensordesc<tensor<128x32xf32, #shared1>>, tensor<128x32xf32, #blocked9>
        %dqN_144 = arith.mulf %dqs_138, %cst_33 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32xf32, #blocked1>
        %dqN_145 = ttg.convert_layout %dqN_144 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : tensor<128x32xf32, #blocked1> -> tensor<128x32xf32, #blocked9>
        tt.descriptor_reduce add, %desc_dq[%q_94, %c96_i32], %dqN_145 {async_task_id = array<i32: 0>, loop.cluster = 1 : i32, loop.stage = 0 : i32} : !tt.tensordesc<tensor<128x32xf32, #shared1>>, tensor<128x32xf32, #blocked9>
        %curr_m_146 = arith.addi %arg47, %c128_i32 {async_task_id = array<i32: 0, 2, 3>, loop.cluster = 0 : i32, loop.stage = 0 : i32} : i32
      ...
      %dv_55, %dv_56 = ttng.tmem_load %dv[%curr_m#4] {async_task_id = array<i32: 3>, tmem.end = array<i32: 8, 9>} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<128x128xf32, #blocked>
      ...
      %dk_68, %dk_69 = ttng.tmem_load %dk[%curr_m#6] {async_task_id = array<i32: 3>, tmem.end = array<i32: 11, 12>} : !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable> -> tensor<128x128xf32, #blocked>
