...
// but is a different attribute. Warp specialization fuses the scale load into
// that buffer so TMA writes it directly, and TMA can only write nvmma_shared.
// So the fused buffer must be re-spelled with the descriptor's layout; keeping
// shared_linear emits an illegal TMA copy and verification fails.
// (Upstream applies the same equivalence rule on its own WS path.)
