...
// RUN: TRITON_USE_META_WS=1 triton-opt %s --nvgpu-warp-specialization="capability=100 num-stages=3 smem-budget=232448" --triton-nvidia-gpu-clc-materialize | FileCheck %s --check-prefix=MATERIALIZED --implicit-check-not=perThread
...
// One owner-partition leader arrival from each of four CTAs.
// MATERIALIZED: ttng.init_barrier {{.*}}, 4
...
// MATERIALIZED-NEXT: ttng.arrive_barrier
