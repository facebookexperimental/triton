// RUN: triton-opt %s -split-input-file --nvgpu-2cta-insert-sync | FileCheck %s

// Test case: Insert cross-CTA synchronization for 2-CTA mode.
// When TCGen5MMAOp has two_ctas=true, this pass should insert:
// 1. ClusterCTAIdOp to get CTA rank
// 2. Compute leader predicate (cta_rank % 2 == 0)
// 3. Compute leader rank (cta_rank & ~1)
// Note: Actual barrier operations require a pre-allocated 2-CTA sync barrier

// CHECK-LABEL: @matmul_2cta_insert_sync
// CHECK: %[[CTA_ID:.*]] = nvgpu.cluster_id
// CHECK: arith.remsi %[[CTA_ID]]
// CHECK: arith.cmpi eq
// CHECK: arith.constant -2
// CHECK: arith.andi %[[CTA_ID]]
...
module attributes {"ttg.cluster-dim-x" = 2 : i32, "ttg.cluster-dim-y" = 1 : i32, "ttg.cluster-dim-z" = 1 : i32, "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @matmul_2cta_insert_sync(
    ...
    %accumulator = scf.for %k = %c0_i32 to %c1_i32 step %c1_i32 iter_args(%acc = %cst) -> (tensor<128x128xf32, #blocked>) : i32 {
      ...
      // TCGen5MMA with two_ctas=true - this triggers 2-CTA sync insertion
      // The pass should insert cross-CTA synchronization pattern before this op
...
// CHECK-LABEL: @matmul_no_2cta_sync
// CHECK: ttng.tc_gen5_mma
// CHECK-NOT: two_ctas
// CHECK-NOT: nvgpu.cluster_id
// CHECK-NOT: ttng.map_to_remote_buffer
...
module attributes {"ttg.cluster-dim-x" = 1 : i32, "ttg.cluster-dim-y" = 1 : i32, "ttg.cluster-dim-z" = 1 : i32, "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @matmul_no_2cta_sync(
      %x_desc: !tt.tensordesc<tensor<128x64xf16>>,
      %w_desc: !tt.tensordesc<tensor<64x128xf16>>,
