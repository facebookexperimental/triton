...
`barrier_expect` and fences its shared state. The owner partition then
synchronizes locally and its leader performs one arrival on rank zero's ready
mbarrier. Rank zero waits with the current carried phase before issuing the
request. This rendezvous also proves every CTA consumed the previous response
before its completion barrier is reused. The ready-barrier arrival count is the
physical cluster size and is independent of the owner partition's warp count.
