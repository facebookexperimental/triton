...
def _validate_provenance(payload: dict[str, Any], source_sha256: str) -> None:
    ...
    normalization_u = _require_int(
        provenance.get("normalization_u"),
        "provenance.normalization_u",
        minimum=1,
    )
    normalization_u_effective = _require_int(
        provenance.get("normalization_u_effective"),
        "provenance.normalization_u_effective",
        minimum=1,
    )
    if normalization_u_effective < normalization_u:
        raise ArtifactError(
            "provenance.normalization_u_effective is smaller than "
            "provenance.normalization_u"
        )
    retry_multiple, remainder = divmod(
        normalization_u_effective, normalization_u
    )
    if (
        remainder != 0
        or retry_multiple.bit_count() != 1
        or retry_multiple > MAX_U_MULTIPLIER
    ):
        raise ArtifactError(
            "provenance.normalization_u_effective must be a power-of-two "
            f"retry multiple no greater than {MAX_U_MULTIPLIER}x"
        )
    model_options = _require_object(
        provenance.get("model_options"), "provenance.model_options"
    )
    if not strict_equal(model_options, DEFAULT_MODEL_OPTIONS):
        raise ArtifactError(
            "provenance.model_options is not the production solver model"
        )
