...
def _physical_warps(ir, plan: dict[str, Any], source: str) -> set[int]:
    """The author declares one realization per physical warp of the IR.

    Warp identity comes from the solution itself, so unlike the emitter's
    group encoding there is no logical-to-physical binding to check -- only
    that every warp is realized exactly once.
    """
    expected = {warp["id"] for warp in ir.payload["warps"]}
    result: set[int] = set()
    for position, raw in enumerate(_required_list(plan, "physical_warps")):
        entry = _required_object(raw, f"physical_warps[{position}]")
        warp_id = _required_non_negative_int(
            entry.get("warp"), f"physical_warps[{position}].warp"
        )
        if warp_id in result or warp_id not in expected:
            raise AuditError(f"invalid or duplicate physical warp {warp_id}")
        if warp_id >= B200_CAPACITIES["physical_warps"]:
            raise AuditError(f"physical warp {warp_id} exceeds the B200 warp count")
        result.add(warp_id)
        for field in (
            "thread_mapping",
            "entry_predicate",
            ...
            _required_string(entry.get(field), f"physical warp {warp_id}.{field}")
        _source_anchor(
            source,
            entry.get("dispatch_anchor"),
            f"physical warp {warp_id}.dispatch_anchor",
        )
    if result != expected:
        raise AuditError("physical_warps must cover every scheduled warp")
    return result
...
def _mapping_nodes(
    ...
    for position, raw in enumerate(_required_list(plan, "nodes")):
        ...
        if not strict_equal(manual.get("physical_warps"), instruction["warps"]):
...
def audit_mapping(
    ...
    _physical_warps(ir, plan, source)
    _mapping_regions(ir, plan, source)
    _mapping_nodes(ir, plan, source, disassembly)
...
def _memory_budget(plan: dict[str, Any], allocations: list[tuple], ir) -> None:
    ...
    expected_warps = len(ir.payload["warps"])
    fields = {
    ...
    if planned["physical_warps"] != expected_warps:
        raise AuditError("planned physical_warps does not match the IR warps")
...
def _channel_map(ir, plan: dict[str, Any]) -> dict[str, dict[str, Any]]:
    valid_warps = {warp["id"] for warp in ir.payload["warps"]}
    ...
    for position, raw in enumerate(_required_list(plan, "channels")):
        ...
        for field in ("producer_warps", "consumer_warps"):
            ...
            if not set(values).issubset(valid_warps):
                raise AuditError(
                    f"channel {channel_id}.{field} references an unscheduled warp"
                )
...
def _sync_dependencies(ir, plan: dict[str, Any], channels: dict[str, dict[str, Any]]):
    ...
    for position, raw in enumerate(_required_list(plan, "dependencies")):
        ...
        if mechanism == "program_order":
            ...
            if not set(expected[edge_index]["producer_warps"]).issubset(
                channel_spec["producer_warps"]
            ):
                raise AuditError(
                    f"sync dependency {edge_index} channel lacks producer warps"
                )
            if not set(expected[edge_index]["consumer_warps"]).issubset(
                channel_spec["consumer_warps"]
            ):
                raise AuditError(
                    f"sync dependency {edge_index} channel lacks consumer warps"
                )
...
def _expanded_edges(ir, wait_edges, iterations):
    ...
    for warp in ir.payload["warps"]:
        trace = [entry["node"] for entry in warp["issue_trace"]]
        for phase in range(iterations):
            edges.extend(
...
def audit_bundle(
    ...
    mapped_warps = {entry["warp"] for entry in mapping["physical_warps"]}
    ...
    for binding in synchronization["dependencies"]:
        ...
        if binding["mechanism"] == "channel":
            ...
            if not set(binding["producer_warps"]).issubset(
                channel["producer_warps"]
            ):
                raise AuditError("channel producer warps disagree with dependency")
            if not set(binding["consumer_warps"]).issubset(
                channel["consumer_warps"]
            ):
                raise AuditError("channel consumer warps disagree with dependency")
    ...
    for obligation in synchronization["additional_obligations"]:
        ...
        producer_warps = instructions[obligation["instruction_id"]]["warps"]
        if not set(producer_warps).issubset(channel["producer_warps"]):
        ...
        if obligation["wait_kind"] == "instruction":
            consumer_warps = instructions[obligation["wait_node"]]["warps"]
            if not set(consumer_warps).issubset(channel["consumer_warps"]):
