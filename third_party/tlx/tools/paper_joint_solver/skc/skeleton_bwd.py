"""Parameterized Blackwell FMHA-backward skeleton (SKC Phase A, M4).

Faithful extraction of sched2tlx/examples/case4_FA_bwd/handwritten.py
(fa_bwd_dkdv_ws): same four-role structure and barrier protocol —
  load       1 warp,  REGS_LOAD regs — TMA K/V once, Q ring + dO per blk
  mma        1 warp,  REGS_MMA  regs — skewed prolog/main/epilog 5-dot pipeline
  compute    default task            — softmax pT + dS, dV/dK epilogue stores
  reduction  4 warps, REGS_REDUCTION — dQ atomic-add
TMEM is single-buffered with storage aliasing (qk/p share; dP/dS/dQ share) so
HEAD_DIM=128 fits in 512 columns.  The 1-stage dK/dQ skew is part of the
protocol (constructed, not a transform).  Bindable: Q ring depth, register
quotas, tile geometry.
"""
...
def check_register_budget_bwd(p):
    mma = 1 * _WARP * p["REGS_MMA"]
    red = 4 * _WARP * p["REGS_REDUCTION"]
    load = 1 * _WARP * p["REGS_LOAD"]
    compute_floor = 4 * _WARP * 24
    total = mma + red + load + compute_floor
    if total > _REG_FILE:
        raise ValueError(f"bwd register quota table exceeds reg file: {total}")
    return total
...
def fa_bwd_dkdv_skc(
    ...
    # reduction task's dq read is what frees it.
    ...
    with tlx.async_tasks():
        ...
        with tlx.async_task(num_warps=1, num_regs=REGS_MMA):
        ...
        with tlx.async_task(num_warps=4, num_regs=REGS_REDUCTION):
            offs_d = tl.arange(0, HEAD_DIM)
            for blk in range(num_steps):
        ...
        with tlx.async_task(num_warps=1, num_regs=REGS_LOAD):
...
def bwd_attention(qf, kf, vf, dof, dq, dk, dv, M, D, N_CTX, params=None):
    """Launch the bwd skeleton with an explicit parameter binding.

    Tensors follow the case4 convention: (BH, N_CTX, HEAD_DIM) contiguous,
    Q pre-scaled by 1/sqrt(HEAD_DIM), M = logsumexp*log2e, D = rowsum(dO*O),
    dq zeroed by the caller (atomic-add accumulation).
    """
    p = dict(DEFAULT_PARAMS_BWD)
    if params:
        p.update(params)
    check_register_budget_bwd(p)
    ...
    grid = (N_CTX // p["BLOCK_N"], bh)
    fa_bwd_dkdv_skc[grid](
        qf, kf, vf, dof, dq, dk, dv, M, D,
        head_dim, head_dim, N_CTX,
        BLOCK_M=p["BLOCK_M"],
        BLOCK_N=p["BLOCK_N"],
        HEAD_DIM=head_dim,
        NUM_BUFFERS_Q=p["NUM_BUFFERS_Q"],
        REGS_MMA=p["REGS_MMA"],
        REGS_REDUCTION=p["REGS_REDUCTION"],
