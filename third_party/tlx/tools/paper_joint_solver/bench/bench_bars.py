...
RETIRED from the canonical pipeline: the automatic TLX emitter arm this runner
measures is no longer part of the literal reproduction (see DEVIATIONS.md).
Its v6-era results stand as the final performance record of the guarded
regime; the literal batch produces no bench artifacts.
...
JOS_BWD_LR_KERNEL = (
    EXAMPLES
    / "case4_FA_bwd_subtiled"
    / "generated_jos_lr4096_v8.py"
)
JOS_BARS = {"tlx_jos_fwd", "tlx_jos_bwd", "tlx_jos_bwd_lr"}
_JOS_MANIFEST_ENV = os.environ.get("JOS_EMISSION_MANIFEST", "")
JOS_EMISSION_MANIFEST = (
    Path(_JOS_MANIFEST_ENV)
    if _JOS_MANIFEST_ENV
    else PKG_DIR / "solutions" / "bench_emit_emitter_v8" / "manifest.json"
)
JOS_ARTIFACTS = {
    "tlx_jos_fwd": ("fwd_subtiled_emitter_v8", JOS_FWD_KERNEL),
    "tlx_jos_bwd": ("bwd_emitter_v8", JOS_BWD_KERNEL),
    "tlx_jos_bwd_lr": ("bwd_lr4096_emitter_v8", JOS_BWD_LR_KERNEL),
}
EMISSION_SOURCE_FILES = (
    PKG_DIR / "emit_bench_kernels.py",
    PKG_DIR / "paper_joint_solver" / "graph_writer.py",
    PKG_DIR / "paper_joint_solver" / "machine.py",
    PKG_DIR / "paper_joint_solver" / "resource_model.py",
    TOOLS_DIR / "sched2tlx" / "sched2tlx" / "__init__.py",
    TOOLS_DIR / "sched2tlx" / "sched2tlx" / "__main__.py",
    TOOLS_DIR / "sched2tlx" / "sched2tlx" / "cli.py",
    TOOLS_DIR / "sched2tlx" / "sched2tlx" / "emitter.py",
    TOOLS_DIR / "sched2tlx" / "sched2tlx" / "schedule_graph.py",
    TOOLS_DIR / "sched2tlx" / "sched2tlx" / "semaphore_ir.py",
)
...
class EmissionProvenanceError(BarUnavailable):
    pass
...
def _emission_sources_sha256():
    digest = hashlib.sha256()
    for path in EMISSION_SOURCE_FILES:
        name = str(path.relative_to(TOOLS_DIR))
...
def _verified_jos_provenance(name, manifest_path=None):
    manifest_path = (
        JOS_EMISSION_MANIFEST
    ...
    if not manifest_path.is_file():
        raise EmissionProvenanceError(
            f"missing successful JOS emission manifest: {manifest_path}"
        )
    try:
        manifest = json.loads(manifest_path.read_text())
    except (OSError, json.JSONDecodeError) as error:
        raise EmissionProvenanceError(
            f"invalid JOS emission manifest: {error}"
        ) from error
    if manifest.get("schema_version") != "jos-bench-emission-v1":
        raise EmissionProvenanceError("unexpected JOS emission manifest schema")
    if manifest.get("status") != "success":
        raise EmissionProvenanceError("JOS emission manifest is not successful")
    ...
    if constraints != {
        ...
        raise EmissionProvenanceError(
            "JOS emission manifest lacks explicit non-paper constraints"
        )
    current_sources = _emission_sources_sha256()
    if manifest.get("emission_sources_sha256") != current_sources:
        raise EmissionProvenanceError(
            "JOS emission sources changed after kernel generation"
        )

    case_name, kernel_path = JOS_ARTIFACTS[name]
    case = manifest.get("cases", {}).get(case_name)
    if not isinstance(case, dict):
        raise EmissionProvenanceError(
            f"JOS emission manifest has no case {case_name}"
        )
    if Path(case.get("kernel_output", "")).resolve() != kernel_path.resolve():
        raise EmissionProvenanceError(f"{case_name}: kernel path mismatch")
    if not kernel_path.is_file():
        raise EmissionProvenanceError(f"{case_name}: missing kernel {kernel_path}")
    kernel_sha256 = _sha256(kernel_path)
    if case.get("kernel_sha256") != kernel_sha256:
        raise EmissionProvenanceError(f"{case_name}: kernel hash mismatch")

    graph_path = Path(case.get("graph_output", ""))
    if not graph_path.is_file():
        raise EmissionProvenanceError(
            f"{case_name}: missing rewritten graph {graph_path}"
        )
    graph_sha256 = _sha256(graph_path)
    if case.get("graph_sha256") != graph_sha256:
        raise EmissionProvenanceError(f"{case_name}: graph hash mismatch")

    solution_path = Path(case.get("solution", ""))
    if not solution_path.is_file():
        raise EmissionProvenanceError(
            f"{case_name}: missing emitter solution {solution_path}"
        )
    solution_sha256 = _sha256(solution_path)
    if case.get("solution_sha256") != solution_sha256:
        raise EmissionProvenanceError(f"{case_name}: solution hash mismatch")
    return {
        "emission_manifest": str(manifest_path),
        "emission_manifest_sha256": _sha256(manifest_path),
        "emission_sources_sha256": current_sources,
        "emitter_solution": str(solution_path),
        "emitter_solution_sha256": solution_sha256,
        "schedule_graph": str(graph_path),
        "schedule_graph_sha256": graph_sha256,
...
def build_tlx_bwd(cfg, path, kernel_name="fa_bwd_dkdv_5mma"):
...
def make_registry(args):
    fwd = {
        ...
        "tlx_default": lambda cfg: build_tlx_fwd(
            ...
            "fa_fwd_kernel_nows"),
        "tlx_jos_fwd": lambda cfg: build_tlx_fwd(
            cfg, JOS_FWD_KERNEL, "fa_fwd_kernel_nows_subtiled"),
        "cudnn": build_cudnn,
        "fa4": None,
    }
    bwd = {
        ...
        "tlx_jos_bwd": lambda cfg: build_tlx_bwd(
            cfg, JOS_BWD_KERNEL, "fa_bwd_dkdv_subtiled"),
        "tlx_jos_bwd_lr": lambda cfg: build_tlx_bwd(
            cfg, JOS_BWD_LR_KERNEL, "fa_bwd_dkdv_subtiled"),
    }
...
def _set_skip(rec, *, phase, reason):
    rec.update(ok=False, status="SKIP", skip_phase=phase, reason=reason)


def _cleanup_after_run(rec):
    errors = []
    for label, action in (
    ...
    if not errors:
        return
    rec["cleanup_errors"] = errors
    if rec.get("ok"):
        _set_skip(rec, phase="cleanup", reason="; ".join(errors))


def run_one(name, builder, cfg, mode, args):
    rec = {"env_probe": {"before": _nvsmi()}}
    if name in JOS_BARS:
        rec["schedule_classification"] = "non-paper-emitter-compatibility"
        rec["selection_constraints"] = {
            "prefix_lane_masks": True,
            "full_group_lane_masks": True,
        }
    phase = "provenance" if name in JOS_BARS else "build"
    try:
        if name in JOS_BARS:
            rec.update(
                _verified_jos_provenance(
                    name, getattr(args, "jos_manifest", None)
                )
            )
            phase = "build"
        if name in EXTERNAL_BARS:
            ...
            if not rec.get("ok"):
                _set_skip(
                    rec,
                    phase="external_worker",
                    reason=rec.get("reason", "external worker failed"),
                )
        else:
            built = builder(cfg)
            fn, check = built[0], built[1]
            fwd_baseline = built[2] if len(built) > 2 else None
            phase = "compile_or_correctness"
            reason = check()
            torch.cuda.synchronize()
            if reason:
                _set_skip(
                    rec,
                    phase="correctness",
                    reason=f"correctness gate: {reason}",
                )
            else:
                phase = "benchmark"
                ...
                if fwd_baseline is not None:
                    ...
                    rec["fwd_ms"] = fwd_ms
                rec.update(ok=True, status="OK", ms=ms, lo=lo, hi=hi)
    except EmissionProvenanceError as e:
        _set_skip(rec, phase="provenance", reason=str(e))
    except BarUnavailable as e:
        _set_skip(rec, phase="availability", reason=str(e))
    except Exception as e:  # keep the sweep going
        _set_skip(rec, phase=phase, reason=f"{type(e).__name__}: {e}")
    if rec.get("ok") and rec["ms"] <= 0:
        _set_skip(
            rec,
            phase="benchmark",
            reason=f"non-positive time {rec['ms']:.4f} ms",
        )
    if rec.get("ok"):
        rec["tflops"] = total_flops(mode, cfg.seqlen) * 1e-12 / (rec["ms"] *
                                                                 1e-3)
    _cleanup_after_run(rec)
...
def main(argv=None):
    ...
    ap.add_argument(
        "--jos-manifest",
        default=str(JOS_EMISSION_MANIFEST),
        help=(
            "successful emit_bench_kernels manifest; defaults to "
            "JOS_EMISSION_MANIFEST or the standard record directory"
        ),
    )
