# Historical sched2tlx/TLX gap analysis: `generated_jos.py` vs `generated.py`

Twill §5 emits software-pipelined, warp-annotated IR for expert manual
implementation. The paper's §6.1 CUDA C++ was hand-compiled; automatic memory
allocation, layout, and synchronization lowering are out of scope. This
checkout stops at the manual-CUDA handoff IR and has no paper handwritten CUDA
source, so paper performance is unreplicated. TLX is related work only.

Both kernels analyzed here are historical sched2tlx/TLX experiments. Neither
is Twill/SKC codegen or the paper CUDA implementation. `jos` is a legacy
filename, not a provenance claim. All counters, diagnoses, and variants below
apply only to these two emitted TLX kernels.
...
matches the local benchmark sweep: **205.1 vs 666.2 TFLOPS (5.36 ms vs
...
| metric | legacy `generated_jos.py` | historical TLX baseline | ratio |
|---|---|---|---|
...
In legacy `generated_jos.py`, wg3 (`role=TC`, 4 warps) per iteration: issues both QK MMAs, **tmem_loads both
...
recovered with `killgpu.sh`). The serial fallback was load-bearing in that historical
emitter; its skewed path has a synchronization bug that blocks recovering this headroom.
...
The historical sched2tlx experiment's five-group partition is the largest local
lever: colocate the accumulator rescale with the
...
This diagnosis does not characterize Twill handoff IR or the expert-written
CUDA C++ evaluated in §6.1.
