...
of used warp ids.  Sparse or non-canonical ids therefore remain distinct;
graphs with more than eight used groups are rejected rather than silently
reusing a color.  The legend and per-node labels carry warp identity in text
...
def _warp_colors(warp: dict[int, int] | None) -> dict[int, str]:
    if warp is None:
        return {}
    used = sorted(set(warp.values()))
    if len(used) > len(PALETTE):
        raise ValueError(
            f"cannot render {len(used)} used warp groups with the "
            f"{len(PALETTE)}-color palette: {used}"
        )
    return {group: PALETTE[index] for index, group in enumerate(used)}
...
def main(argv: list[str] | None = None) -> None:
    ...
    if prob is None:
        # Graph-only and unversioned inputs are inspection paths for archived
        # schedules; they never feed a new solve.
        prob = load_problem(
            args.ddg,
