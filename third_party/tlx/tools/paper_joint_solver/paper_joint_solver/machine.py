...
The paper's model D is the first group of members: the functional-unit
capacities, the SMEM/TMEM capacities, the physical warp count, the per-warp
register budget and the spill cost.  Those are the only members the paper
solve path reads.

The second group — group widths and their quantization helpers, SM-wide
register-file accounting, barrier bytes, TMEM column bytes and the four
fixed-overhead knobs — describes what the downstream emitter needs.  The
paper entry never reads them; they exist for the emitter entry, the graph
writer and the bench kernels.
"""
...
def register_allocation_words(value_words: int, width: int) -> int:
    """Emitter-only allocation granularity: ptxas allocates whole warps."""
...
class MachineModel:
    ...
    # Fixed function-scope TMEM columns genuinely absent from the loop DDG.
    # DDG-visible MMA accumulators are memory objects and must not be
    # compensated through this knob.
    ...
    def registers_fit(
        ...
        """Emitter-only accounting: per-lane share plus the SM-wide total.

        The paper entry asserts its per-warp bound inside the solver and has
        no SM-wide bound at all."""
        total = 0
