...
_MODEL_PROFILES = {
    PAPER_MODEL_PROFILE: DEFAULT_MODEL_OPTIONS,
    EMITTER_MODEL_PROFILE: EMITTER_MODEL_OPTIONS,
}
_SOLVER_SOURCES = (
    "__main__.py",
    "ddg.py",
...
def solution_provenance(
    ...
    normalization_u: int,
    normalization_u_effective: int | None = None,
    model_options: dict | None = None,
) -> dict:
    if normalization_u_effective is None:
        normalization_u_effective = normalization_u
    ...
    return {
        ...
        "normalization_u_effective": normalization_u_effective,
...
def _figure5_liveness(problem, cycles, ii, copies, horizon):
    memory_members = {
        member
        for storage in problem.memory_objects.values()
...
def _validate_figure5_constraints(
    ...
    ws_data_edges = [
        edge
        for edge in problem.edges
        if not edge.signal_only and problem.edge_has_ws_semantics(edge)
    ]
...
def load_schedule_plan(
    ...
    *,
    ...
    if (
        ...
        or normalization_u_effective < normalization_u
    ):
        raise ScheduleArtifactError(
            "provenance.normalization_u_effective must be an integer "
            "greater than or equal to normalization_u"
        )
    model_options = provenance.get("model_options")
    if not isinstance(model_options, dict):
        raise ScheduleArtifactError("provenance.model_options must be an object")
    try:
        required_model_options = _MODEL_PROFILES[model_profile]
    except (KeyError, TypeError) as error:
        raise ScheduleArtifactError(
            f"unknown production model profile: {model_profile!r}"
        ) from error
    if not _strict_equal(model_options, required_model_options):
        if model_profile == PAPER_MODEL_PROFILE:
            raise ScheduleArtifactError(
                "diagnostic model options or emitter selection constraints "
                "cannot be used for a "
                f"paper production handoff: {model_options!r}"
            )
        raise ScheduleArtifactError(
            "solution model options do not match the requested production "
            f"mode: {model_options!r}"
        )
    ...
    if problem.normalization_u_effective != normalization_u_effective:
        raise ScheduleArtifactError(
            "normalization_u_effective mismatch: "
            f"solution={normalization_u_effective}, "
            f"recomputed={problem.normalization_u_effective}"
        )
    ...
    for group, width in group_widths.items():
        ...
        for node_id in members:
            ...
            if model_options["prefix_lane_masks"] and lanes != tuple(
                range(node_width)
            ):
                raise ScheduleArtifactError(
            ...
            if model_options["full_group_lane_masks"] and width != node_width:
                raise ScheduleArtifactError(
    ...
    expected_stats = {
        **problem.normalization_stats(),
        ...
        "lane_mask_model": (
            "emitter_full_group"
            if model_options["full_group_lane_masks"]
            else "emitter_prefix"
            if model_options["prefix_lane_masks"]
