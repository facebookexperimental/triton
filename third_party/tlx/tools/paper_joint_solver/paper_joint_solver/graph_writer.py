...
TIMING_MODEL_SCHEMA = "twill-emitter-timing-v1"
...
def normalized_timing_model(problem):
    ...
    return {
        ...
        "normalization_u_effective": problem.normalization_u_effective,
        "nodes": {
            node_id: {
...
def _apply_timing_model(schedule_loop, timing_model):
    ...
    for field in ("normalization_u", "normalization_u_effective"):
        value = timing_model.get(field)
        if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
            raise ValueError(f"normalized_timing.{field} must be positive")
    if timing_model["normalization_u_effective"] < timing_model["normalization_u"]:
        raise ValueError(
            "normalized_timing effective U must be at least the requested U"
        )
