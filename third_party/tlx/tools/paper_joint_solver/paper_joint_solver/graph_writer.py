...
RETIRED from the canonical pipeline: the automatic TLX emitter arm is no
longer part of the literal reproduction (see DEVIATIONS.md).  The paper's own
workflow hands the pipelined IR to an expert for a manual implementation, so
the literal branch ends at the paper artifacts and this file is kept for
reference only.  Nothing here is produced or validated by a lit1 run.

The paper-faithful path is ``paper_joint_solver.pipelined_ir``.  This module is
retained only for isolated TLX experiments and must not be treated as the
paper's code generation or evaluation evidence.

It consumes EMITTER-profile artifacts from the curated ddg + baseline graph
chain -- ``ddg.load_problem`` is curated-only, so the raw chain this path was
written for no longer exists; group-width snapping, channel synthesis, ring
depths and barrier-kind selection are emitter transformations recorded here,
not model decisions.  The node-coverage gate in ``_apply_timing_model`` still
expects nodes that curation deletes, which is one more reason this path is
...
schedule + warp assignment:
  ...
  * node latency, occupancy, and RRT rewritten into the same normalized time
    scale as the solution cycles and dependence delays before buffer lifetimes
    are recomputed;
...
TIMING_MODEL_SCHEMA = "emitter-timing-v1"


def normalized_timing_model(problem):
    """Serialize the normalized node timing consumed by the joint solver."""
    return {
        "schema_version": TIMING_MODEL_SCHEMA,
        "scale": "zlp-normalized",
        "normalization_u": problem.normalization_u,
        "nodes": {
            node_id: {
                "latency": problem.lat[node_id],
                "occupancy": problem.occ[node_id],
                "rrt": {
                    resource: list(row)
                    for resource, row in problem.rrt[node_id].items()
                },
            }
...
def _apply_timing_model(schedule_loop, timing_model):
    if not isinstance(timing_model, dict):
        raise ValueError("normalized_timing must be an object")
    if timing_model.get("schema_version") != TIMING_MODEL_SCHEMA:
        raise ValueError(
            f"normalized_timing must use schema {TIMING_MODEL_SCHEMA}"
        )
    if timing_model.get("scale") != "zlp-normalized":
        raise ValueError("normalized_timing scale must be zlp-normalized")
    for field in ("normalization_u",):
        value = timing_model.get(field)
        if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
            raise ValueError(f"normalized_timing.{field} must be positive")

    raw_nodes = timing_model.get("nodes")
    if not isinstance(raw_nodes, dict):
        raise ValueError("normalized_timing.nodes must be an object")
    try:
        timing_nodes = {int(node_id): value for node_id, value in raw_nodes.items()}
    except (TypeError, ValueError) as error:
        raise ValueError("normalized_timing node ids must be integers") from error
    nodes = {node["id"]: node for node in schedule_loop["graph"]["nodes"]}
    if len(timing_nodes) != len(raw_nodes) or set(timing_nodes) != set(nodes):
        raise ValueError(
            "normalized_timing must cover exactly the schedule-graph nodes"
        )

    for node_id, node in nodes.items():
        timing = timing_nodes[node_id]
        if not isinstance(timing, dict):
            raise ValueError(f"normalized timing for node {node_id} must be an object")
        latency = _nonnegative_int(
            timing.get("latency"), f"node {node_id} normalized latency"
        )
        occupancy = _nonnegative_int(
            timing.get("occupancy"), f"node {node_id} normalized occupancy"
        )
        if occupancy > latency:
            raise ValueError(
                f"node {node_id} normalized occupancy {occupancy} exceeds "
                f"latency {latency}"
            )
        raw_rrt = timing.get("rrt")
        if not isinstance(raw_rrt, dict):
            raise ValueError(f"node {node_id} normalized RRT must be an object")
        rrt = {}
        for resource, raw_row in raw_rrt.items():
            if not isinstance(resource, str) or not isinstance(raw_row, (list, tuple)):
                raise ValueError(f"node {node_id} has an invalid normalized RRT row")
            row = [
                _nonnegative_int(
                    amount, f"node {node_id} RRT[{resource},{cycle}]"
                )
                for cycle, amount in enumerate(raw_row)
            ]
            if len(row) != latency:
                raise ValueError(
                    f"node {node_id} RRT[{resource}] length {len(row)} "
                    f"does not match normalized latency {latency}"
                )
            rrt[resource] = row
        active_cycles = sum(
            any(row[cycle] for row in rrt.values()) for cycle in range(latency)
        )
        if active_cycles != occupancy:
            raise ValueError(
                f"node {node_id} normalized occupancy {occupancy} does not "
                f"match {active_cycles} active RRT cycles"
            )
        node["latency"] = latency
        # self_latency has no separate normalized solver variable. Keeping the
        # normalized active duration avoids leaking its raw-cycle value.
        node["self_latency"] = occupancy
        node["occupancy"] = occupancy
        node["rrt"] = rrt

    schedule_loop["timing_model"] = {
        key: value for key, value in timing_model.items() if key != "nodes"
    }
...
def _recompute_buffers(sl, ii, ops, normalized_time=False):
    ...
    for b in buffers:
        ...
        live_start = (
            min(starts)
            if starts
            else (0 if normalized_time else b["live_start"])
        )
...
def rewrite_schedule_graph(
    ...
    *,
    ...
    nodes = sl["graph"]["nodes"]
    by_id = {n["id"]: n for n in nodes}
    if (dependence_edges is None) != (normalized_timing is None):
        raise ValueError(
            "dependence_edges and normalized_timing must be supplied together; "
            "refusing to mix normalized and raw cycle scales"
        )
    if normalized_timing is not None:
        _apply_timing_model(sl, normalized_timing)
    ...
    _recompute_buffers(
        ...
        ii,
        data.get("ops"),
        normalized_time=normalized_timing is not None,
    )
