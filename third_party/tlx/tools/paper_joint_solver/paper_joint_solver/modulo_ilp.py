...
def solve_modulo(prob: Problem, ii: int) -> ModuloSchedule | None:
    ...
    stage = {v: m.addVar(f"stage_{v}", vtype="I", lb=0) for v in ids}
    ...
    if status != "optimal":
        raise RuntimeError(
            f"unbounded SCIP solve ended with unexpected status {status} "
            f"at II={ii}"
        )
    cycles = {}
    for v in ids:
