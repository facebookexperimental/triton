...
def test_emit_graph_rejects_non_sat_solution_without_output(tmp_path):
    ...
    solution.write_text(
        json.dumps(
            {
                "schema_version": "twill-joint-solution-v1",
                "ii": 1,
                "length": 1,
                "copies": 1,
                "horizon": 1,
...
def test_bench_loader_requires_explicit_emitter_profile(tmp_path, monkeypatch):
    ...
    def fake_load(solution_path, ddg, baseline, *, model_profile):
...
def test_emitter_solve_workflow_is_separate_and_hard_gated():
    source = RUN_EMITTER_CASES.read_text()

    assert source.index('if [[ "$MACHINE_HOSTNAME" != dgx003') < source.index(
        'mkdir -p "$OUT"'
    )
    assert '"$LSCPU_MODEL" != *8570*' in source
    assert 'taskset -c "$SOLVER_CPU"' in source
    assert "--emitter-compatible-lane-masks" in source
    assert "classification=%s" in source
    assert "non-paper-emitter-compatibility" in source
    assert "cuda_version=%s" in source
    assert "nvidia_smi=%s" in source
    assert "PYTHONPATH=%s" in source
    assert "module_{name}_file" in source
    assert "fwd_subtiled_emitter_v8" in source
    assert "bwd_lr4096_emitter_v8" in source
