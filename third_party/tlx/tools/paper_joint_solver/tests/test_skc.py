...
def _edge(index, src, dst, producer_group, consumer_group, distance=0, latency=1):
    return {
        "index": index,
        "src": src,
        "dst": dst,
...
def _complete_sync(result):
    ...
    for dependency in sync["dependencies"]:
        ...
        if dependency["producer_group"] != dependency["consumer_group"]:
...
def _complete_bundle(tmp_path):
    ir_path, handoff_path, result = _scaffold(tmp_path)
