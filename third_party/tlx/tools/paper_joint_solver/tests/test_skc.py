...
@pytest.mark.parametrize("effective_u", (19200, 76800))
def test_schema_accepts_adaptive_normalization_retry(effective_u):
    payload = _ir()
    payload["provenance"]["normalization_u_effective"] = effective_u

    validate_pipelined_ir(payload)


@pytest.mark.parametrize("effective_u", (301, 153600))
def test_schema_rejects_invalid_adaptive_normalization_retry(effective_u):
    payload = _ir()
    payload["provenance"]["normalization_u_effective"] = effective_u

    with pytest.raises(ArtifactError, match="power-of-two retry multiple"):
        validate_pipelined_ir(payload)
