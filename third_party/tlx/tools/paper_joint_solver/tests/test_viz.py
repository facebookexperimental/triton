...
def test_to_dot_reranks_sparse_group_ids_without_color_collisions(prob):
    ...
    sparse_groups = (0, 8, 19, 31, 47, 63, 80, 101)
    sparse_warp = dict(zip(visible[:8], sparse_groups))
    dot = to_dot(prob, sparse_warp)

    for node_id, group, color in zip(visible, sparse_groups, PALETTE):
    ...
    assert len(set(PALETTE)) == len(sparse_groups)


def test_to_dot_rejects_more_groups_than_the_palette(prob):
    from paper_joint_solver.viz import PALETTE
    ...
    with pytest.raises(ValueError, match=r"cannot render 9 used warp groups"):
        to_dot(prob, warp)
