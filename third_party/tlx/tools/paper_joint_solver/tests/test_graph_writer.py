...
def _timing_model(baseline, latencies=None):
    data = json.loads(baseline.read_text())
    ...
    return {
        ...
        "nodes": {
            node["id"]: {
                "latency": latencies.get(node["id"], node["latency"]),
                "occupancy": 0,
                "rrt": {},
            }
            for node in nodes
...
def test_cross_wg_barriers_are_rebuilt_for_buffer_edges(tmp_path):
    ...
    rewrite_schedule_graph(
        ...
        normalized_timing=_timing_model(baseline),
    )
...
def test_replaced_dependences_require_explicit_normalized_timing(tmp_path):
    baseline = SCHED2TLX / "examples" / "case1_simple_gemm" / "schedule_graph.json"
    data = json.loads(baseline.read_text())
    loop = data["loops"][0]["schedule_loop"]
    nodes = loop["graph"]["nodes"]
    ...
    with pytest.raises(ValueError, match="mix normalized and raw cycle scales"):
        rewrite_schedule_graph(
            baseline,
            tmp_path / "mixed-scale.json",
            ii=loop["II"],
            cycles=cycles,
            warp=warp,
            length=max(cycles.values()) + 1,
...
def test_normalized_timing_requires_replacement_dependences(tmp_path):
    baseline = SCHED2TLX / "examples" / "case1_simple_gemm" / "schedule_graph.json"
    ...
    with pytest.raises(ValueError, match="supplied together"):
        rewrite_schedule_graph(
            baseline,
            tmp_path / "timing-with-raw-edges.json",
            ii=ii,
            cycles=cycles,
            warp=warp,
            length=length,
...
def test_normalized_node_timing_drives_buffer_lifetimes(tmp_path):
    baseline = SCHED2TLX / "examples" / "case1_simple_gemm" / "schedule_graph.json"
    data = json.loads(baseline.read_text())
    loop = data["loops"][0]["schedule_loop"]
    nodes = loop["graph"]["nodes"]
    cycles = {node["id"]: node["schedule"]["cycle"] for node in nodes}
    warp = {node["id"]: node["warp_group"] for node in nodes}
    timing = _timing_model(
        baseline,
        {0: 1, 1: 0, 2: 1, 3: 0, 4: 2},
    )
    timing["nodes"][4].update(occupancy=2, rrt={"TC": [1, 1]})
    edges = [dict(edge, latency=0) for edge in loop["graph"]["edges"]]
    out_json = tmp_path / "normalized-timing.json"
    ...
    rewrite_schedule_graph(
        ...
        cycles=cycles,
        warp=warp,
        length=max(cycles.values()) + 3,
        dependence_edges=edges,
        normalized_timing=timing,
    )

    rewritten = json.loads(out_json.read_text())["loops"][0]["schedule_loop"]
    mma = next(node for node in rewritten["graph"]["nodes"] if node["id"] == 4)
    assert rewritten["timing_model"]["scale"] == "zlp-normalized"
    assert (mma["latency"], mma["self_latency"], mma["occupancy"]) == (2, 2, 2)
    assert mma["rrt"] == {"TC": [1, 1]}
    assert {
        buffer["count"]
        for buffer in rewritten["buffers"]
        if buffer["kind"] != "barrier"
    } == {1}
