# @generated by sched2tlx, then HAND-PATCHED (2026-07-06) — the diff against
# fa_subtiled_rau_generated.py is the spec of what the emitter must learn to
# produce for sub-tiled (multi-instance) graphs. Three defect classes fixed:
#
# 1. Singleton cross-region channels: the epilogue m_i/l_i channels and their
#    barrier pairs are keyed by VALUE NAME, so the second sub-tile's instances
#    were emitted with identical names (Python shadowing → one physical buffer,
#    corrupted hand-off). Fix: rename the second instances to epi_*_1_* and
#    rebind the default-task epilogue to per-sub-tile channels (acc from
#    acc_tmem_5 for sub0, acc_tmem for sub1, divide by the matching l_i).
# 2. Per-MMA hardware arrives vs per-WG dedup: a K tile consumed by BOTH QK
#    dots gets one hardware arrive per async_dot mBarrier, but the emitter
#    counted distinct consumer WGs (=1) → L0_smem_1_empty and acc_tmem_empty
#    had arrive_count=1 and trapped with "unspecified launch failure" in the
#    tcgen05 warp. Fix: arrive_count=2 on both.
# 3. Dropped epilogue ops: the M_lse stores (log2(l_i) + m_i) are absent from
...
def fa_fwd_kernel_nows_subtiled(
    ...
    epi_m_i_1_smem = tlx.local_alloc((128,), tl.float32, 1)
    ...
    epi_l_i_1_smem = tlx.local_alloc((128,), tl.float32, 1)
    ...
    # epi_m_i_0_smem: legacy carve-out (cross-loop iter_arg or intra-WG async load)
    epi_m_i_1_smem_full = tlx.alloc_barriers(num_barriers=1, arrive_count=1)
    epi_m_i_1_smem_empty = tlx.alloc_barriers(num_barriers=1, arrive_count=1)
    ...
    # epi_l_i_0_smem: legacy carve-out (cross-loop iter_arg or intra-WG async load)
    epi_l_i_1_smem_full = tlx.alloc_barriers(num_barriers=1, arrive_count=1)
    epi_l_i_1_smem_empty = tlx.alloc_barriers(num_barriers=1, arrive_count=1)
    with tlx.async_tasks():
        ...
        with tlx.async_task("default"):
            ...
            tlx.barrier_wait(epi_m_i_1_smem_full[0], 0)
            m_i_1 = tlx.local_load(epi_m_i_1_smem[0])
            tlx.barrier_wait(epi_l_i_0_smem_full[0], 0)
            l_i_0 = tlx.local_load(epi_l_i_0_smem[0])
            tlx.barrier_wait(epi_l_i_1_smem_full[0], 0)
            l_i_1 = tlx.local_load(epi_l_i_1_smem[0])
            acc = tlx.local_load(acc_tmem_5[0])
            tlx.barrier_arrive(acc_tmem_empty[0], 1)
            acc_1 = tlx.local_load(acc_tmem[0])
            tlx.barrier_arrive(acc_tmem_empty[0], 1)
            ...
            trunc_14 = (acc_1 / l_i_1[:, None]).to(tl.bfloat16)
        ...
        with tlx.async_task(num_warps=1, num_regs=24):
            ...
            for kv in range(0, div_12, 1):
                ...
                tlx.async_descriptor_load(k_desc, L0_smem_1[buf], [(mul_3 + (kv * 64)), 0], L0_smem_1_full[buf])
        ...
        with tlx.async_task(num_warps=1, num_regs=24):
            ...
            for kv in range(0, div_12, 1):
                ...
                buf = _it % 1
                phase = (_it // 1) & 1
                mdt_16 = tlx.local_trans(L0_smem_1[(_it % 2)])
                # MMA
                tlx.barrier_wait(L0_smem_1_full[(_it % 2)], ((_it // 2) & 1))
                tlx.barrier_wait(sem6_full[0], (_it & 1))
                use_acc = False
                tlx.async_dot(q_smem_8[0], tlx.local_trans(L0_smem_1[(_it % 2)]), acc_tmem_7[0], use_acc=use_acc, mBarriers=[L0_smem_1_empty[(_it % 2)], sem1_full[0]])
                # MMA
                tlx.barrier_wait(L0_smem_1_full[(_it % 2)], ((_it // 2) & 1))
                tlx.barrier_wait(sem8_full[0], (_it & 1))
                use_acc = False
                tlx.async_dot(q_smem_9[0], tlx.local_trans(L0_smem_1[(_it % 2)]), acc_tmem_6[0], use_acc=use_acc, mBarriers=[L0_smem_1_empty[(_it % 2)], sem4_full[0]])
        ...
        with tlx.async_task(num_warps=4, num_regs=152):
            ...
            tlx.barrier_arrive(epi_m_i_1_smem_full[0], 1)
            tlx.local_store(epi_l_i_1_smem[0], l_i_0)
            tlx.barrier_arrive(epi_l_i_1_smem_full[0], 1)


if __name__ == "__main__":
    import sys

    Z, H, N_CTX, D = 1, 32, 8192, 128
    ...
    if "--bench" in sys.argv:
        ...
        print(f"({Z},{H},{N_CTX}) sub-tiled solver kernel: {ms:.3f} ms = {tf:.1f} TFLOPS")
