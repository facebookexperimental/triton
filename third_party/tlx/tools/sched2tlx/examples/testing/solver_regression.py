...
CONFIGS = {
    # schedule backend only (deletes guard 2's II window for this path)
    "cpsat": {"TRITON_USE_MODULO_SCHEDULE": "cpsat"},
    # partition-only joint solve (v1: cycles fixed)
    "joint1": {"TRITON_MODULO_CPSAT_JOINT": "1"},
    # cycles + wg in one solve (v2)
    "joint2": {"TRITON_MODULO_CPSAT_JOINT": "2"},
    # full solver stack
    "full": {"TRITON_USE_MODULO_SCHEDULE": "cpsat", "TRITON_MODULO_CPSAT_JOINT": "2"},
    # full stack at the unguarded MinII (guard-1 deletion experiment)
    "full-noguard": {
        "TRITON_USE_MODULO_SCHEDULE": "cpsat",
        "TRITON_MODULO_CPSAT_JOINT": "2",
...
def dump_and_emit(opt: Path, case: str, cfg_env: dict, tmp: Path) -> Path | None:
    """Run the modulo pass under cfg_env and emit the candidate kernel.
