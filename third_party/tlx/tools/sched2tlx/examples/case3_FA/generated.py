...
# Warp groups (loop 0): wg0=[NONE+TMA], wg1=[NONE+TMA], wg2=[NONE+TC], wg3=[CUDA+NONE+SFU], wg4=[CUDA+NONE+TC]
import torch
import triton
...
def fa_fwd_kernel_nows(
    ...
    # sem4: N8→N7  ttng.tmem_load→ttng.tc_gen5_mma  cyc1458→cyc558  LOOP-CARRY  buf=6  kind=mbarrier
    sem4_full = tlx.alloc_barriers(num_barriers=1, arrive_count=1)
    # sem5: fan-out: producer N28 → N23, N27
    ...
    with tlx.async_tasks():
        ...
        with tlx.async_task(num_warps=4, num_regs=152):
            ...
            for tile_id in range(0, div_6, 1):
                ...
                buf = _it % 3
                phase = (_it // 3) & 1
                tlx.barrier_wait(sem5_full[0], (_it & 1))  # fan-out: producer N28 → N23, N27
                ...
                use_acc = (tile_id > 0)
                tlx.async_dot(L0_acc_tmem_2[0], L0_smem_0[buf], acc_tmem[0], use_acc=use_acc, mBarriers=[L0_acc_tmem_2_empty[0], L0_smem_0_empty[(_it % 3)], sem5_full[0]])
                i0_2 = True
