"""Case 7 — run the emitter-generated wgrad GEMM + bias-reduce on B200 vs torch."""
    ...
    for M, K_out, N_in in shapes:
        ...
        try:
            generated.wgrad_bias_nows[grid](
                dout, act, dw, db, M, K_out, N_in, num_warps=4, num_ctas=1, num_stages=2
            )
        except Exception as e:
            print(
                f"[FAIL-COMPILE] M={M} K_out={K_out} N_in={N_in}: {str(e).splitlines()[-1][:90]}"
            )
            failed += 1
            continue
