...
def emit(graph: ScheduleGraph) -> str:
    ...
    if _use_semaphore_ir():
        ...
        if rctx.has_acc_tmem_handoff and "acc_tmem" not in {c.name for c in channels}:
            ...
            # One arrive per function-scope epilogue tmem_load OF THE
            # CANONICAL acc_tmem: non-canonical accumulators (sub-tiled
            # second instance, FA-bwd dK/dV) carry their own full/empty
            # pair (_epilogue_acc_wg) and arrive those instead. Persistent
            # kernels (outer-loop epilogue) have zero function-scope loads
            # — keep the legacy count of 1.
            n_epi_acc_loads = sum(
                1 for op in graph.ops.values()
                if op.kind == "ttng.tmem_load" and op.scope == "function"
                and op.operands and isinstance(op.operands[0], OpRef)
                and rctx.alloc_op_var.get(op.operands[0].op_id, "acc_tmem")
                == "acc_tmem"
            )
            lines += (
                f"acc_tmem_empty = tlx.alloc_barriers"
