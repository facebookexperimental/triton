...
from third_party.tlx.tools.agents.worker.source import (
...
VALIDATOR_TARGETS_DIR = AGENTS_DIR / "validator" / "targets"
...
class HarnessTest(unittest.TestCase):
    ...
    def _bundle(
        ...
        target_dir = root / "validator" / "targets" / "arch" / "kernel"
        target_dir.mkdir(parents=True)
        if arch_knowledge is not None:
            knowledge = root / "knowledge_keeper" / "knowledge" / "arch" / "architecture.md"
            knowledge.parent.mkdir(parents=True)
            knowledge.write_text(arch_knowledge)
        if target_guidance is not None:
            guidance = root / "knowledge_keeper" / "knowledge" / "arch" / "targets" / "kernel.md"
            guidance.parent.mkdir(parents=True)
            guidance.write_text(target_guidance)
        if tl_strategy is not None:
            strategy = root / "tl" / "strategies" / "arch" / "kernel.md"
            strategy.parent.mkdir(parents=True)
            strategy.write_text(tl_strategy)
    ...
    def test_shipped_gfx942_bundle_supplies_guidance(self) -> None:
        target = VALIDATOR_TARGETS_DIR / "gfx942" / "gfx942" / "target.json"
        guidance = _resolve_guidance(target, "")
        self.assertIn("gfx942.md", guidance)
        self.assertIn("architecture.md", guidance)
    ...
    def test_shipped_blackwell_bundle_supplies_tl_strategy(self) -> None:
        target = VALIDATOR_TARGETS_DIR / "blackwell" / "gemm" / "target.json"
        guidance = _resolve_guidance(target, "")
        self.assertIn("gemm.md", guidance)
        self.assertNotIn("truncated", guidance)
    ...
    def test_resolves_arch_first_harness_layout(self) -> None:
        ...
        self.assertEqual(
            harness,
            VALIDATOR_TARGETS_DIR / "hopper" / "gemm" / "harness.py",
        )
    ...
    def test_default_arch_only_uses_arches_with_matching_target(self) -> None:
        ...
        self.assertEqual(
            harness,
            VALIDATOR_TARGETS_DIR / "host" / "vector_add" / "harness.py",
        )
        self.assertEqual(cases.name, "cases.json")
        self.assertEqual(target.name, "target.json")
    ...
    def test_mock_provider_replays_canned_candidates(self) -> None:
        ...
        from third_party.tlx.tools.agents.manager.models import PerformanceSummary as PS

        ctx = provider.propose.__code__  # touch to avoid unused
        del ctx
        # Simulate two sequential proposals via the same provider instance.
        dummy_perf = PS(cases=())
        from third_party.tlx.tools.agents.worker.providers import CandidateContext
    ...
    def test_provider_never_reads_knowledge_from_outside_the_bundle(self) -> None:
        ...
        from third_party.tlx.tools.agents.worker import providers
