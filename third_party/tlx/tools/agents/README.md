...
This directory contains the TLX-local optimization team for standalone Triton and TLX
kernels. Each role owns one directory containing `agent.md` and its executable actions.
The roles are Manager, Knowledge Keeper, TL, Profiler, Worker, Build, Validator, and
...
|---|---|---|
| Manager | `manager/` | Project state, budgets, orchestration, and verdicts |
| Knowledge Keeper | `knowledge_keeper/` | Human-approved durable guidance |
| TL | `tl/` | Evidence-backed causal findings |
| Profiler | `profiler/` | Raw profiler collection and normalization |
| Worker | `worker/` | Isolated source candidates and artifacts |
| Build Agent | `build_agent/` | Process isolation, rebuilds, recovery, and VCS |
| Validator | `validator/` | Correctness, benchmarks, and target bundles |
| Self Diagnosis | `self_diagnosis/` | Agent-system regression tests |
...
The default harness mode is **subprocess isolation** (`validator/runner.py` subprocess per candidate):
candidate state and imported kernel modules never leak across evaluations. An in-process
`StandaloneHarness` is available programmatically (via
`from third_party.tlx.tools.agents.build_agent.harness import StandaloneHarness`)
for debugging and unit tests.

Build/verify/benchmark/profile run in a new subprocess for every source candidate via
`validator/runner.py`. On timeout the agent sends `SIGTERM` then `SIGKILL` to the whole process
...
python -m third_party.tlx.tools.agents.manager.cli \
  ...
  --cases third_party/tlx/tools/agents/validator/targets/gfx942/gfx942/cases_819200x192x1024.json \
...
`validator/targets/blackwell/gemm/harness.py` runs any complete candidate source that exports
...
Canonical workflow guidance lives in `profiler/docs/proton.md` for Proton and
`profiler/docs/nvidia-ncu.md` for NVIDIA NCU. These documents guide harness and
...
Target-specific `harness.py`/`cases.json`/`target.json` live colocated under
`validator/targets/<arch>/<kernel>/` (B200,
...
Three optional Markdown files owned by TL and Knowledge Keeper are concatenated ahead
of `target.json`'s inline `optimization_guidance` string. TL strategy comes first,
followed by knowledge from broadest to narrowest scope, and the resolved text is
delivered as one field:

```text
tl/strategies/<arch>/<kernel>.md                       # TL strategy
knowledge_keeper/knowledge/<arch>/architecture.md     # architecture-wide
knowledge_keeper/knowledge/<arch>/targets/<kernel>.md # this target
```

The Manager resolves these paths from the selected validation target. The candidate
provider stays kernel- and architecture-neutral and receives one resolved guidance
string. The resolved block is capped at 16 KB, with truncation announced in the text.
...
`validator/targets/host/vector_add/harness.py` is a minimal CPU-friendly harness for smoke tests
