...
class KnowledgeBundle:
    ...
    def applies_to(self, target: KernelTarget) -> bool:
        ...
        ) and (self.selection is None or self.selection in target.optimization_skills)
    ...
    for manifest_path in sorted(_KNOWLEDGE_ROOT.glob("**/manifest.json")):
        ...
        bundles.append(
            KnowledgeBundle(
                ...
                selection=(
                    str(payload["selection"]).strip().lower()
...
def knowledge_paths(target: KernelTarget) -> tuple[Path, ...]:
    selected = set(target.optimization_skills)
    supported = {
        bundle.selection for bundle in knowledge_bundles() if bundle.selection is not None
    }
    ...
    if unknown:
        ...
        platform = "AMD " if backend in {"amd", "hip", "rocm"} else ""
        raise ValueError(
            f"unsupported {platform}optimization skill {name!r}; "
            f"supported skills: {supported_text}"
        )
    selected_bundles = [
        bundle
        for bundle in knowledge_bundles()
        if bundle.selection is not None and bundle.selection in selected
    ]
    if any(not bundle.applies_to(target) for bundle in selected_bundles):
        raise ValueError(
            f"optimization_skills are not supported for {target.backend} targets"
        )
    return tuple(
