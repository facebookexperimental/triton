...
def resolve_target_paths(
    ...
    operation = target_name or kernel.stem
    matches = [bundle for bundle in target_bundles() if bundle.operation == operation]
    ...
    if harness is None or cases is None or target is None:
        ...
        raise SystemExit(
            f"missing required {missing}; pass paths explicitly or register a target bundle for {operation!r}"
        )
    return harness, cases, target
