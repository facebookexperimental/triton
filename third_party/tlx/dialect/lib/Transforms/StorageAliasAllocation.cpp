...
LogicalResult materializeStorageAliasAllocations(
    ModuleOp m, const DenseMap<Value, std::tuple<int64_t, int64_t, int64_t>> &offsetMap,
  ...
  for (auto allocOp : allocOpsToReplace) {
    ...
    if (offsetIt != offsetMap.end()) {
      ...
      // Check if bytes_between_buffer_groups divides evenly by original buffer size
    ...
    if (offsetIt != offsetMap.end()) {
      ...
      if (scaleFactor > 1 || offsetSlots > 0) {
        ...
        for (auto indexOp : indexOpsToRewrite) {
          ...
          // Compute: newIndex = scaleFactor * originalIndex + offsetSlots + (originalIndex % groupSize)
          ...
          if (groupSize > 1) {
            ...
            Value modVal = builder.create<arith::RemSIOp>(loc, originalIndex, groupSizeVal);
