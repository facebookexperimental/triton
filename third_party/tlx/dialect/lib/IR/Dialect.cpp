...
struct TLXInferLayoutInterface : public triton::DialectInferLayoutInterface {
  ...
  LogicalResult inferSliceLike(Attribute operandEncoding,
    ...
    if (auto slice = dyn_cast<triton::gpu::SliceEncodingAttr>(result)) {
      ...
      // The nested no_verify on the parent is sufficient to defer validation:
      // hasNoVerifyLayout recursively inspects slice parents.  Do not add an
      // outer wrapper as well, because later slice canonicalization retains
      // only the parent wrapper and result-type inference must be idempotent.
