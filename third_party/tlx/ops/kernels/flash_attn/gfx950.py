"""AMD CDNA4 Flash Attention for ``tlx.ops``.

The public differentiable path combines the rotated-cluster forward with the
validated dense BF16 backward families in :mod:`gfx950_bwd`. It supports
contiguous square MHA with head dimension 64 or 128 when the shape is accepted
by both kernels. Inference retains the forward kernel's broader shape contract.
Legacy tutorial entry points remain at the end of the module for explicit
forward-kernel experiments.
"""
...
_CLUSTER_AUTOTUNE_KEY = [
    *_CLUSTER_WORKLOAD_KEY,
    "STORE_LSE",
    "ALLOW_PRESCALE_Q",
]
_CLUSTER_PERSISTENT_AUTOTUNE_KEY = [*_CLUSTER_WORKLOAD_KEY, "NUM_SMS", "NUM_XCDS"]
...
def _attn_cluster_tile(
    ...
    PRESCALE_Q: tl.constexpr = (
        ALLOW_PRESCALE_Q and Q_PRESCALE_DOES_NOT_MAGNIFY and HEAD_DIM <= 128
    ...
    if STORE_LSE:
        # The running maximum and denominator use base-2 softmax. Backward
        # consumes natural-log LSE, so convert only after combining both terms.
        lse = (state.m_i + tl.math.log2(state.l_i)) * _LN_2
...
def _attn_fwd_cluster_pipeline(
    ...
    ALLOW_PRESCALE_Q: tl.constexpr = True,
):
    ...
    if STORE_LSE:
        lse_base = LSE + (off_z * H + off_h) * N_CTX
    else:
        lse_base = Out
    _attn_cluster_tile(
        pid_m,
...
def flash_attn_cluster_pipeline(
    ...
    **kw,
    ...
    if _lse is not None and not _return_lse:
        raise ValueError("_lse requires _return_lse=True")
    o = torch.empty_like(q)
    lse = (_lse if _lse is not None else torch.empty(q.shape[:-1], device=q.device,
                                                     dtype=torch.float32)) if _return_lse else o
    ...
    if _return_lse:
        launch_meta.update({
            "LSE": lse,
            "STORE_LSE": True,
            "ALLOW_PRESCALE_Q": _allow_prescale_q,
        })
    ...
    return (o, lse) if _return_lse else o
...
def _flash_attn_forward(q, k, v, sm_scale, causal, space):
    ...
    return flash_attn_cluster_pipeline(
        ...
        causal=causal,
        _return_lse=True,
        _allow_prescale_q=False,
        _lse=lse,
        **config,
    )


def _flash_attn_inference(q, k, v, sm_scale, causal, space):
    """Run the original forward path when no backward state is required."""
    config = {} if space == "full" else {"USE_DIRECT_LOAD": False}
    return flash_attn_cluster_pipeline(q, k, v, sm_scale, causal=causal, **config)
...
class _attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, sm_scale, causal, space):
        o, lse = _flash_attn_forward(q, k, v, sm_scale, causal, space)
...
def flash_attn(q, k, v, causal=False, sm_scale=None, *, space="full"):
    ...
    try:
        ...
        raise InvalidInput("sm_scale must be a finite number") from error
    if not math.isfinite(sm_scale):
        raise InvalidInput("sm_scale must be a finite number")
    if not torch.is_grad_enabled() or not any(tensor.requires_grad for tensor in (q, k, v)):
        return _flash_attn_inference(q, k, v, sm_scale, causal, space)
    return _attention.apply(q, k, v, sm_scale, causal, space)
