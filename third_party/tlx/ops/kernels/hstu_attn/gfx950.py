...
"""gfx950 HSTU ragged attention -- the ``tlx.ops.hstu_attn_dev`` backend.

Promoted from ``tutorials/hstu_self_attn`` after the forward and backward paths
were finalized. The tutorial remains a frozen standalone snapshot; this module
is the production implementation exposed through ``tlx.ops``.
...
bias, contextual prefix, max_attn_len, targets -- is carried over unchanged.
Porting the load path to TLX is a follow-up.
"""
...
_FWD_CONFIGS = _get_fw_configs()
_FWD_SMOKE_CONFIGS = [
    config for config in _FWD_CONFIGS if config.kwargs["BLOCK_M"] == 64 and config.kwargs["BLOCK_N"] == 32
    and config.kwargs["matrix_instr_nonkdim"] == 16 and config.num_stages == 1 and config.num_warps == 4
]
assert len(_FWD_SMOKE_CONFIGS) == 1

_tlx_gfx950_ragged_hstu_attn_fwd_jit = _tlx_gfx950_ragged_hstu_attn_fwd
...
)(_tlx_gfx950_ragged_hstu_attn_fwd_jit.fn)
_tlx_gfx950_ragged_hstu_attn_fwd_smoke = triton_autotune(
    configs=_FWD_SMOKE_CONFIGS,
    key=[
        "AUTOTUNE_Z",
        "H",
        "AUTOTUNE_MAX_SEQ_LEN",
...
# The public operator selects the reduced smoke space for correctness tests.
# Direct kernel/performance callers retain the full space by default.
_SPACE = "full"
...
def tlx_gfx950_ragged_attention_fwd(
    ...
    fwd_kernel = _tlx_gfx950_ragged_hstu_attn_fwd if _SPACE == "full" else _tlx_gfx950_ragged_hstu_attn_fwd_smoke
    fwd_kernel[grid](
        Q=q,
        K=k,
        V=v,
...
def _tlx_gfx950_ragged_hstu_attn_bwd_one_col_block(  # noqa C901
    ...
    # LDS staging for q / dOut. The base vectors are copied from
    # third_party/tlx/ops/kernels/flash_attn/gfx950_bwd.py; they must match the
    # tile shape exactly (a row bit past the tile makes the memdesc reinterpret
...
# The OSS implementation keeps it a plain function -- it is only ever called from
...
# -----------------------------------------------------------------------------
# Backward variants and public entry points
# -----------------------------------------------------------------------------
#
...
def hstu_attn(
    ...
    *,
    ...
    """HSTU ragged attention over ``(total_tokens, H, HEAD_DIM)``.

    The fully interleaved gfx950 backward is selected for the production-shaped
    target-causal D=128 path. Other supported shapes use the general backward.
    ``space="full"`` uses the complete forward autotune space, while
    ``space="smoke"`` uses one representative configuration for correctness.
    """
    if not causal:
        raise ValueError("hstu_attn is causal-only; causal=False is not implemented")
    if space not in ("full", "smoke"):
        raise ValueError(f"hstu_attn does not provide space={space!r}")
    global _SPACE
    _SPACE = space
    use_fa_schedule = (num_targets is not None and attn_scale is None and max_attn_len == 0 and contextual_seq_len == 0
                       and q.shape[-1] == 128 and v.shape[-1] == 128)
    bwd_variant = DEFAULT_BWD_VARIANT if use_fa_schedule else "default"
    return tlx_gfx950_hstu_mha(
        max_seq_len=max_seq_len,
        alpha=alpha,
        q=q,
        k=k,
