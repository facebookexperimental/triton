...
def _plan_for(a, b):
    ...
    m, k = a.shape
    _, n = b.shape
    if not (a.dtype == torch.float16 and b.dtype == torch.float16 and a.is_cuda and a.device == b.device
...
def supports(a, b):
    ...
    return _plan_for(a, b) is not None or _register_plan_for(a, b) is not None


def matmul(a, b, out=None):
    """Run the selected gfx950 GEMM specialization."""
    local_split_u_plan = _plan_for(a, b)
    register_plan = (None if local_split_u_plan is not None else _register_plan_for(a, b))
    if local_split_u_plan is None and register_plan is None:
    ...
    return _launch_validated(a, b, out, local_split_u_plan)
