"""V4 - Making the epilogue nearly free.
...
V4 keeps V3's deep pipeline and persistent schedule and attacks the three costs
left in the epilogue:
  * Fold the bias into the accumulator (acc initialized to bias), so the K-loop
    yields X = bias + A@B directly and the bias add disappears.
  * Prefetch Y into registers just before the drain matmuls, so its HBM latency
    hides under those tail dots and Y is resident when the gate runs.
...
def addmm_glu_v4(
    ...
    for tile_id in tl.range(start_pid, num_tiles, NUM_CUS):
        ...
        # Prologue: 3 global reads, A+B combined per group.
        k_start = 0
        ...
        # Bias folded into the accumulator init: the K-loop yields X = bias + A@B.
        ...
        # Register-resident Y prefetch before the drain dots (hides its HBM latency).
        ...
        # Fused gate: bias already in acc, Y resident -> single packed FMA.
...
def run(a, b, bias, y):
    """Launch the V4 (persistent, fused-epilogue) kernel. Returns out = (A@B + bias) + (A@B + bias) * Y."""
    Mx, K = a.shape
    _, Nx = b.shape
    cfg = BEST_CONFIG[K]
