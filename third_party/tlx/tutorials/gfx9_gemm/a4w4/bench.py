...
def launch_matmul(a, b, a_scales, b_scales, out=None, extra_compile_options=None):
    M = a.shape[0]
    K_packed = a.shape[1]
    K = K_packed * 2
    N = b.shape[0]
...
def benchmark_matmul(args, a, b, a_scales, b_scales, out=None):
...
def compile_shape(shape, cache_root, wave_split_barriers=False):
    M, N, K = shape
...
def compile_shape_worker(shape, cache_root, wave_split_barriers=False):
    start = time.monotonic()
    compile_shape(shape, cache_root, wave_split_barriers)
    return shape, time.monotonic() - start


def precompile_shapes(sizes, cache_root, workers, wave_split_barriers=False):
    if workers == 0:
        return
    ...
    if workers == 1:
        for shape in sizes:
            compiled_shape, elapsed = compile_shape_worker(shape, cache_root, wave_split_barriers)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=workers, mp_context=context) as executor:
        futures = [executor.submit(compile_shape_worker, shape, cache_root, wave_split_barriers) for shape in sizes]
...
def main():
    ...
    parser.add_argument(
        "--wave-split-barriers",
        action="store_true",
        help="compile kernels with tlx_wave_enable_split_barriers=True; requires the tlx_wave backend",
    )
    parser.add_argument(
        "--compile-workers",
        type=nonnegative_int,
    ...
    if args.wave_split_barriers:
        ...
        if target.backend != "tlx_wave":
            parser.error("--wave-split-barriers requires TRITON_DEFAULT_BACKEND=tlx_wave")

    precompile_shapes(sizes, args.cache_dir, args.compile_workers, args.wave_split_barriers)
    ...
    for M, N, K in sizes:
        ...
        with knobs.cache.scope():
            ...
            benchmark_matmul(args, a, b, a_scales, b_scales, out=c)
            ...
            if ok and not args.no_bench:
                ms = measure_matmul(
                    args,
                    lambda: benchmark_matmul(args, a, b, a_scales, b_scales, out=c),
                )
