"""TLX MXFP4 GEMM for gfx950 -- 64x128 tile, for the M=256/512 x N=4096..8192 shapes.

Working and exact on 256x{4096,8192}x4096 and 512x{4096,8192}x4096, validated
bit-for-bit against the 256x256 kernel. Zero ds_write in the main loop; the
epilogue keeps 4 (see "Epilogue" below).

Ratio vs AITER on the official harness (higher is better), by change:

    shape            K=512/body   K=1024/body   + B direct-to-VGPR
    256x4096x4096       0.767        0.854            0.889
    256x8192x4096       0.874        0.961            0.953
    512x4096x4096       0.869        0.955            0.988
    512x8192x4096       0.824        0.726            0.829

Use this tile for M=256 and for 512x4096. At 512x8192 the 128x128 kernel still
wins (0.996), matching AITER's own choice of a 128x128 kernel there.

The loop body now matches AITER's 64x128 kernel op-for-op: 64 MFMA, 32
ds_read_b128, 8 buffer_load_dwordx4 to LDS, 16-18 to VGPR, 4 s_barrier,
8 s_waitcnt -- at 232 VGPR with no spills.
...
def _a4w4_kernel_64(
    ...
    #
    # smem_a is (BLOCK_M, BLOCK_K/2) = (64, 256) = 2**14. Left on the compiler
    # default these allocs gave LDSBankConflict = 18.9% with MfmaUtil at 12%, i.e.
    # the MFMA unit idle 88% of the time while MemUnitStalled was ~0: LDS-latency
    # bound, not memory bound. The [[1024, 32]] pad breaks the conflicting stride.
    ...
    a_row = (offs_asm % 16) * 16 + ((offs_asm // 16) % 2) * 4 + (offs_asm // 32) * 8
    a_col = (offs_sk % 4) * 256 + (offs_sk // 4)
    ...
    a_sc_buf0 = tlx.require_layout(tlx.buffer_load(a_scales_base, a_scale_offsets, contiguity=16), scale_mfma_layout_a)
    ...
    a_sc_buf1 = tlx.require_layout(tlx.buffer_load(a_scales_base, a_scale_offsets_next, contiguity=16),
                                   scale_mfma_layout_a)
    ...
    for _ in tl.range(0, iter_max - 2, 2, num_stages=1):
        ...
        tlx.buffer_load_to_local(smem_a[0], a_base, a_offsets)
        b_buf0 = tlx.require_layout(tlx.buffer_load(b_base, b_offsets, contiguity=16), b_operand_layout)
        a_sc_buf0 = tlx.require_layout(tlx.buffer_load(a_scales_base, a_scale_offsets, contiguity=16),
                                       scale_mfma_layout_a)
...
def _shuffle_scale(scales, tile_rows):
    ...
    if tile_rows == 64:
        pos = ((r % 16)[:, None] * 16 + ((r // 16) % 2)[:, None] * 4 + (r // 32)[:, None] * 8 + (k % 4)[None, :] * 256 +
               (k // 4)[None, :])
    else:
