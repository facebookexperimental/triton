...
def _load_a_fragment(region, phase: tl.constexpr, tile: tl.constexpr, ready):
    ...
    fragment = tlx.require_layout(tlx.local_load(fragment_view, token=ready), fragment_layout)
...
def _prefetch_tile_subpanels(
    ...
    a1_early_view = tlx.local_slice(smem_a[buffer], [0, 0, 1, 0, 0, 0], [1, 2, 1, 8, 4, 32])
    a1_early = tlx.buffer_load_to_local(a1_early_view, a_ptr, a_subpanel1_request_off + tile_k * stride_ak)
    tlx.async_load_commit_group(tokens=[a0, a1_early])

    a1_late1_view = tlx.local_slice(smem_a[buffer], [0, 0, 1, 0, 4, 0], [1, 2, 1, 8, 4, 32])
    a1_late2_view = tlx.local_slice(smem_a[buffer], [0, 0, 1, 0, 8, 0], [1, 2, 1, 8, 4, 32])
    a1_late3_view = tlx.local_slice(smem_a[buffer], [0, 0, 1, 0, 12, 0], [1, 2, 1, 8, 4, 32])
    b0_view = tlx.local_slice(smem_b[buffer], [1, 0, 0, 0, 0, 0], [1, 2, 1, 8, 32, 16])
    b1_early_view = tlx.local_slice(smem_b[buffer], [1, 0, 1, 0, 0, 0], [1, 2, 1, 8, 32, 4])
    a1_late1 = tlx.buffer_load_to_local(a1_late1_view, a_ptr,
                                        a_subpanel1_request_off + 32 * stride_am + tile_k * stride_ak)
    a1_late2 = tlx.buffer_load_to_local(a1_late2_view, a_ptr,
                                        a_subpanel1_request_off + 64 * stride_am + tile_k * stride_ak)
    a1_late3 = tlx.buffer_load_to_local(a1_late3_view, a_ptr,
    ...
    b1_early = tlx.buffer_load_to_local(b1_early_view, b_ptr, b_subpanel1_request_off + tile_k * stride_bk)
    tlx.async_load_commit_group(tokens=[a1_late1, a1_late2, a1_late3, b0, b1_early])

    b1_late1_view = tlx.local_slice(smem_b[buffer], [1, 0, 1, 0, 0, 4], [1, 2, 1, 8, 32, 4])
    b1_late2_view = tlx.local_slice(smem_b[buffer], [1, 0, 1, 0, 0, 8], [1, 2, 1, 8, 32, 4])
    b1_late3_view = tlx.local_slice(smem_b[buffer], [1, 0, 1, 0, 0, 12], [1, 2, 1, 8, 32, 4])
    b1_late1 = tlx.buffer_load_to_local(b1_late1_view, b_ptr,
                                        b_subpanel1_request_off + 32 * stride_bn + tile_k * stride_bk)
    b1_late2 = tlx.buffer_load_to_local(b1_late2_view, b_ptr,
                                        b_subpanel1_request_off + 64 * stride_bn + tile_k * stride_bk)
    b1_late3 = tlx.buffer_load_to_local(b1_late3_view, b_ptr,
...
def _mma_range(a, b, acc, begin: tl.constexpr, end: tl.constexpr):
    ...
    emitted = tl.tuple([
        tl.dot(
            ...
            b[ordinal // 8],
            acc[(ordinal // 8) * 8 + ((ordinal % 8) if (ordinal // 8) % 2 == 0 else 7 - ordinal % 8)],
        ) if begin <= ordinal and ordinal < end else
        acc[(ordinal // 8) * 8 + ((ordinal % 8) if (ordinal // 8) % 2 == 0 else 7 - ordinal % 8)]
        for ordinal in range(64)
    ])
    return tl.tuple([
        tlx.release_layout(emitted[(index // 8) * 8 + ((index % 8) if (index // 8) % 2 == 0 else 7 - index % 8)])
...
def _compute_tile_and_refill(
    ...
    refill_a1_early_token = tlx.buffer_load_to_local(refill_a1_early, refill_a_ptr, a_subpanel1_request_off)
    ...
    refill_a1_late1_token = tlx.buffer_load_to_local(refill_a1_late1, refill_a_ptr,
                                                     a_subpanel1_request_off + 32 * stride_am)
    refill_a1_late2_token = tlx.buffer_load_to_local(refill_a1_late2, refill_a_ptr,
                                                     a_subpanel1_request_off + 64 * stride_am)
    refill_a1_late3_token = tlx.buffer_load_to_local(refill_a1_late3, refill_a_ptr,
                                                     a_subpanel1_request_off + 96 * stride_am)
    refill_b0_token = tlx.buffer_load_to_local(refill_b0, refill_b_ptr, b_subpanel0_off)
    refill_b1_early_token = tlx.buffer_load_to_local(refill_b1_early, refill_b_ptr, b_subpanel1_request_off)
    ...
    refill_b1_late1_token = tlx.buffer_load_to_local(refill_b1_late1, refill_b_ptr,
                                                     b_subpanel1_request_off + 32 * stride_bn)
    refill_b1_late2_token = tlx.buffer_load_to_local(refill_b1_late2, refill_b_ptr,
                                                     b_subpanel1_request_off + 64 * stride_bn)
    refill_b1_late3_token = tlx.buffer_load_to_local(refill_b1_late3, refill_b_ptr,
    ...
    return acc, (na0, na1, na2, na3, na4, na5, na6, na7), (nb0, nb1, nb2, nb3, nb4, nb5, nb6, nb7), next_ready
...
@triton.jit
def _store_acc_strip(c_ptr, acc, pid_m, pid_n, stride_cm, stride_cn, local_m: tl.constexpr):
    ...
    offs_n = (pid_n * 256 + (within_tile_n // 16) * 128 + (within_tile_n % 16) * 8 + tile_n)
...
def wave_4wave_specialized(
    ...
    a_load_layout: tl.constexpr = tlx.padded_shared_layout_encoding.with_identity_for([(512, 8)], [2, 2, 2, 8, 16, 32],
                                                                                      order=[5, 1, 3, 4, 2, 0])
    b_load_layout: tl.constexpr = tlx.padded_shared_layout_encoding.with_identity_for([(512, 8)], [2, 2, 2, 8, 32, 16],
                                                                                      order=[4, 1, 3, 5, 2, 0])
    ...
    smem_b = tlx.local_alloc((2, 2, 2, 8, 32, 16), tl.float16, 2, reuse=smem_a, layout=b_load_layout)
    ...
    a_subpanel0_off = ((pid_m * BLOCK_M + a_row * 8 + a_tile) * stride_am + (a_phase * 32 + a_k) * stride_ak)
    a_subpanel1_request_off = ((pid_m * BLOCK_M + 128 + a_request_row * 8 + a_tile) * stride_am +
    ...
    b_subpanel0_off = ((b_phase * 32 + b_k) * stride_bk + (pid_n * BLOCK_N + b_col * 8 + b_tile) * stride_bn)
    b_subpanel1_request_off = ((b_phase * 32 + b_k) * stride_bk +
                               (pid_n * BLOCK_N + 128 + b_request_col * 8 + b_tile) * stride_bn)

    zero = tl.zeros((32, 32), dtype=tl.float32)
    acc = tl.tuple([zero for _ in range(64)])
    iter_max: tl.constexpr = K // BLOCK_K

    _prefetch_tile_subpanels(smem_a, smem_b, 0, a_ptr, b_ptr, a_subpanel0_off, a_subpanel1_request_off, b_subpanel0_off,
                             b_subpanel1_request_off, 0, stride_am, stride_ak, stride_bk, stride_bn)
    _prefetch_tile_subpanels(smem_a, smem_b, 1, a_ptr, b_ptr, a_subpanel0_off, a_subpanel1_request_off, b_subpanel0_off,
                             b_subpanel1_request_off, BLOCK_K, stride_am, stride_ak, stride_bk, stride_bn)
    ...
    for k in tl.range(0, iter_max - 2, num_stages=1):
        ...
        acc, a0, b0, current_ready = _compute_tile_and_refill(a0, b0, acc, current_a, current_b, next_a, next_b,
                                                              refill_a_ptr, refill_b_ptr, a_subpanel0_off,
                                                              a_subpanel1_request_off, b_subpanel0_off,
                                                              b_subpanel1_request_off, stride_am, stride_bn,
                                                              current_ready)
        current_a = next_a
        current_b = next_b
...
def matmul(a, b):
    ...
    assert b.stride(0) == 1, ("wave_4wave_specialized expects a K-contiguous transposed-B view")
    M, K = a.shape
    K, N = b.shape
