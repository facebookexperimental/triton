...
def mxgemm_tdm_pipelined_kernel(
    ...
    if TDM_SPLIT:
        ...
        tl.static_assert(NUM_BUFFERS >= 2, "split MXFP TDM pipeline requires at least two buffers")
    ...
    if SCHEDULE == "sliceMNK":
        ...
        if TDM_SPLIT:
            ...
            a00, _ = _mxgemm_load_a_operand_split(a0_buf, a1_buf, a_scale_buf, wmma_idx, 0, 0, BLOCK_M, BLOCK_K,
                                                  DIV_FACTOR_A, SCALE_BLOCK, BLOCK_K_SCALE, BLOCK_M_PRESHUFFLED,
                                                  SCALE_KWIDTH, NUM_BUFFERS, NUM_SUBTILES_M, NUM_SUBTILES_K,
                                                  SCALE_PRESHUFFLE, WITH_A_SCALE)
            b00, _ = _mxgemm_load_b_operand_split(b0_buf, b1_buf, b_scale_buf, wmma_idx, 0, 0, BLOCK_N, BLOCK_K,
        ...
        for i in tl.range(0, K_ITERS):
            if TDM_SPLIT:
                # Keep the split scale loads in the iteration that consumes them.
                # This lets scaled WMMA select its canonical scale layouts directly
                # instead of fixing a loop-carried scale layout.
                _, scale_a00 = _mxgemm_load_a_operand_split(a0_buf, a1_buf, a_scale_buf, wmma_idx, 0, 0, BLOCK_M,
                                                            ...
                                                            WITH_A_SCALE)
                _, scale_b00 = _mxgemm_load_b_operand_split(b0_buf, b1_buf, b_scale_buf, wmma_idx, 0, 0, BLOCK_N,
...
def _validate_split_pipeline_depth(K: int, block_k: int, num_buffers: int) -> None:
    if num_buffers < 2:
        raise ValueError(f"NUM_BUFFERS must be at least 2, got {num_buffers}")
    if triton.cdiv(K, block_k) < num_buffers:
        raise ValueError(f"K={K} with BLOCK_K={block_k} does not provide the {num_buffers} pipeline tiles required")
...
def mxgemm_tdm_pipelined(
    ...
    assert K == Kb
    if TDM_SPLIT:
        _validate_split_pipeline_depth(K, BLOCK_K, NUM_BUFFERS)
