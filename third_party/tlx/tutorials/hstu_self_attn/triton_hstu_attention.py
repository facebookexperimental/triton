...
def _fma_f32x2(a, b, c):
    return tl.inline_asm_elementwise(
        ...
        """,
        "=r,=r,r,r,r,r,r,r", [a, b, c],
        dtype=tl.float32, is_pure=True, pack=2,
    )
...
def backward_activation(qk_trans, alpha, scale, valid_mask_trans, k):
    ...
    one_plus_tanh = _fma_f32x2(_tanh_approx_fp32(half_qk), 1.0, 1.0)
...
def backward_d_activation_prescaled(dact_qk_trans, one_plus_tanh, half_qk, scale):
    ...
    derivative = _fma_f32x2(half_qk, one_minus_tanh, 1.0)
...
def _hstu_attn_bwd_one_block_0(  # noqa C901
    ...
    if MASK_IF:
        ...
        if apply_mask:
            ...
            if DQ_REUSE:
                qk_trans, sig_trans, act_qk_trans = backward_activation_prescaled(
            ...
            if DQ_REUSE:
                qk_trans, sig_trans, act_qk_trans = backward_activation_prescaled_unmasked(
    ...
    if MASK_IF:
        if apply_mask:
            if DQ_REUSE:
                dqk_trans = backward_d_activation_prescaled(dact_qk_trans, sig_trans, qk_trans, scale)
            else:
...
def _hstu_attn_bwd(  # noqa C901
    ...
    DQ_REUSE: tl.constexpr = False,
    DKDV_SUBTILE: tl.constexpr = 1,
    SMEM_ALLOC_ALGO: tl.constexpr = 1,
