...
def _single_warp_per_simd_issue_loads(
    ...
    if not TRANSPOSE_B:
        tlx.async_amd_descriptor_load_fused([
            ...
            (tlx.update_tensor_descriptor(b_desc, add_offsets=[producer * BLOCK_K, off_n], clamp_bounds=False,
    ...
    if not TRANSPOSE_B:
        tlx.async_amd_descriptor_load_fused([
            (tlx.update_tensor_descriptor(a_desc, add_offsets=[off_m, producer * BLOCK_K],
                                          clamp_bounds=False), tlx.local_view(a_buf, slot), 3),
            (tlx.update_tensor_descriptor(b_desc, add_offsets=[producer * BLOCK_K, off_n],
                                          clamp_bounds=False), tlx.local_view(b_buf, slot), 12),
        ])
    else:
        tlx.async_amd_descriptor_load_fused([
            (tlx.update_tensor_descriptor(a_desc, add_offsets=[off_m, producer * BLOCK_K],
...
def matmul_tdm_pipelined_kernel(
    ...
    tlx.async_amd_descriptor_load(a_desc, tlx.local_view(a_buf, 0), [off_m, 0], clamp_bounds=False)
    tlx.async_amd_descriptor_load(b_desc, tlx.local_view(b_buf, 0), [0, off_n], clamp_bounds=False)
    ...
    tlx.async_amd_descriptor_store(c_desc, c_view, [off_m, off_n], clamp_bounds=False)
    tlx.async_amd_descriptor_wait(0)
...
def matmul(a: torch.Tensor, b: torch.Tensor, config=None) -> torch.Tensor:
    """Compute C = A @ B with the TDM pipeline and the tutorial config API."""
    cfg = {"BLOCK_M": 128, "BLOCK_N": 128, "BLOCK_K": 32}
    if config is not None:
        cfg.update(config)
    return matmul_tdm_pipelined(a, b, **cfg)
...
if __name__ == "__main__":
    ...
    a = torch.randn((args.M, args.K), device=triton.runtime.driver.active.get_active_torch_device(),
                    dtype=torch.float16)
    if args.kernel == "single_warp" and args.transpose_b:
        b = torch.randn((args.N, args.K), device=triton.runtime.driver.active.get_active_torch_device(),
                        dtype=torch.float16)
    else:
        b = torch.randn((args.K, args.N), device=triton.runtime.driver.active.get_active_torch_device(),
                        dtype=torch.float16)
