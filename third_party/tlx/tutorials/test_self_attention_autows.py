...
def test_self_attention_bwd_autows_clc_loop_dq_matrix(split_causal_loops, dq_transposed, L, target_count):
    ...
    The target suffix starts inside a KV tile while a later Q tile exists, which
    exercises the target-clamped causal mask without introducing a partial Q
    tile. Under targets the split path deliberately has an empty unmasked
    sibling, so those cells cover the empty-sibling release in rule #35 rather
    than a masked/unmasked split. The reused-CTA grid checks that release.
    """
    if not torch.cuda.is_available():
        pytest.skip("requires CUDA")
    Z = 120
