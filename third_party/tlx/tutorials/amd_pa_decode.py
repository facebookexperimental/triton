...
def pa_decode_tlx(output,  # [num_tokens, num_q_heads, HEAD_DIM]
    ...
    if num_warps is None:
        if streaming_kv:
            num_warps = 2 if short_streaming else 4
        else:
            num_warps = 2 if query_length > 1 else 4
    if streaming_kv:
    ...
    # KV compute tile width in keys, independent of PAGE_SIZE. The D64 decode
    # path uses a 256-token tile once the context reaches 32K: amortizing page
    # gathers and online-softmax updates wins there, while the smaller 128-token
    # tile remains faster at 8K. MTP keeps the narrower tile to limit pressure.
    if block_n is None:
        if streaming_kv:
    ...
    if num_splits is None:
        if streaming_kv:
            # Match SGLang's production two-wave target by default. Medium
            # batch/context is the exception: 16 shorter partitions hide its
            # long per-CTA dependency chain better than the usual eight.
    ...
    # Shape-specific compiler occupancy controls confirmed with alternating
    # fresh-process measurements on MI350X. Keep explicit caller values intact:
    # the same WPE=2 constraint is not a general win for this register kernel.
    ...
    if fused:
        ...
        if workspace is None:
            ...
            mid, lse = workspace
            assert mid.shape == mid_shape and lse.shape == lse_shape
