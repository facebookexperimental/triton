...
def _attn_fwd_mxf8_ws(sm_scale, M,  #
    ...
    # Compute p_dtype from V descriptor (softmax output P will use same precision as V)
    p_dtype = tlx.dtype_of(desc_v)
    ...
    K_BYTES_PER_ELEM: tl.constexpr = tlx.size_of(tlx.dtype_of(desc_k))
    V_BYTES_PER_ELEM: tl.constexpr = tlx.size_of(tlx.dtype_of(desc_v))
    P_BYTES_PER_ELEM: tl.constexpr = tlx.size_of(p_dtype)
    qk_dtype = tl.float32
    QK_BYTES_PER_ELEM: tl.constexpr = tlx.size_of(qk_dtype)
    # Padding factor for P buffer indexing to maintain correct byte offsets when reusing qk_tiles.
    # P tiles are designed to occupy the second half of each qk buffer. With smaller dtypes
    # (e.g., fp8 vs fp16), we need to pad the buffer index to land at the same byte offset.
    # The factor represents: differences in bytes allocated.
    P_PADDING: tl.constexpr = QK_BYTES_PER_ELEM // (2 * P_BYTES_PER_ELEM)
    ...
    # Explicitly allocate 2 q_scale buffers using indices 0 and 1
    q_scale_tiles_0 = tlx.local_alloc((1, REP_M, REP_HEAD, 2, 256), tl.uint8, 1)
    q_scale_tiles_1 = tlx.local_alloc((1, REP_M, REP_HEAD, 2, 256), tl.uint8, 1)
    # Explicitly allocate 1 k_scale buffer (k_bufIdx is always 0 since accum_cnt_kv increments by 2)
    k_scale_tiles_0 = tlx.local_alloc((1, REP_N, REP_HEAD, 2, 256), tl.uint8, 1)

    v_scale_tiles_1 = tlx.local_alloc((1, REP_N, REP_HEAD, 2, 256), tl.uint8, 1)
    ...
    p_scale_tiles = tlx.local_alloc((1, REP_M, REP_HEAD, 2, 256), tl.uint8, 1, tlx.storage_kind.tmem)

    # Explicitly allocate 2 q_scale barrier sets using indices 0 and 1
    q_scale_fulls_0 = tlx.alloc_barriers(num_barriers=1)
    q_scale_fulls_1 = tlx.alloc_barriers(num_barriers=1)
    q_scale_empties_0 = tlx.alloc_barriers(num_barriers=1)
    q_scale_empties_1 = tlx.alloc_barriers(num_barriers=1)
    # Explicitly allocate 1 k_scale barrier set (k_bufIdx is always 0)
    k_scale_fulls_0 = tlx.alloc_barriers(num_barriers=1)
    k_scale_empties_0 = tlx.alloc_barriers(num_barriers=1)
    # Explicitly allocate 1 v_scale barrier set (v_bufIdx is always 1)
    v_scale_fulls_1 = tlx.alloc_barriers(num_barriers=1)
    v_scale_empties_1 = tlx.alloc_barriers(num_barriers=1)
    ...
    # When working with smaller dtypes (e.g. FP8), we pad the original data to match the original
    # boundaries and prevent overlap.
    # For FP8 (USE_SCALE_MMA), p_tiles are in SMEM (not reused), and scales are in TMEM.
    # For non-FP8, p_tiles reuse qk_tiles from TMEM.
    NUM_P_BUFFERS: tl.constexpr = NUM_MMA_GROUPS * NUM_MMA_SLICES * 2 * P_PADDING
    # For FP8: p_tiles in SMEM (not reused from qk_tiles)
    # TODO: Validate
    p_tiles = tlx.local_alloc(
        (BLOCK_M_SPLIT, BLOCK_N // NUM_MMA_SLICES),
        p_dtype,
    ...
    with tlx.async_tasks():
        ...
        with tlx.async_task(num_warps=1, registers=24):
            ...
            for j in range(0, tiles_per_sm):
                ...
                # Use explicit q_scale buffer 0 for group 0 (q0)
                tlx.barrier_wait(q_scale_fulls_0[0], q_phase)
                # k_bufIdx is always 0 (accum_cnt_kv % 2 where accum_cnt_kv increments by 2)
                tlx.barrier_wait(k_scale_fulls_0[0], k_phase)
                ...
                # Use explicit q_scale buffer 1 for group 1 (q1)
                tlx.barrier_wait(q_scale_fulls_1[0], q_phase)
                tlx.async_dot_scaled(
                    q_tiles[1],
                    k_tile,
                    qk_tiles[1],
                ...
                tlx.barrier_wait(v_scale_fulls_1[0], v_phase)
                # Use p[NUM_MMA_SLICES + slice_id] for cid=0, and
                # p[NUM_MMA_GROUPS * NUM_MMA_SLICES + NUM_MMA_SLICES + slice_id] for cid=1
                for slice_id in tl.static_range(0, NUM_MMA_SLICES):
                    tlx.barrier_wait(p_fulls[slice_id + 0 * NUM_MMA_SLICES], qk_phase)
                ...
                for i in tl.range(lo + BLOCK_N, hi, BLOCK_N):
                    ...
                    for slice_id in tl.static_range(0, NUM_MMA_SLICES):
                        ...
                        mBarriers_scaled = ([kv_empties[v_bufIdx_prev], v_scale_empties_1[0]]
                    ...
                    tlx.async_dot_scaled(
                        ...
                        K_FP8_FORMAT,
                        use_acc=False,
                        mBarriers=[qk_fulls[1], kv_empties[k_bufIdx], k_scale_empties_0[0]],
                    )
                ...
                for slice_id in tl.static_range(0, NUM_MMA_SLICES):
                    ...
                    # v_bufIdx is always 1, use explicit buffer
                    mBarriers_scaled = ([acc_empties[1], kv_empties[v_bufIdx], v_scale_empties_1[0]]
                                        if slice_id == NUM_MMA_SLICES - 1 else [])
                    tlx.async_dot_scaled(
                        p_tiles[p_bufIdx],
                        kv_slice,
        ...
        with tlx.async_task(num_warps=1, registers=24):
            ...
            for i in range(0, tiles_per_sm):
                ...
                # Use explicit q_scale buffer 0 for group 0 (q0)
                tlx.barrier_wait(q_scale_empties_0[0], q_phase ^ 1)
                tlx.barrier_expect_bytes(q_scale_fulls_0[0], Q_SCALE_BYTES)
                ...
                tlx.barrier_wait(k_scale_empties_0[0], k_phase ^ 1)
                tlx.barrier_expect_bytes(k_scale_fulls_0[0], K_SCALE_BYTES)
                ...
                # Load Q scale for q1 - use explicit q_scale buffer 1 for group 1
                tlx.barrier_wait(q_scale_empties_1[0], q_phase ^ 1)
                tlx.barrier_expect_bytes(q_scale_fulls_1[0], Q_SCALE_BYTES)
                # 5D TMA offset: [batch_head, m_offset, head_offset, 0, 0]
                ...
                tlx.barrier_wait(v_scale_empties_1[0], v_phase ^ 1)
                tlx.barrier_expect_bytes(v_scale_fulls_1[0], V_SCALE_BYTES)
                # 5D TMA offset: [batch_head, n_offset, head_offset, 0, 0]
                tlx.async_descriptor_load(
                    desc_v_scale,
                    v_scale_tiles_1[0],
...
class _attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, q_scale, k_scale, v_scale, sm_scale, causal, BWD_BLOCK_M1, GROUP_SIZE_M):
        # shape constraints
        ...
        # Create scale descriptors if scales are provided (for MXFP8)
        desc_q_scale = None
        desc_k_scale = None
        desc_v_scale = None
...
def generate_attention_inputs(shape, device, dtype):
    ...
    if dtype == torch.float8_e5m2:
        ...
        k = k_ref.to(dtype)
        v = v_ref.to(dtype)
        return (q, None, q_ref), (k, None, k_ref), (v, None, v_ref)
...
def bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, mode, provider, causal, BWD_BLOCK_M1, GROUP_SIZE_M, device=DEVICE,
    ...
    if "triton" in provider:
        ...
        v = v.requires_grad_()
        sm_scale = 1.3
        fn = lambda: attention(q, k, v, q_scale, k_scale, v_scale, sm_scale, causal, BWD_BLOCK_M1, GROUP_SIZE_M)
        if mode == "bwd":
            o = fn()
