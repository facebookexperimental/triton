...
def compile_kernel_shape(kernel_name, shape, num_cus=None):
    ...
    if kernel_name == "tlx_baseline":
        ...
        run_kernel_optimized_async(a, b, bias, y, out, ASYNC_BEST_CONFIG[K], warmup=True)
...
def run_perf_test(args):
    ...
    for (kernel_name, K), baseline in sorted(PERF_BASELINE_TFLOPS.items()):
        ...
        try:
            got = measure_tflops(get_kernel(kernel_name), M, N, K, warmup=args.warmup, rep=args.rep)
    ...
    for M in args.M:
        for N in args.N:
            for K in args.K:
                ...
                ms = triton.testing.do_bench(pt_lambda, warmup=args.warmup, rep=args.rep)
                ...
                for kernel_name in args.kernel:
                    ...
                    try:
                        ...
                        continue
                    ms = triton.testing.do_bench(k_lambda, warmup=args.warmup, rep=args.rep)
                ...
                ms = triton.testing.do_bench(rb_lambda, warmup=args.warmup, rep=args.rep)
...
def parse_args():
    ...
    p.add_argument("--rep", type=int, default=100, help="timed repetitions; default: 100")
    p.add_argument("--warmup", type=int, default=25, help="warmup iterations; default: 25")
    p.add_argument(
        "--compile-workers",
        type=int,
