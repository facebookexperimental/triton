        ...
        for start_n in tl.range(0, seq_len_kv, BLOCK_N, num_stages=1):
            ...
            for g in range(G):
                ...
                for start_m in tl.range(
                    0, seq_len_q, BLOCK_M, num_stages=1, warp_specialize=AUTOWS
                ):
                    offs_m = start_m + tl.arange(0, BLOCK_M)
...
def _hstu_attn_bwd_inner(  # noqa C901
    ...
    if HAS_CAUSAL:
        ...
        low_q = 0
    # EXPERIMENT (autoWS, depth-2 per-head reduce_dq): warp_specialize the inner
    # Q loop. warp_specialize requires beta triton (-m ovr_config//triton:beta).
    for start_m in tl.range(low_q, seq_len_q, BLOCK_M, warp_specialize=AUTOWS):
        offs_m = start_m + tl.arange(0, BLOCK_M)
        mask_m = offs_m < seq_len_q
        q_offset = seq_start_q + start_m
        q = desc_q.load([q_offset.to(tl.int32), off_h * stride_qh])
...
@triton_hstu_cross_attn_v3_fwd.register_fake
@triton_hstu_cross_attn_v3_fwd.register_kernel("cpu")
def _triton_hstu_cross_attn_v3_fwd_fake(
    max_seq_len: int,
    alpha: float,
    q: torch.Tensor,
    k: torch.Tensor,
...
@triton_hstu_cross_attn_v3_bwd.register_fake
def triton_hstu_cross_attn_v3_bwd_fake(
    dout: torch.Tensor,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    return (
        torch.empty_like(q),
        torch.empty_like(k),
        torch.empty_like(v) if not shared_kv else torch.empty(0),
    )
