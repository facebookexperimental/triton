...
Recommended:
    third_party/tlx/denoise.sh \
        python third_party/tlx/tutorials/testing/test_amd_addmm_gfx942_perf.py
...
from triton.language.extra.tlx.tutorials.amd_addmm_gfx942 import (  # noqa: E402
...
METHODS = {
    "tlx_gfx942": lambda bias, a, b: _amd_addmm_gfx942(bias, a, b),
}
...
def create_benchmark(versions, dtype):
    ...
    @triton.testing.perf_report(
        triton.testing.Benchmark(
            ...
            ylabel="TFLOPS",
            plot_name=f"amd-gfx942-addmm-performance-{dtype_name}",
            args={},
        ))
    def benchmark(M, N, K, provider):
        bias, a, b = make_inputs(M, N, K, dtype)
        if provider == REF:
            ms = measure(lambda: torch.addmm(bias, a, b))
        else:
            fn = METHODS[provider]
            ms = measure(lambda: fn(bias, a, b))
...
def run_speedup_table(versions, dtype):
    ...
    print(f"\n=== amd_addmm_gfx942 vs {REF} ({dtype_name}) ===")
    print(header)
    for M, N, K in SHAPES:
        bias, a, b = make_inputs(M, N, K, dtype)
        ref_ms = measure(lambda: torch.addmm(bias, a, b))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Benchmark the TLX gfx942 fused addmm tutorial against aten")
