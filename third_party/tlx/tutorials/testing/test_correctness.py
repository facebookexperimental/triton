...
from triton.language.extra.tlx.tutorials.amd_kda_decode import (
    kda_recurrent_decode_tlx as _amd_kda_decode,
)
from triton.language.extra.tlx.tutorials.amd_kda_prefill import (
    kda_paged_prefill_tlx as _amd_kda_prefill,
)
...
# =============================================================================
# AMD Kimi Delta Attention Tests (gfx950)
# =============================================================================
...
def _check_amd_kda_prefill(lengths, heads, scale, cu_on_cpu=False):
    ...
    actual_output, actual_state = _amd_kda_prefill(
        q,
        k,
        v,
        g,
...
@pytest.mark.skipif(not is_hip_cdna4(), reason="Requires gfx950 hardware (CDNA4)")
def test_amd_kda_prefill_chunk_boundaries(scale):
    _check_amd_kda_prefill(
...
@pytest.mark.skipif(not is_hip_cdna4(), reason="Requires gfx950 hardware (CDNA4)")
def test_amd_kda_prefill_smoke_shapes(lengths, heads):
    _check_amd_kda_prefill(lengths, heads=heads, scale=128**-0.5)


def _make_amd_kda_decode_inputs(batch, heads, key_dim, value_dim, strided):
...
@pytest.mark.skipif(not is_hip_cdna4(), reason="Requires gfx950 hardware (CDNA4)")
def test_amd_kda_decode_indexed_state(heads, key_dim, value_dim, strided_inputs, scale_mode):
    torch.manual_seed(13)
    batch = 3
    scale = key_dim**-0.5 if scale_mode == "attention" else 1.0
    q, k, v, g, beta = _make_amd_kda_decode_inputs(
        batch, heads, key_dim, value_dim, strided_inputs)
    state_pool = torch.randn(
        7, heads, value_dim, key_dim, device=DEVICE, dtype=torch.float32)
...
@pytest.mark.skipif(not is_hip_cdna4(), reason="Requires gfx950 hardware (CDNA4)")
def test_amd_kda_decode_graph_padding_and_slot_stride():
    ...
    original_pool = state_pool.clone()
    q, k, v, g, beta = _make_amd_kda_decode_inputs(
        batch, heads, key_dim, value_dim, strided=True)
    ...
    cu_seqlens = torch.tensor([0, 1, 2, 2, 2], device=DEVICE, dtype=torch.int64)

    # Warm-up is required before stream capture; writes target slots distinct
    # from the read slots, so replay remains deterministic.
    _amd_kda_decode(
        q,
        k,
        v,
...
@pytest.mark.skipif(not is_hip_cdna4(), reason="Requires gfx950 hardware (CDNA4)")
def test_amd_kda_decode_invalid_indices(scale, invalid_index):
    torch.manual_seed(41)
    batch, heads, key_dim, value_dim = 2, 2, 8, 5
    q, k, v, g, beta = _make_amd_kda_decode_inputs(
        batch, heads, key_dim, value_dim, strided=False)
    ...
    torch.testing.assert_close(
        actual.float(), torch.stack(expected_output).unsqueeze(0), atol=2e-2, rtol=2e-2)
    for untouched in range(pool.shape[0]):
        torch.testing.assert_close(pool[untouched], original_pool[untouched], atol=0, rtol=0)


@pytest.mark.skipif(not is_hip_cdna4(), reason="Requires gfx950 hardware (CDNA4)")
def test_amd_kda_decode_cpu_metadata_and_malformed_rows():
