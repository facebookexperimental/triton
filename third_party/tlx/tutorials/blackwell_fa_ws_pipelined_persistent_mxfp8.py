    ...
    with tlx.async_tasks():
        ...
        with tlx.async_task(num_warps=1, registers=24):
            ...
            for _i in range(tiles_per_sm):
                ...
                tmem_buf_id, tmem_phase = _get_bufidx_phase(blk_idx, NUM_BUFFERS_TMEM)
                tmem_buf_id_prev, tmem_phase_prev = _get_bufidx_phase(blk_idx - 1, NUM_BUFFERS_TMEM)
                persistent_tmem_idx, persistent_tmem_phase = _get_bufidx_phase(_i, NUM_BUFFERS_TMEM)
                ...
                for j in range(1, num_steps):
                    ...
                    tlx.barrier_wait(dp_empties[tmem_buf_id_prev], tmem_phase_prev)
                    # Fence for scale copies.
                    tlx.fence("async_shared")
...
def attention_bwd(
    ...
    def grid(meta):
        return (
            min(NUM_SMS,
                triton.cdiv(N_CTX, meta["BLOCK_N1"]) * Z * H),
            1,
            1,
        )
