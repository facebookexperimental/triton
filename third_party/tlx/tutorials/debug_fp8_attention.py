        ...
        with tlx.async_task(num_warps=1, registers=24):
            ...
            for j in range(0, tiles_per_sm):
                ...
                for slice_id in tl.static_range(0, NUM_MMA_SLICES):
                    ...
                    # v_bufIdx is always 1, use explicit buffer
                    mBarriers_scaled = ([acc_empties[1], kv_empties[v_bufIdx], kv_scale_empties[v_bufIdx]]
                                        if slice_id == NUM_MMA_SLICES - 1 else [])
                    tlx.async_dot_scaled(
                        p_tiles[p_bufIdx],
                        kv_slice,
...
class _attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, q_scale, k_scale, v_scale, sm_scale, causal, GROUP_SIZE_M):
        # shape constraints
...
def main():
    ...
    ref_out = attention(q_ref, k_ref, v_ref, None, None, None, sm_scale, causal, GROUP_SIZE_M)
    # triton implementation
    tri_out = attention(q, k, v, q_scale, k_scale, v_scale, sm_scale, causal, GROUP_SIZE_M).to(dtype)
