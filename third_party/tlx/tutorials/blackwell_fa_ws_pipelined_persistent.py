...
if _fwd_num_buffers_kv is not None:
    configs = [
        config for config in configs
        if config.kwargs["NUM_BUFFERS_KV"] == int(_fwd_num_buffers_kv)
    ]
...
if _fwd_rescale_opt is not None:
    configs = [
        config for config in configs
        if config.kwargs["RESCALE_OPT"] == bool(int(_fwd_rescale_opt))
    ]
...
if _fwd_use_where is not None:
    configs = [
        config for config in configs
        if config.kwargs["USE_WHERE"] == bool(int(_fwd_use_where))
    ]
