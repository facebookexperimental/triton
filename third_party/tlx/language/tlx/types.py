...
class amd_wmma_layout_encoding(layout_encoding):
    """
    AMD WMMA register layout.

    This mirrors Gluon's AMDWMMALayout closely enough for TLX kernels to make
    the WMMA repetition and warp tiling explicit at the call site.
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.version = int(tl._unwrap_if_constexpr(version))
        self.transposed = bool(tl._unwrap_if_constexpr(transposed))
        self.warp_bases = [list(inner) for inner in tl._unwrap_if_constexpr(warp_bases)]
        reg_bases = [] if reg_bases is None else tl._unwrap_if_constexpr(reg_bases)
        self.reg_bases = [list(inner) for inner in reg_bases]
        instr_shape = [16, 16, 16] if instr_shape is None else tl._unwrap_if_constexpr(instr_shape)
        self.instr_shape = [int(x) for x in instr_shape]
        cga_layout = [] if cga_layout is None else tl._unwrap_if_constexpr(cga_layout)
        self.cga_layout = [list(inner) for inner in cga_layout]
        self.rank = int(tl._unwrap_if_constexpr(rank)) if rank is not None else 2

    def with_instr_shape(self, instr_shape):
        return amd_wmma_layout_encoding(
            self.version,
            self.transposed,
            self.warp_bases,
            self.reg_bases,
    ...
    def tiles_per_warp(self):
        tiles = [1] * self.rank
        for base in self.reg_bases:
            for dim, value in enumerate(base):
                if value != 0:
                    tiles[dim] *= 2
        return tiles

    def to_ir(self, builder: ir.builder):
        return builder.make_amd_wmma_encoding_attr(
            self.version,
            self.transposed,
            self.warp_bases,
            self.reg_bases,
    ...
    def __repr__(self):
        return (f"amd_wmma_layout_encoding<{self.version}, transposed={self.transposed}, "
                f"warp_bases={self.warp_bases}, reg_bases={self.reg_bases}, "
                f"instr_shape={self.instr_shape}, cga_layout={self.cga_layout}>")

    def __eq__(self, other):
        return (isinstance(other, amd_wmma_layout_encoding) and self.version == other.version
                and self.transposed == other.transposed and self.warp_bases == other.warp_bases
                and self.reg_bases == other.reg_bases and self.instr_shape == other.instr_shape
                and self.cga_layout == other.cga_layout and self.rank == other.rank)

    def __hash__(self):
        return hash((self.version, self.transposed, tuple(tuple(x) for x in self.warp_bases),
                     tuple(tuple(x) for x in self.reg_bases), tuple(self.instr_shape),
                     tuple(tuple(x) for x in self.cga_layout), self.rank))
