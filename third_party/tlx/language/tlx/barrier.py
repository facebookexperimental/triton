...
def sched_barrier(mask: tl.constexpr = 0, _semantic=None) -> None:
    """AMD instruction-scheduling fence (no memory/exec barrier). mask 0 = hard
    boundary (no instruction class crosses). Lowers to rocdl.sched.barrier."""
    if not is_hip():
        raise NotImplementedError("tlx.sched_barrier is AMD (HIP) only")
    mask_val = mask.value if isinstance(mask, tl.constexpr) else mask
    _semantic.builder.create_sched_barrier(mask_val)
...
def sched_region_begin(_semantic=None) -> None:
    ...
    _semantic.builder.create_sched_barrier(_LLIR_SCHED_REGION_BEGIN_MASK)
...
def sched_region_end(_semantic=None) -> None:
    ...
    _semantic.builder.create_sched_barrier(_LLIR_SCHED_REGION_END_MASK)
