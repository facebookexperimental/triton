...
## 2026-07-26: case11 stage fallback removed; lowering-plan shadow contract

Case11 exposed a deterministic contract mismatch hidden behind solver
nondeterminism. The v2 model could move nodes between stages even though the
C++ consumer rejected every stage-changing response. Equal-primary solver
draws therefore alternated between an accepted 6-WG schedule and a v1 fallback
that selected the slower 7-WG schedule. The fixed-II joint model now enforces
the existing emitter contract directly: each node's solved stage equals its
committed input stage. The C++ check remains a defense-in-depth verifier.

The first lowering-plan contract is also in tree. `joint-solver-0.2` carries
symbolic event templates, accounts for active event issue slots in chip and WG
resources, and uses a proven-primary lexicographic pass to minimize blocking
wait inversions. The schedule graph now dumps the self-contained templates and
concrete plan, and sched2tlx parses and validates them.

This milestone is deliberately shadow-only. v2 plans are marked
`shadow_verified` only when they still match final ownership after infra
propagation and cross-loop reconciliation; v1 plans are `shadow_unmodeled`, and
drift becomes `shadow_stale`. Emission still derives synchronization from
`cross_wg_barriers`. See [LoweringPlan.md](LoweringPlan.md) for the schema,
coverage, and promotion gates. The remaining root task is to make the shared
planner generate final channels/events and then feed that exact model into the
fixed-II feasibility sweep, rather than treating lowering as a secondary cost.

The first stability sweep showed that fixing stages alone was insufficient:
with 8-worker CP-SAT, 2/100 direct runs still fell back to v1 because the
initial schedule face itself was nondeterministic. The fixed-II schedule solve
and v2 primary solve now both use one worker and seed 0; the secondary already
did so. After that change, 100/100 direct Joint pass runs produced `II=825`, 6
warp groups, and `shadow_verified`, with no `shadow_unmodeled` v1 fallback.

Follow-up review closed three proof gaps in that milestone. Event occupancy now
scales by `frequencyMultiplier`; synchronization events reserve only their
owner WG's issue stream instead of inventing a TC/CUDA chip-pipeline conflict;
and every solve is single-worker with seed 0 under a deterministic-time budget.
Channel capacity is deduplicated by `(src, dst, srcResultIdx)`, and v2
recomputes cycle-dependent data/barrier depths, re-merges physical buffers, and
checks SMEM/TMEM before commit. A second audit runs after infra propagation and
real channel materialization, deduplicating repeated graph views by loop
identity and charging the private allocations sched2tlx uses when alias reuse
is unsafe. It also reserves SemIR's full/empty barrier arrays conservatively,
including intra-WG async consumers that have no cross-WG record. Explicit
A.5/A.7 ring-depth floors survive cycle refinement. The generic II sweep also
stops at the first `UNKNOWN` candidate instead of
accepting a higher II without a minimality proof, and v2 arbitration preserves
`UNKNOWN` separately from proven `INFEASIBLE` while sharing one deterministic
budget across its primary, secondary, v1, and probe solves.
