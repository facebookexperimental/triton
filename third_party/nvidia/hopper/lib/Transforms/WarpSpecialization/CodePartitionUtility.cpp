...
Operation *getProtocolSameLevelOp(Operation *p, Operation *c) {
  Operation *op = c;
  while (!isa<triton::FuncOp>(op)) {
    if (getEffectiveProtocolParent(op) == getEffectiveProtocolParent(p)) {
      while (auto subtiled = op->getParentOfType<ttng::SubtiledRegionOp>()) {
        if (subtiled->getParentOp() == getEffectiveProtocolParent(p))
          op = subtiled;
        else
          break;
      }
  ...
  op = p;
  while (!isa<triton::FuncOp>(op)) {
    if (getEffectiveProtocolParent(c) == getEffectiveProtocolParent(op)) {
