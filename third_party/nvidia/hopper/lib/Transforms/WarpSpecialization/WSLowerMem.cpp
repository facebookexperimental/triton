...
getTMAWritableEncoding(tt::DescriptorLoadLikeOpInterface load,
...
convertDescriptorLoadLikeToNVWS(tt::DescriptorLoadLikeOpInterface descOp) {
  ...
  // Remove only dead layout-rematerialization chains before deriving the
  // replacement local_load's consumer task IDs. Whole-function DCE would also
  // erase intentionally dead consumers in test/debug IR.
  eraseDeadScheduleRematerializations(result);
  if (result.use_empty()) {
    descOp->erase();
    return success();
  }
  ...
  if (soleAlloc) {
    if (auto encoding = getTMAWritableEncoding(
            descOp, tensorType, cast<ttg::MemDescType>(soleAlloc.getType()),
            descEnc)) {
      fusedEncoding = *encoding;
    } else {
      // Keep the consumer allocation when its layout requires a separate
      // TMA-compatible landing buffer.
      soleAlloc = nullptr;
    }
  }
