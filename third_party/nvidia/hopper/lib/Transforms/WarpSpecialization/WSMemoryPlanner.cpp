...
static void allocateTMem(Operation *parentOp, SmallVector<Channel *> &channels,
  ...
  for (auto it = allocs.begin(), e = allocs.end(); it != e; ++it) {
    ...
    if (TheCh->isOperandD) {
      ...
      LDBG("update tmem liveness: " << updatedInterval.start() << " "
                                   << updatedInterval.end());
  ...
  sort(allocs, [&](ttng::TMEMAllocOp a, ttng::TMEMAllocOp b) {
    ...
    // When size is the same, sort by isOperandD etc.
  ...
  for (auto it = allocs.begin(), e = allocs.end(); it != e; ++it) {
    ...
    if (allocToChannel[alloc.getOperation()]->isOperandD ||
      ...
      if (firstRun) {
        ...
        alloc->setAttr("buffer.offset",
                       IntegerAttr::get(
                           IntegerType::get(alloc->getContext(), 32),
