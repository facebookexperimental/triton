...
public:
  ...
  LogicalResult allocateBuffers(unsigned bufferId) {
    ...
    for (auto *ctrlOp : innermostLoops) {
      ...
      auto result = allocateTMemAllocs2(
    ...
    if (!lastAllocs.empty()) {
      auto result = allocateTMemAllocs2(
          lastAllocs, buffers, // allocToIntervals,
          /*allocToSize,*/ allocToChannel, operationId, nullptr, bufferId);
      if (failed(result))
        return failure();
      bufferId = *result;
    }
...
LogicalResult doMemoryPlanner(triton::FuncOp &funcOp, unsigned numBuffers,
  ...
  // ops with smaller cluster IDs appear before ops with larger cluster IDs.
  // TODO: Disabled - reorderOpsBySchedule has a bug that causes
  // "'math.exp2' op operation destroyed but still has uses" because it doesn't
  // check if moving an op would break uses of its results.
