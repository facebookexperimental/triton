...
findReuseCandidate(WSBuffer &candidate, SmallVector<WSBuffer> &wsBuffers,
  ...
  LDBG("findReuseCandidate: candidate bufferId="
       << candidate.bufferId << " \"" << getLocName(candidate.allocOp)
  ...
  for (auto &buf : wsBuffers) {
    ...
    if (!areReuseEncodingsCompatible(candidate, buf)) {
      LDBG("  findReuseCandidate: target bufferId="
           << buf.bufferId << " \"" << getLocName(buf.allocOp)
  ...
  if (best) {
    LDBG("  findReuseCandidate: best target bufferId="
         << best->bufferId << " \"" << getLocName(best->allocOp)
         << "\" order=" << bestOrder.linearOrder << " (candidate \""
         << getLocName(candidate.allocOp) << "\" reuses it)");
...
public:
  LogicalResult run(unsigned bufferId) override {
    ...
    for (auto it = allocs.begin(), e = allocs.end(); it != e; ++it) {
      ...
      LLVM_DEBUG(alloc.dump());
      LDBG("TMEM buffer \"" << getLocName(alloc.getOperation())
                            << "\" cols=" << allocSize.numCols
                            << " rows=" << allocSize.numRows << " liveness=["
                            << liveInterval.start() << "," << liveInterval.end()
                            << "]");
    ...
    if (!tmemAnnotations.empty()) {
      ...
      for (auto &[bid, group] : annotatedGroups) {
        ...
        LDBG("TMEM pre-assign: owner alloc \""
             << getLocName(ownerAlloc.getOperation()) << "\" buffer.id=" << bid
        ...
        for (size_t i = 1; i < group.size(); ++i) {
          ...
          LDBG("TMEM pre-assign: reuser \""
               << getLocName(reuserAlloc.getOperation()) << "\" buffer.id="
               << bid << " reuses owner \""
               << getLocName(ownerAlloc.getOperation()) << "\" colOffset="
               << nextColOffset << " size=" << reuserBuf->rowSize << "x"
               << reuserBuf->colSize);
    ...
    for (auto &[owner, priority] : candidates) {
      ...
      LLVM_DEBUG({
        LDBG("tryAllocate: trying reuse \""
             << getLocName(allocs[idx].getOperation()) << "\" ["
...
// names, and (B) each buffer's producer-partition -> consumer-partition(s) edge.
// Shown under -debug-only=nvgpu-ws-memory-planner.
static void dumpPartitionAndBufferSummary(triton::FuncOp funcOp,
  ...
  // Stable per-op id (program order) so Section A and Section B cross-reference:
  // an op#N in A is the same op referenced as srcOp/dstOp op#N in B.
  ...
  funcOp.walk([&](scf::ForOp forOp) {
    ...
    LDBG("[ws-summary] " << opRef(forOp.getOperation()) << " scf.for depth="
                         << depth << " parent="
                         << forRef(forOp.getOperation()) << " tasks={" << tasks
                         << "}");
  });
  ...
  funcOp.walk([&](Operation *op) {
    ...
    bool key = isa<ttng::MMAv5OpInterface, ttng::TMEMAllocOp,
                   ttng::TMEMLoadOp, ttng::TMEMStoreOp, ttg::LocalAllocOp,
                   ttg::LocalStoreOp, ttg::LocalLoadOp, ttg::MemDescTransOp>(
                   op) ||
  ...
  for (Channel *ch : channels) {
    ...
    if (auto sAlloc = dyn_cast_or_null<ttg::LocalAllocOp>(alloc)) {
      ...
      sizeStr = " cols=" + std::to_string(sz.numCols) + " rows=" +
                std::to_string(sz.numRows);
    ...
    std::string src = srcOp ? (opRef(srcOp) + " " +
                               srcOp->getName().getStringRef().str())
                            : std::string("op#? ?");
    std::string dst = dstOp ? (opRef(dstOp) + " " +
                               dstOp->getName().getStringRef().str())
                            : std::string("op#? ?");
