...
| `../../Materialize2CTAExchange.cpp` | `nvgpu-materialize-2cta-exchange` | Lowers planned peer gathers after pipelining; BM64 completes on the existing AutoWS full barrier, while experimental BM128 relay mode separates peer completion from dQ MMA publication |
| [AutoWS2CTARelayTriage.md](AutoWS2CTARelayTriage.md) | relay implementation status | Records the BM128 one-warp relay design, synchronization invariants, current validation status, pass ownership, and required production-derived fixtures |
