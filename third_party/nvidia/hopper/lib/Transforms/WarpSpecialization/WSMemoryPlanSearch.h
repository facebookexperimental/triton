...
/// candidate is bidirectionally data-dependent with them (BufferModel::dependsOn
/// either way) -- mirroring hasPotentialReuse's proven predicate. The dependency
/// is load-bearing: two liveness-disjoint but *independent* buffers can live in
/// different warp partitions and be concurrent at run time despite disjoint
/// op-id intervals, so encoding + liveness alone is unsafe. Footprint is
/// `max(member rows) x max(member cols)`; feasibility is total rows <= 512 and
/// per-owner cols <= 512. Copies stay 1 (non-accumulator TMEM multi-copy is not
/// yet legal — the caller pins them). Holds a reference to `model`, which must
/// outlive it.
