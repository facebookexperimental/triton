...
static PyObject *launchKernel(PyObject *self, PyObject *args) {
...

  {
    ...
    if (grid_status != CUDA_SUCCESS) {
      uint32_t required[3];
      const unsigned long long physical[3] = {
          (unsigned long long)gridX * (unsigned long long)num_ctas,
          (unsigned long long)gridY, (unsigned long long)gridZ};
      ...
      PyErr_Format(PyExc_ValueError,
                   ...
                   "(%llu, %llu, %llu) is not divisible by required cluster "
                   "shape (%u, %u, %u)",
                   physical[0], physical[1], physical[2], required[0],
...
static PyObject *TritonDispatcher_vectorcall(PyObject *callable,
  ...
  if (grid_status != CUDA_SUCCESS) {
    uint32_t required[3];
    const unsigned long long physical[3] = {
        (unsigned long long)gx * self->grid_mult, (unsigned long long)gy,
...
static PyObject *ProxyRunner_vectorcall(PyObject *callable,
  ...
  const uint32_t grid[3] = {self->grid[0], self->grid[1], self->grid[2]};
  CUresult grid_status = triton_validate_exact_cluster_grid(
      grid, (int)d->grid_mult, d->cluster_dims);
  if (grid_status != CUDA_SUCCESS) {
    uint32_t required[3];
    const unsigned long long physical[3] = {
        (unsigned long long)self->grid[0] * d->grid_mult,
        (unsigned long long)self->grid[1],
        (unsigned long long)self->grid[2]};
    triton_get_exact_cluster_shape((int)d->grid_mult, d->cluster_dims,
                                   required);
    PyErr_Format(PyExc_ValueError,
                 "Triton cluster requirement failed: physical grid "
                 "(%llu, %llu, %llu) is not divisible by required cluster "
