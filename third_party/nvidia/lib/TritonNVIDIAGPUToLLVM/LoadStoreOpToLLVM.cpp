...
static LinearLayout getMsgToPackedOffsetLayout(ttg::MemDescType ty,
  ...
  auto cgaBlockBases = cgaLayout.getLinearLayout().getBases().lookup(kBlock);
  auto splitBlockBases = msgToOffset.getBases().lookup(kBlock);
  if (cgaBlockBases.size() != splitBlockBases.size()) {
    auto bases = msgToOffset.getBases();
    std::vector<std::vector<int32_t>> expandedBlockBases;
    unsigned splitBit = 0;
    for (const auto &cgaBasis : cgaBlockBases) {
      if (llvm::any_of(cgaBasis,
                       [](int32_t component) { return component != 0; })) {
        expandedBlockBases.push_back(splitBlockBases.at(splitBit++));
      } else {
        expandedBlockBases.emplace_back(msgToOffset.getNumOutDims(), 0);
      }
    }
    assert(splitBit == splitBlockBases.size());
    bases[kBlock] = std::move(expandedBlockBases);
    msgToOffset = LinearLayout(std::move(bases), msgToOffset.getOutDims(),
                               /*requireSurjective=*/false);
  }
  return msgToOffset;
}
...
struct AsyncTMACopyGlobalToLocalOpConversion
  ...
  matchAndRewrite(triton::nvidia_gpu::AsyncTMACopyGlobalToLocalOp op,
    ...
    if (affineBlockMask || op.getTwoCta())
    ...
    for (int copyIdx = 0; copyIdx < numCopies; copyIdx += numWarps) {
      ...
      Value multicastMask = op.getMulticastTargets();
      ...
      bool clusterScope = computeCapability < 100 || crossCTABarrier ||
                          affineBlockMask || multicast ||
                          multicastMask != nullptr || op.getTwoCta();
      std::string tmaInst =
          "@$0 cp.async.bulk.tensor." + std::to_string(rank) + "d." + ctaGroup +
          "shared::" + (clusterScope ? "cluster" : "cta") + ".global" +
          (isIm2Col ? ".im2col" : "") + ".mbarrier::complete_tx::bytes";
...
static LogicalResult iterateGatherScatterIndices(
  ...
  unsigned offsetBlockBits = msgToOffset.getInDimSizeLog2(kBlock);
  for (auto [bit, basis] : llvm::enumerate(msgBases[kBlock]))
    basis.back() =
        bit < offsetBlockBits ? msgToOffset.getBasis(kBlock, bit, kDim1) : 0;
