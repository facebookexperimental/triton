...
struct AsyncTMACopyGlobalToLocalOpConversion
  ...
  matchAndRewrite(triton::nvidia_gpu::AsyncTMACopyGlobalToLocalOp op,
    ...
    for (int copyIdx = 0; copyIdx < numCopies; copyIdx += numWarps) {
      ...
      auto multicastTargets = op.getMulticastTargets();
      ...
      bool clusterScope = computeCapability < 100 || crossCTABarrier ||
                          multicast || multicastTargets != nullptr ||
      ...
      if (op.getTwoCta() && !affineBlockMask)
        tmaInst += ".cta_group::2";
      if (multicast || multicastTargets != nullptr)
      ...
      if (multicast || multicastTargets != nullptr) {
        Value multicastMask = multicastTargets;
        if (!multicastMask)
