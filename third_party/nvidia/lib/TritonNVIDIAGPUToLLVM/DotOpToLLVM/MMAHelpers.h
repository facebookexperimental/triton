...
public:
  ...
  build(Location loc, RewriterBase &rewriter, gpu::MemDescType memTy,
    ...
    if (auto encoding = dyn_cast<triton::gpu::NVMMASharedEncodingAttr>(
      ...
      // Compute the pseudoinverse layout for coordinate routing in smemLoad().
      // For NPOT contiguous dims with swizzle, use the split-dim layout
      // (3 output dims: dim0, contig_intra, contig_phase) where each dim uses a
      // single algebra, giving a correct pseudoinverse.
      // For pow2 or NPOT non-contiguous dims, the standard 2-dim layout works.
      ...
      if (isFp4) {
        // For fp4 (E2M1), the memdesc stores packed i8 (2 fp4 per byte).
        // Prepend identity1D(2) to double the K-dim resolution and halve
        // bitwidth for correct byte offset computation.
    ...
    // Fallback for non-NVMMAShared encodings: use the brute-force
    // getDescriptor search via the inner build() overload.
  ...
  Value smemLoad(int a, int b, ConversionPatternRewriter &rewriter,
    ...
    // For NPOT paths (buildFromLL), the pseudoinverse may have only
    // the "offset" out dim (block dim stripped). For the standard path,
    // it has both "offset" and "block".
...
private:
    ...
    if (mmaVersion == 3 && mmaTy.has_value()) {
      ...
      if (llInvInDims.size() == 3) {
        ...
        if (mmaWarps.getTotalOutDimSizeProduct() == newProduct) {
          ...
          // NPOT-SM90: mmaWarps has pow2 K-dim (e.g., 64) but llInv has
          // NPOT split K-dims (e.g., 16×3=48). Warps only advance in the
          // MN dim — the K component of mmaWarps is always 0 (identity of
          // size 1 at line 336). Reconstruct mmaWarps with llInv's dim
          // sizes: pad each basis vector with zeros for the new split dims
          // (warps contribute nothing to the K-split dimensions).
  ...
  //
  // Coupling with smemLoad's pseudoinverse: this function and the split-dim
  // pseudoinverse (from nvmmaSharedToSplitLinearLayout) are computed
  // independently -- both derive strides from the same NVMMASharedEncodingAttr
  // and allocation shape. There is no effectiveRowStride variable or direct
  // dependency on the split-dim layout here; the ISA formulas use tile
  // geometry and contigDimSize from the encoding/allocShape directly.
  static FailureOr<MMASMEMDescriptor>
