...
static void createBarrier(TritonLLVMIRRewriter &b, unsigned barIdx,
  ...
  static LogicalResult findTrivialSubcomputation(
      LLVM::LLVMFuncOp func, Value capture, SetVector<Operation *> & ops) {
  ...
  static LogicalResult rewriteWarpGroupBarriers(
      ...
      unsigned threadsPerWarp, unsigned defaultWarpGroupSize) {
  ...
  // LLVM's LICM will be tempted to hoist code out of the switch loop generated
  // by the `ttg.warp_specialize` lowering. However, neither NVPTX or `ptxas`
  // will rematerialize this code back in to the partition regions, resulting in
  // long liveranges for an arbitrary number of registers.
  //
  // Due to reduced warp group registers, these live values can induce spilling
  // in the partition regions. Prevent this by disabling LICM on the switch
  // loop.
  ...
  static LogicalResult lowerWarpSpecialize(
    ...
    if (tlx::tlxIsClustered(func) && !tlx::tlxExplicitClusterSync(func)) {
      ...
      bool hasClusterBarWait = func.walk([&](NVVM::ClusterWaitOp) {
    ...
    for (auto [ws, stateMap] : llvm::zip(wsOps, warpToState)) {
      ...
      // ensures they have read the captures before the memory is released upon
      // entry.
      createAllBarrier(b, kSwitchLoopBarrierIdx);
      if (auto actRegs = ws.getActualRegisters())
