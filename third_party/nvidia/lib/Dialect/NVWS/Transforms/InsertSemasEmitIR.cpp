...
static Value emitMixedCopyLocalView(OpBuilder &b, Location loc, Value owner,
                                    const GroupDag &g, const Member &member) {
  ...
  Value logical = gpu::MemDescReinterpretOp::create(b, loc, logicalType, owner);
  Value slot = arith::ConstantIntOp::create(b, loc, member.circularStart, 32);
  ...
  for (auto [i, member] : llvm::enumerate(members)) {
    ...
    for (unsigned j = 0; j < members.size(); ++j)
      if (j != i && members[j].backingPrimary == i && !g.isTmem() &&
        ...
        backing[j] =
            members[j].copies == g.numCopies
                ? gpu::MemDescReinterpretOp::create(
                      b, loc, backingType(g, members[j]), backing[i])
                : emitMixedCopyLocalView(b, loc, backing[i], g, members[j]);
    for (int j = members.size() - 1; j >= 0; --j)
      if (j != i && members[j].backingPrimary == i) {
        if (!g.isTmem() && members[j].allocOp->hasAttr(kBufferStartAttrName)) {
          continue;
        }
...
static void materializeTokenlessMembers(EmitCtx &ctx,
  ...
  for (const GroupDag *group : groups) {
    ...
    for (auto [index, member] : llvm::enumerate(group->pieceTable.members)) {
      ...
      Value zero =
          arith::ConstantIntOp::create(b, member.allocOp->getLoc(), 0, 32);
      auto view = gpu::MemDescIndexOp::create(
          b, member.allocOp->getLoc(), member.type, backing[index], zero);
      for (StringRef name :
           {"async_task_id", gpu::kPartitionAttrName,
            gpu::kWarpSpecializeTagAttrName, "loop.stage", "loop.cluster"})
        if (Attribute attr = member.allocOp->getAttr(name)) {
    ...
    for (auto indexed : llvm::enumerate(group.semas)) {
      ...
      if (conflict)
        return semaError(group.root->op)
               << "stage " << llvm::countr_zero(conflict) << " of semaphore "
               << indexed.value().name
...
private:
  ...
  void record(Seen &seen, const Node *node, int64_t cursor, bool acquire) {
  ...
  States replayIf(Node *ifNode, States inputs) {
    ...
    States other = replayChain(ifNode->children.back(), std::move(inputs));
  ...
  States replayChain(Node *node, States states) {
    for (; node; node = node->next) {
      ...
      case Node::Release:
        states =
            replayEvent(node, std::move(states), node->kind == Node::Acquire);
        break;
      case Node::For:
...
LogicalResult
planFreshHandoffProtocol(const GroupDag &group,
                         const DenseMap<const Node *, bool> &freshWriteAcquires,
                         DenseMap<const Node *, int64_t> &stageOffsets,
                         SmallVectorImpl<uint32_t> &releasedMasks) {
...
static LogicalResult
finalizeFreshHandoffProtocols(EmitCtx &ctx, ArrayRef<const GroupDag *> groups) {
  for (const GroupDag *group : groups) {
    ...
    for (auto [node, offset] : stageOffsets) {
      auto stageOp =
          dyn_cast_or_null<nvws::SemaphoreStageInterface>(ctx.op(node));
      if (!stageOp)
        return semaError(ctx.func)
    ...
    for (auto [sid, mask] : llvm::enumerate(releasedMasks)) {
      auto create =
          ctx.semaphore(*group, sid).getDefiningOp<nvws::SemaphoreCreateOp>();
      if (!create)
        return semaError(ctx.func)
      ...
      if (++slotsPerBacking[backing] > 1)
        return semaError(forOp)
               << "two token slots for one semaphore group in a single loop "
                  "cannot be threaded by AssignSemaphoreStagePhase";
    }
    return success();
  };
  for (Operation *op : operations) {
