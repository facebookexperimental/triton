// ACCESS-DAG and boundary-owner analysis for semaphore insertion.
...
FailureOr<SmallVector<GroupDag, 0>> collectGroups(triton::FuncOp funcOp,
                                                  Block *functionBlock) {
  auto allocations = nvws::buffer::collectGroups(funcOp, functionBlock);
  if (failed(allocations))
    return failure();

  SmallVector<GroupDag, 0> groups;
  for (const nvws::buffer::Group &allocationGroup : *allocations) {
...
static LogicalResult collectTouches(GroupDag &g, Operation *op,
  ...
  if (auto localAlloc = dyn_cast<gpu::LocalAllocOp>(op)) {
    if (Value src = localAlloc.getSrc();
      ...
      if (isa_and_nonnull<triton::DescriptorLoadOp, triton::DescriptorGatherOp>(
              src.getDefiningOp()))
  ...
  if (auto x = dyn_cast<nvidia_gpu::TMEMLoadOp>(op))
    ...
    read = x.getSrc();
  else if (auto x = dyn_cast<nvidia_gpu::AsyncTMACopyLocalToGlobalOp>(op))
    read = x.getSrc();
...
static LogicalResult deriveTMACompletionAnchor(Node *access, Value token) {
  ...
  auto wait =
      dyn_cast<nvidia_gpu::TMAStoreTokenWaitOp>(*token.getUsers().begin());
  if (!wait)
    return semaError(tma) << "managed TMA store token user must be "
...
static LogicalResult deriveCompletionAnchor(Node *access) {
  if (auto copy = dyn_cast<nvidia_gpu::AsyncTMACopyLocalToGlobalOp>(access->op))
