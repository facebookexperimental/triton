...
_AMDGPU_REGISTER_OPERAND_RE = re.compile(
    r"([va])(?:\[(\d+):(\d+)\]|(\d+))"
)
...
def _accvgpr_referenced_registers(instruction):
    ...
    return {
        register
        for register in _amdgcn_registers(operands)
        if register[0] == "a"
    }
        ...
        for line in block:
            ...
            if is_conditional_branch or is_unconditional_branch:
                for token in re.findall(r"[.$A-Za-z_][\w.$]*", operands):
                    if token in labels:
                        block_successors.add(labels[token])
...
def _repair_scheduled_mfma_block(lines, incoming_results):
    ...
    for line in lines:
        if _SCHEDULED_MFMA_MARKER in line:
            if pending_marker:
                raise ValueError(
                    "consecutive scheduled MFMA markers in AMDGCN assembly"
                )
            pending_marker = True
            output.append(line)
            continue
        ...
        if pending_marker and instruction is not None:
            ...
            if opcode not in _GFX950_SCHEDULED_MFMA_OPCODES:
                raise ValueError(
                    "scheduled MFMA marker is followed by an unsupported "
            ...
            if (
                not marked_destination
                or any(kind != "a" for kind, _ in marked_destination)
                or not source_a_registers
            ...
            if mfma_operands[3] != "0":
                accumulator_registers = _amdgcn_register_operand(
                    mfma_operands[3]
                )
                if (
                    not accumulator_registers
                    or accumulator_registers != marked_destination
                    or any(kind != "a" for kind, _ in accumulator_registers)
                ):
                    raise ValueError(
                        "cannot parse marked MFMA accumulator registers"
                    )

        if instruction is not None and outstanding_results:
            if marked_destination is None:
                referenced_registers = _accvgpr_referenced_registers(
                    instruction
                )
            else:
            ...
            early_references = referenced_registers.intersection(
                outstanding_results
            )
            if early_references:
                residual_wait_states = max(
                    outstanding_results[register]
                    for register in early_references
                )
                ...
                _advance_outstanding_mfma_results(
                    outstanding_results, residual_wait_states
                )
        ...
        if marked_destination is not None:
            source_wait_states = _wait_states_since(
                history,
                2,
                lambda previous: _legacy_valu_may_write(
                    previous, source_registers
                ),
            )
            source_agpr_wait_states = _wait_states_since(
                history,
                3,
                lambda previous: _accvgpr_write_may_write(
                    previous, source_registers
                ),
            )
            accumulator_agpr_wait_states = (
                _wait_states_since(
                    ...
                    1,
                    lambda previous: _accvgpr_write_may_write(
                        previous, accumulator_registers
                    ),
                )
        ...
        if instruction is not None:
            _advance_outstanding_mfma_results(
                outstanding_results, _amdgcn_wait_states(instruction)
            )
            if marked_destination is not None:
                result_wait_states = _GFX950_MFMA_RESULT_WAIT_STATES[
                    instruction[0]
                ]
    ...
    return output, outstanding_results


def _insert_scheduled_mfma_hazard_nops(amdgcn, arch):
    """Repair marked gfx950 MFMA hazards after scheduling and register allocation."""
    if _SCHEDULED_MFMA_MARKER not in amdgcn:
        return amdgcn
    if arch != "gfx950":
        raise ValueError(
            f"scheduled MFMA post-RA hazard repair is unsupported on {arch}"
        )
    ...
    while worklist:
        ...
        _, outgoing_results = _repair_scheduled_mfma_block(
            blocks[block_index], incoming_results[block_index]
        )
        for successor in successors[block_index]:
            changed = False
            successor_results = incoming_results[successor]
            for register, wait_states in outgoing_results.items():
    ...
    for block, block_results in zip(blocks, incoming_results):
        repaired, _ = _repair_scheduled_mfma_block(block, block_results)
...
class HIPBackend(BaseBackend):
    ...
    def make_amdgcn(src, metadata, options):
        ...
        amdgcn = _insert_scheduled_mfma_hazard_nops(amdgcn, options.arch)
