...
// Given a shared-memory offset layout, derive the register-tensor layout whose
// direct-to-LDS write into it is coalesced: each lane writes `loadContig`
// consecutive LDS elements. Fails if the shape is too small or that
// contiguity is unreachable. Shared by the async-copy coalescer and
// buffer_load_to_local offset-layout inference.
FailureOr<triton::LinearLayout> deduceRegLayoutFromShared(
