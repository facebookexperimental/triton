...
// relocated to a neutral path (see claude/amd_modulo_scheduling_plan.md).
...
static void lowerLoadToStagedCopy(scf::ForOp forOp, triton::LoadOp ld,
  ...
  if (auto dotOpEnc = loadDotOperandEnc(ld)) {
    ...
    sharedEnc = composePaddedLayout(targetInfo, dotOpEnc, srcTOM, sharedOrder,
                                    /*useAsyncCopy=*/true);
    if (!sharedEnc)
...
// scheduler and the runtime test gate for AMDLatencyModel. See
// claude/amd_modulo_scheduling_plan.md.
