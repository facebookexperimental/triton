...
# ===----------------------------------------------------------------------===//
    ...
    def _load_block_pointer(self, ptr, mask, other, boundary_check, padding, cache, eviction, is_volatile, latency):
    ...
    def _load_legacy(self, ptr, mask, other, boundary_check, padding, cache, eviction, is_volatile, latency):
        ...
        # Build IR
        if mask is None:
            ret = self.tensor(self.builder.create_load(ptr.handle, cache, eviction, is_volatile, latency), dst_ty)
        else:
            ret = self.tensor(
                self.builder.create_masked_load(ptr.handle, mask.handle, other.handle if other else None, cache,
                                                eviction, is_volatile, latency), dst_ty)
        if is_bool:
    ...
    def load(self, ptr: TensorTy, mask: Optional[TensorTy], other: Optional[TensorTy], boundary_check: Tuple,
        ...
        if ptr.type.is_ptr() and ptr.type.element_ty.is_block():
            ...
            # Load by a tensor of pointers or a pointer of scalar: `block_type<pointer_type<>>` or `pointer_type<>`
            return self._load_legacy(ptr, mask, other, boundary_check, padding, cache, eviction, is_volatile, latency)
