...
_TRUTHY_ENV_VALUES = frozenset(("1", "on", "true"))


def env_flag_enabled(name: str, default: bool) -> bool:
    value = os.environ.get(name)
    return default if value is None else value.lower() in _TRUTHY_ENV_VALUES
...
def _solver_deterministic_time(solver):
    return float(solver.ResponseProto().deterministic_time)
...
def _async_flight_blocking():
    ...
    return env_flag_enabled("TRITON_MODULO_ASYNC_FLIGHT_BLOCKING", False)
...
def _recmii_rerank_enabled():
    ...
    return env_flag_enabled("TRITON_MODULO_RECMII_RERANK", True)


_JOINT_SCHEMA_V1 = "joint-solver-0.1"
_JOINT_SCHEMA_V2 = "joint-solver-0.2"
_LOWERING_PLAN_VERSION = "lowering-plan-0.1"
_LOWERING_EVENT_KINDS = {
    "wait",
    "arrive",
    "expect",
    "local_store",
...
def _validate_problem_schema(prob):
    mode = prob.get("mode")
    expected = _JOINT_SCHEMA_V2 if mode in ("partition", "joint") else _JOINT_SCHEMA_V1
    if prob.get("version") != expected:
        raise ValueError(f"expected schema {expected}, got {prob.get('version')!r}")

    node_ids = {int(nd["id"]) for nd in prob.get("nodes", [])}
    cluster_ids = {int(c["id"]) for c in prob.get("clusters", [])}
    cluster_by_node = {}
    for cluster in prob.get("clusters", []):
        cluster_id = int(cluster["id"])
        for node_id_value in cluster.get("nodes", []):
            node_id = int(node_id_value)
            if node_id not in node_ids:
                raise ValueError(f"unknown node {node_id} in cluster {cluster_id}")
            if node_id in cluster_by_node:
                raise ValueError(f"node {node_id} belongs to multiple clusters")
            cluster_by_node[node_id] = cluster_id
    template_ids = set()
    for template in prob.get("lowering_templates", []):
        tid = int(template["id"])
        if tid in template_ids:
            raise ValueError(f"duplicate lowering template id {tid}")
        template_ids.add(tid)
        if template.get("relation") not in ("always", "same_wg", "different_wg"):
            raise ValueError(f"invalid lowering relation in template {tid}")
        if (
            int(template["src_node"]) not in node_ids
            or int(template["dst_node"]) not in node_ids
        ):
            raise ValueError(f"unknown lowering endpoint in template {tid}")
        if (
            int(template["src_cluster"]) not in cluster_ids
            or int(template["dst_cluster"]) not in cluster_ids
        ):
            raise ValueError(f"unknown lowering cluster in template {tid}")
        if cluster_by_node.get(int(template["src_node"])) != int(
            template["src_cluster"]
        ) or cluster_by_node.get(int(template["dst_node"])) != int(
            template["dst_cluster"]
        ):
            raise ValueError(f"lowering endpoint/cluster mismatch in template {tid}")
        event_ids = set()
        for event in template.get("events", []):
            eid = int(event["id"])
            if eid in event_ids:
                raise ValueError(f"duplicate event id {eid} in template {tid}")
            event_ids.add(eid)
            if event.get("kind") not in _LOWERING_EVENT_KINDS:
                raise ValueError(f"invalid event kind in template {tid}")
            if event.get("owner") not in ("src", "dst"):
                raise ValueError(f"invalid event owner in template {tid}")
            if event.get("placement") not in ("before", "after"):
                raise ValueError(f"invalid event placement in template {tid}")
            if int(event["anchor_node"]) not in node_ids:
                raise ValueError(f"unknown event anchor in template {tid}")
            owner_cluster = int(
                template["src_cluster"]
                if event["owner"] == "src"
                else template["dst_cluster"]
            )
            if cluster_by_node.get(int(event["anchor_node"])) != owner_cluster:
                raise ValueError(f"event owner/anchor mismatch in template {tid}")
            if (
                int(event.get("issue_duration", 0)) < 0
                or int(event.get("completion_latency", 0)) < 0
                or int(event.get("frequency", 1)) <= 0
            ):
                raise ValueError(f"invalid event timing in template {tid}")


def _template_presence_literal(template, cindex, same_wg):
    relation = template["relation"]
    if relation == "always":
        return None
    ci = cindex[int(template["src_cluster"])]
    cj = cindex[int(template["dst_cluster"])]
    same = same_wg(ci, cj)
    return same if relation == "same_wg" else same.Not()


def _event_issue_duration(event):
    return int(event.get("issue_duration", 0)) * int(event.get("frequency", 1))


def _and_literal(model, literals, name):
    if not literals:
        return None
    if len(literals) == 1:
        return literals[0]
    out = model.NewBoolVar(name)
    model.AddBoolAnd(literals).OnlyEnforceIf(out)
    model.AddBoolOr([lit.Not() for lit in literals]).OnlyEnforceIf(out.Not())
    return out


def _distance_zero_reachability(nodes, edges):
    succ = {int(nd["id"]): [] for nd in nodes}
    for edge in edges:
        if int(edge["distance"]) == 0:
            succ.setdefault(int(edge["src"]), []).append(int(edge["dst"]))
    reachable = {}
    for start in succ:
        seen = set()
...
def _blocking_inversion_expr(model, prob, cycle, cluster_of, cindex, same_wg):
    """Work left behind a blocking wait in the same emitted WG stream.

    Only independent fixed-stage CUDA/SFU work participates. Dependences keep
    critical readers in place; deferrable waits sink below independent work,
    matching the emitter's in-order stream semantics without changing the
    primary schedule/partition objective.
    """
    nodes = {int(nd["id"]): nd for nd in prob["nodes"]}
    reach = _distance_zero_reachability(prob["nodes"], prob["edges"])
    ii = int(prob["ii"])
    terms = []
    for template in prob.get("lowering_templates", []):
        present = _template_presence_literal(template, cindex, same_wg)
        for event in template.get("events", []):
            if event.get("kind") != "wait" or not event.get("blocking", False):
                continue
            anchor = int(event["anchor_node"])
            owner_cluster_id = int(
                template["src_cluster"]
                if event["owner"] == "src"
                else template["dst_cluster"]
            )
            owner_cluster = cindex[owner_cluster_id]
            anchor_stage = int(nodes[anchor]["cycle"]) // ii
            for other, nd in nodes.items():
                other_cluster = cluster_of.get(other)
                if other == anchor or other_cluster is None:
                    continue
                if (
                    nd["pipeline"] not in ("CUDA", "SFU")
                    or int(nd.get("duration", 0)) <= 0
                ):
                    continue
                if int(nd["cycle"]) // ii != anchor_stage:
                    continue
                if other in reach.get(anchor, ()) or anchor in reach.get(other, ()):
                    continue

                before = model.NewBoolVar(
                    f"wait_before_t{template['id']}_e{event['id']}_n{other}"
                )
                model.Add(cycle[anchor] < cycle[other]).OnlyEnforceIf(before)
                model.Add(cycle[anchor] >= cycle[other]).OnlyEnforceIf(before.Not())
                literals = [before]
                if present is not None:
                    literals.append(present)
                if owner_cluster != other_cluster:
                    literals.append(same_wg(owner_cluster, other_cluster))
                inversion = _and_literal(
                    model,
                    literals,
                    f"wait_inv_t{template['id']}_e{event['id']}_n{other}",
                )
                weight = int(nd["duration"]) * max(1, int(nd.get("freq", 1)))
                terms.append(weight * inversion)
    return sum(terms) if terms else None


def _instantiate_lowering_plan(prob, result):
    templates = prob.get("lowering_templates", [])
    if not templates:
        return {"version": _LOWERING_PLAN_VERSION, "templates": []}
    wg = {int(k): int(v) for k, v in result.get("wg", {}).items()}
    cycles = {int(nd["id"]): int(nd["cycle"]) for nd in prob["nodes"]}
    cycles.update({int(k): int(v) for k, v in result.get("cycles", {}).items()})
    nodes = {int(nd["id"]): nd for nd in prob["nodes"]}
    planned = []
    event_groups = {}
    for template in templates:
        src_wg = wg[int(template["src_cluster"])]
        dst_wg = wg[int(template["dst_cluster"])]
        relation = template["relation"]
        active = (
            relation == "always"
            or (relation == "same_wg" and src_wg == dst_wg)
            or (relation == "different_wg" and src_wg != dst_wg)
        )
        item = {"id": int(template["id"]), "active": active, "events": []}
        if active:
            for event in template.get("events", []):
                anchor = int(event["anchor_node"])
                owner_cluster = int(
                ...
                event_groups.setdefault((anchor, event["placement"]), []).append(
                    (
                        int(template["id"]),
                        int(event["id"]),
                        _event_issue_duration(event),
                        entry,
                    )
                )
        planned.append(item)
    ...
    active_events = []
    for (anchor, placement), group in event_groups.items():
        group.sort(key=lambda item: (item[0], item[1]))
        if placement == "before":
            cursor = cycles[anchor] - sum(item[2] for item in group)
        else:
            cursor = cycles[anchor] + max(1, int(nodes[anchor].get("duration", 0)))
        for template_id, event_id, duration, entry in group:
            entry["cycle"] = cursor
            cursor += duration
    ...
    per_wg_order = {}
    for wg_id, _, _, _, _, entry in sorted(active_events):
        entry["stream_order"] = per_wg_order.get(wg_id, 0)
        per_wg_order[wg_id] = entry["stream_order"] + 1
    return {"version": _LOWERING_PLAN_VERSION, "templates": planned}
...
def _intrawg_async_reader_pairs(nodes_by_id, edges, cluster_of):
    ...
    """
    if not env_flag_enabled("TRITON_MODULO_INTRAWG_ASYNC_LEGALITY", True):
        return set()
    out = set()
    for e in edges:
...
def _mem_resident_srcs(prob):
    ...
    """
    if not env_flag_enabled("TRITON_MODULO_MEM_RESIDENT_SPANS", False):
        return set()
    out = set()
...
def _has_full_graph(prob):
    ...
    return bool(prob.get("buffers")) and any(
        "src_cluster" not in e for e in prob.get("edges", [])
    )
...
def normalize_values(values, u):
    ...
    model.Minimize(f)
    solver = _new_cp_solver(2.0, num_search_workers=1, random_seed=0)
...
def solve_at_ii(prob, ii, time_limit_s, hint=None):
    ...
    streaming = (
        {nd["id"] for nd in nodes if nd.get("streaming")}
        if prob.get("streaming_vl")
        else set()
    )
    for e in edges:
    ...
    for pipe, members in by_pipe.items():
        ...
        for i in members:
            ...
            segments.append(
                model.NewIntervalVar(phase[i], size1, end1, f"seg1_{pipe}_{i}")
            )
            if dur > 1:
                ...
                segments.append(
                    model.NewOptionalIntervalVar(
                        0, over, over, wraps, f"seg2_{pipe}_{i}"
                    )
                )
    ...
    for b in prob["buffers"]:
        ...
        if b["kind"] == "smem":
            ...
            model.AddMaxEquality(
                last_stage, [stage[c] for c in consumers] + [stage[alloc]]
            )
            ...
            model.AddMaxEquality(
                last_cyc, [cycle[c] for c in consumers] + [cycle[alloc]]
            )
            ...
            tmem_intervals.append(
                model.NewIntervalVar(cycle[alloc], size, end, f"tmem_b{alloc}")
            )
    ...
    rec_span = sum(
        cycle[e["src"]] - cycle[e["dst"]]
        for e in edges
        if e["distance"] > 0 and e["src"] != e["dst"]
    )
    model.Minimize(
        10240000 * max_stage
        - 102400 * sum(depth_vars)
        + 8192 * rec_span
        + 1024 * reg_pressure
        + smem_total
    )
    ...
    solver = _new_cp_solver(time_limit_s, num_search_workers=1, random_seed=0)
    status = solver.Solve(model)
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        return (
            "feasible",
            {i: solver.Value(cycle[i]) for i in range(n)},
...
def solve_partition(prob, fixed_assign=None, deterministic_budget=None):
    ...
    def window(nd, other):
        ...
        if (
            _async_flight_blocking()
            and nd["pipeline"] in ASYNC
            and other["pipeline"] not in ASYNC
        ):
        ...
        for j in range(i + 1, ncl):
            ...
            for u in clusters[i]["nodes"]:
                for v in clusters[j]["nodes"]:
                    ...
                    ov += _mod_overlap(
                        nu["cycle"], window(nu, nv), nv["cycle"], window(nv, nu), ii
                    ) * max(nu["freq"], nv["freq"])
            if ov > 0:
    ...
    for e in prob["edges"]:
        ...
        channel = (e["src"], e["dst"], e.get("src_result_idx", 0))
        if e.get("chan_bytes", 0) > 0 and channel not in seen_pairs:
            seen_pairs.add(channel)
            cb = model.NewIntVar(
                0,
                e["chan_bytes"],
                f"cb_{channel[0]}_{channel[1]}_{channel[2]}",
            )
    ...
    dur_c = [
        int(
            sum(
                max(nodes[u]["duration"], 0) * nodes[u].get("freq", 1)
                for u in c["nodes"]
            )
        )
        for c in clusters
    ...
    if env_flag_enabled("TRITON_MODULO_CAPACITY_TERM", True):
        for w in range(ncl):
    ...
    budget = float(
        prob.get("time_limit_s", 20.0)
        if deterministic_budget is None
        else deterministic_budget
    )
    if budget <= 0:
        return {"status": "unknown", "deterministic_time": 0.0}
    solver = _new_cp_solver(budget, num_search_workers=1, random_seed=0)
    status = solver.Solve(model)
    deterministic_time = _solver_deterministic_time(solver)
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        return {
            ...
            "message": f"partition solve status {status}",
    ...
    if fixed_assign is not None:
        if status != cp_model.OPTIMAL:
            return {
                "status": "unknown",
                "message": "fixed-assignment score is not proven optimal",
                "deterministic_time": deterministic_time,
            }
        return {
            "status": "ok",
    ...
    if _recmii_rerank_enabled() and _has_full_graph(prob):
        ...
        for _ in range(max(0, k - 1)):
            remaining = budget - deterministic_time
            if remaining <= 0:
                break
            prev = candidates[-1][0]
            model.Add(sum(assign[(i, prev[i])] for i in range(ncl)) <= ncl - 1)
            s2 = _new_cp_solver(
                min(3.0, remaining), num_search_workers=1, random_seed=0
            )
            st2 = s2.Solve(model)
            deterministic_time += _solver_deterministic_time(s2)
        ...
        for amap0 in _canonical_candidates(prob):
            ...
            deterministic_time += _solver_deterministic_time(s3)
        model.ClearAssumptions()
        scored = []
        ...
        return {
            "status": "ok",
            "wg": {str(clusters[i]["id"]): vec[i] for i in range(ncl)},
            "used_wgs": uw,
            "objective": obj,
            "rec_cycles": rec,
            "combined": combined,
...
def _arbitrate_v2(prob, v2_amap, deterministic_budget=None):
    ...
    if budget <= 0:
        return None
    probe_reserve = budget * 0.25
    v1 = solve_partition(
        prob, deterministic_budget=max(0.0, budget - probe_reserve)
    )
    if v1.get("status") != "ok" or "combined" not in v1:
        return None
    remaining = budget - float(v1.get("deterministic_time", 0.0))
    if remaining <= 0:
        return None
    probe = solve_partition(
        prob, fixed_assign=v2_amap, deterministic_budget=remaining
    )
...
def solve_joint(prob):
    ...
    cp = _critical_path(
        [node_by_id[i] for i in sorted(node_by_id)],
    ...
    model = cp_model.CpModel()
    cycle = {
        nd["id"]: model.NewIntVar(0, horizon - 1, f"cyc_{nd['id']}") for nd in nodes
    }
    stage = {
        nd["id"]: model.NewIntVar(0, max_stages - 1, f"stg_{nd['id']}") for nd in nodes
    }
    ...
    for nd in nodes:
        ...
        model.AddDivisionEquality(stage[i], cycle[i], ii)
        model.AddModuloEquality(phase[i], cycle[i], ii)
        # Stage moves are not representable by the current TLX emitter.
        model.Add(stage[i] == int(nd["cycle"]) // ii)
    ...
    lowering_edges = {
        (
            int(template["src_node"]),
            int(template["dst_node"]),
            int(template["events"][0].get("distance", 0)),
        )
        for template in prob.get("lowering_templates", [])
    ...
    for e in edges:
        ...
        if (int(e["src"]), int(e["dst"]), int(e["distance"])) not in lowering_edges:
    ...
    def modular_segments_at(start, dur, tag, presence=None):
        segs = []
        end1 = model.NewIntVar(1, ii, f"end1_{tag}")
        model.AddMinEquality(end1, [start + dur, ii])
        size1 = model.NewIntVar(1, dur, f"size1_{tag}")
        model.Add(size1 == end1 - start)
        if presence is None:
            segs.append(model.NewIntervalVar(start, size1, end1, f"seg1_{tag}"))
        else:
            segs.append(
                model.NewOptionalIntervalVar(
                    start, size1, end1, presence, f"seg1_{tag}"
                )
            )
        if dur > 1:
            over = model.NewIntVar(0, dur - 1, f"over_{tag}")
            model.AddMaxEquality(over, [start + dur - ii, 0])
            ...
            if presence is not None:
                ...
                model.AddBoolOr([wraps.Not(), presence.Not()]).OnlyEnforceIf(
                    pres2.Not()
                )
            segs.append(
                model.NewOptionalIntervalVar(0, over, over, pres2, f"seg2_{tag}")
            )
        return segs

    def modular_segments(i, dur, tag, presence=None):
        return modular_segments_at(phase[i], dur, tag, presence)

    pending_lowering_events = []
    for template in prob.get("lowering_templates", []):
        active = _template_presence_literal(template, cindex, same_wg)
        for event in template.get("events", []):
            duration = _event_issue_duration(event)
            if duration <= 0:
                continue
            if duration > ii:
                if active is None:
                    return {
                        "status": "error",
                        "message": (
                            "always-active lowering event exceeds II for "
                            f"t{template['id']}_e{event['id']}"
                        ),
                    }
                model.Add(active == 0)
                continue
            ...
            pending_lowering_events.append(
                {
                    "template_id": int(template["id"]),
                    "event_id": int(event["id"]),
                    "anchor": anchor,
                    "placement": event["placement"],
                    "owner_cluster": owner_cluster_id,
                    "pipeline": event["pipeline"],
                    "duration": duration,
                    "active": active,
                    "tag": f"lev_t{template['id']}_e{event['id']}",
                }
            )

    total_event_duration = sum(event["duration"] for event in pending_lowering_events)
    event_shift = (
        (ii + total_event_duration + ii - 1) // ii
    ) * ii
    grouped_lowering_events = {}
    for event in pending_lowering_events:
        grouped_lowering_events.setdefault(
            (event["anchor"], event["placement"]), []
        ).append(event)

    lowering_issue_events = []
    for (anchor, placement), group in grouped_lowering_events.items():
        group.sort(key=lambda event: (event["template_id"], event["event_id"]))
        for index, event in enumerate(group):
            duration = event["duration"]
            event_cycle = model.NewIntVar(
                -event_shift,
                horizon + event_shift,
                f"lev_c_t{event['template_id']}_e{event['event_id']}",
            )

            def active_duration(other):
                if other["active"] is None:
                    return other["duration"]
                return other["duration"] * other["active"]

            if placement == "before":
                offset = sum(active_duration(other) for other in group[index:])
                model.Add(event_cycle == cycle[anchor] - offset)
            else:
                offset = sum(active_duration(other) for other in group[:index])
                model.Add(
                    event_cycle
                    == cycle[anchor]
                    + max(1, int(node_by_id[anchor]["duration"]))
                    + offset
                )
            shifted = model.NewIntVar(
                0,
                horizon + 2 * event_shift,
                f"lev_shift_t{event['template_id']}_e{event['event_id']}",
            )
            model.Add(shifted == event_cycle + event_shift)
            event_phase = model.NewIntVar(
                0,
                ii - 1,
                f"lev_phase_t{event['template_id']}_e{event['event_id']}",
            )
            model.AddModuloEquality(event_phase, shifted, ii)
            lowering_issue_events.append(
                (
                    event["owner_cluster"],
                    event["pipeline"],
                    event_phase,
    ...
    for _, pipe, _, duration, _, tag in lowering_issue_events:
        if pipe != "NONE" and duration > ii:
            return {"status": "error", "message": f"duration exceeds II for {tag}"}
        if pipe != "NONE":
            by_pipe.setdefault(pipe, [])
    for pipe, members in by_pipe.items():
        segs = []
        for i in members:
            segs += modular_segments(
                i, max(node_by_id[i]["duration"], 1), f"p_{pipe}_{i}"
            )
        for _, event_pipe, event_phase, duration, active, tag in lowering_issue_events:
            if event_pipe == pipe:
                segs += modular_segments_at(
                    event_phase, duration, f"p_{pipe}_{tag}", presence=active
                )
        if len(segs) < 2:
            continue
    ...
    for w in range(ncl):
        ...
        for (
            ...
            active,
            tag,
        ) in lowering_issue_events:
            literals = [assign[(cindex[owner_cluster_id], w)]]
            if active is not None:
                literals.append(active)
            presence = _and_literal(model, literals, f"{tag}_w{w}_present")
            segs += modular_segments_at(
                event_phase, duration, f"{tag}_w{w}", presence=presence
            )
        if segs:
            model.AddNoOverlap(segs)
    ...
    blocking_inversions = _blocking_inversion_expr(
        model, prob, cycle, cluster_of, cindex, same_wg
    )
    model.Minimize(primary_objective)

    time_limit = float(prob.get("time_limit_s", 20.0))
    arbitration_reserve = (
        time_limit * 0.25
        if _recmii_rerank_enabled() and _has_full_graph(prob)
        else 0.0
    )
    primary_budget = max(0.0, time_limit - arbitration_reserve)
    if primary_budget <= 0:
        return {"status": "error", "message": "no joint primary budget"}
    solver = _new_cp_solver(primary_budget, num_search_workers=1, random_seed=0)
    status = solver.Solve(model)
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
    ...
    lowering_value = (
        solver.Value(blocking_inversions) if blocking_inversions is not None else None
    )
    remaining = max(0.0, time_limit - _solver_deterministic_time(solver))
    secondary_budget = max(0.0, remaining - arbitration_reserve)
    if (
        status == cp_model.OPTIMAL
        and blocking_inversions is not None
        and secondary_budget >= 0.05
    ):
        model.Add(primary_objective == primary_value)
        model.Minimize(blocking_inversions)
        lowering_solver = _new_cp_solver(
            secondary_budget, num_search_workers=1, random_seed=0
        )
        lowering_status = lowering_solver.Solve(model)
        remaining = max(
            0.0, remaining - _solver_deterministic_time(lowering_solver)
        )
        if lowering_status == cp_model.OPTIMAL:
            candidate_lowering_value = lowering_solver.Value(blocking_inversions)
            if candidate_lowering_value <= lowering_value:
                selected = lowering_solver
                lowering_value = candidate_lowering_value

    v2_amap = {c["id"]: selected.Value(wg[i]) for i, c in enumerate(clusters)}
    arb = _arbitrate_v2(prob, v2_amap, deterministic_budget=remaining)
    if arb is not None:
        return arb
    result = {
        "status": "ok",
        "wg": {str(cid): w for cid, w in v2_amap.items()},
        "cycles": {str(nd["id"]): selected.Value(cycle[nd["id"]]) for nd in nodes},
        "used_wgs": selected.Value(used_wgs),
        "objective": primary_value,
    }
    if lowering_value is not None:
        result["lowering_objective"] = lowering_value
    return result


def main():
    if sys.argv[1:] == ["--fingerprint"]:
        print(json.dumps(solver_fingerprint(), sort_keys=True, separators=(",", ":")))
        return 0
    if len(sys.argv) != 3:
        print(
            "usage: python -m triton.tools.modulo_joint_solver "
            "<problem.json> <solution.json> | --fingerprint",
            file=sys.stderr,
        )
        return 2
    if cp_model is None:
    ...
    with open(sys.argv[1]) as f:
        prob = json.load(f)
    try:
        _validate_problem_schema(prob)
    except (KeyError, TypeError, ValueError) as exc:
        result = {"status": "error", "message": f"invalid problem schema: {exc}"}
    ...
    if prob.get("mode") in ("partition", "joint"):
        result = (
            solve_partition(prob) if prob["mode"] == "partition" else solve_joint(prob)
        )
        result["version"] = _JOINT_SCHEMA_V2
        if result["status"] == "ok":
            result["lowering_plan"] = _instantiate_lowering_plan(prob, result)
        with open(sys.argv[2], "w") as f:
            json.dump(result, f)
    ...
    for ii in range(min_ii, max_ii + 1):
        ...
        if status == "unknown":
            ...
            result = {
                "status": "error",
                "message": f"joint solve inconclusive at II {ii}",
                "stats": stats,
            }
            break
        if status == "error":
            result = {
                "status": "error",
                "message": "malformed DDG (cycle in distance-0 edges)",
            }
            break
    ...
    if result is None:
        result = {
            "status": "error",
            "message": f"no feasible II in [{min_ii}, {max_ii}]",
            "stats": stats,
        }
    result["version"] = _JOINT_SCHEMA_V1
    with open(sys.argv[2], "w") as f:
        json.dump(result, f)
    return 0 if result["status"] == "ok" else 1
