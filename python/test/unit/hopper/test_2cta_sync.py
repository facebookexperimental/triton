...
(see third_party/tlx/tutorials/triton_addmm_2cta.py).
"""
...
def test_compile_with_ctas_per_cga_does_not_crash():
    """Verify that compiling with ctas_per_cga=(2,1,1) does not crash.
    With ctas_per_cga, num_ctas is set to 1 and PlanCTA is effectively
    bypassed for 2-CTA layout decisions. Without two_ctas=True on tl.dot(),
    canUseTwoCTAs() does not trigger.
    ...
    compiled = triton.compile(
        src,
        target=target,
        options={"num_warps": 4, "num_stages": 2, "ctas_per_cga": (2, 1, 1)},
    )
    ttgir = compiled.asm.get("ttgir", "")
    # The pass should run without crashing. With pointer-based loads and
    # no two_ctas=True, no cross-CTA sync ops expected.
    assert "tc_gen5_mma" in ttgir, "Expected tc_gen5_mma in TTGIR"
    assert "map_to_remote_buffer" not in ttgir, (
        "Pointer-based loads without two_ctas=True shouldn't trigger 2-CTA sync")
