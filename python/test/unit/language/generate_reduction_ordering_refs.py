...
@triton.jit
def sum_kernel(X, Z, stride_x, N_ROWS: tl.constexpr, BLOCK_N: tl.constexpr,
...
@triton.jit
def mul_kernel(X, Z, stride_x, N_ROWS: tl.constexpr, BLOCK_N: tl.constexpr,
               ORDERING: tl.constexpr):
    offs_m = tl.arange(0, N_ROWS)
    offs_n = tl.arange(0, BLOCK_N)
    x = tl.load(X + offs_m[:, None] * stride_x + offs_n[None, :])
    z = tl.reduce(x, axis=1, combine_fn=mul_combine,
                  reduction_ordering=ORDERING)
    tl.store(Z + offs_m, z)
...
def main():
    ...
    for N_ROWS in [1, 4, 16, 32]:
        ...
        torch.save(x.cpu(),
                   os.path.join(DATA_DIR, f"reduction_ordering_input_{N_ROWS}.pt"))

        # Sum reference
        out_sum = torch.empty(N_ROWS, device=device, dtype=torch.float32)
        sum_kernel[(1,)](x, out_sum, x.stride(0), N_ROWS=N_ROWS,
                         BLOCK_N=BLOCK_N,
                         ORDERING=tl.ReductionOrdering.INNER_TREE,
                         num_warps=1)
        torch.save(out_sum.cpu(),
                   os.path.join(DATA_DIR, f"reduction_ordering_sum_ref_{N_ROWS}.pt"))

        # Mul reference (use small uniform values to avoid inf/0)
        x_mul = torch.empty((N_ROWS, BLOCK_N), device=device,
                            dtype=torch.float32).uniform_(0.99, 1.01)
        torch.save(x_mul.cpu(),
                   os.path.join(DATA_DIR, f"reduction_ordering_mul_input_{N_ROWS}.pt"))

        out_mul = torch.empty(N_ROWS, device=device, dtype=torch.float32)
        mul_kernel[(1,)](x_mul, out_mul, x_mul.stride(0), N_ROWS=N_ROWS,
                         BLOCK_N=BLOCK_N,
                         ORDERING=tl.ReductionOrdering.INNER_TREE,
                         num_warps=1)
        torch.save(out_mul.cpu(),
                   os.path.join(DATA_DIR, f"reduction_ordering_mul_ref_{N_ROWS}.pt"))
