...
def test_tl_dot_2cta_truncated_b(device):
    ...
    reference = torch.matmul(a.float(), b.to(torch.bfloat16).float()).to(torch.bfloat16)
    ...
    assert ('"ttg.cluster-dim-x" = 2 : i32, "ttg.cluster-dim-y" = 1 : i32, '
            '"ttg.cluster-dim-z" = 1 : i32') in ttgir
