...
@triton.jit
def _attention_style_2cta_data_partition_kernel(
    q_ptr,
    k_desc,
    v_desc,
    o_ptr,
    ...
    for start_n in tl.range(
        ...
        qk = tl.dot(q, k, two_ctas=True)
        acc = tl.dot(qk.to(q.dtype), v, acc, two_ctas=True)

    tl.store(o_ptr + offs_m[:, None] * HEAD_DIM + offs_d[None, :], acc.to(tl.bfloat16))
...
@pytest.mark.xfail(
    reason="2-CTA FA-style chained dots currently fail in AutoWS handleOperandD; "
    "per-MMA sync is necessary but not sufficient for FA.",
    strict=True,
)
@pytest.mark.parametrize("DATA_PARTITION_FACTOR", [2])
@pytest.mark.skipif(not is_blackwell(), reason="Requires Blackwell")
def test_tl_dot_attention_style_2cta_data_partition(DATA_PARTITION_FACTOR, device):
    torch.manual_seed(0)
    dtype = torch.bfloat16
    n_ctx = 512
    head_dim = 128
    ...
    q = torch.randn((n_ctx, head_dim), device=device, dtype=dtype) * 0.01
    k = torch.randn((n_ctx, head_dim), device=device, dtype=dtype) * 0.01
    v = torch.randn((n_ctx, head_dim), device=device, dtype=dtype)
    o = torch.empty_like(q)

    k_desc = TensorDescriptor(k, k.shape, k.stride(), [block_n, head_dim])
    v_desc = TensorDescriptor(v, v.shape, v.stride(), [block_n, head_dim])
    ...
    with triton.knobs.nvidia.scope():
        ...
        kernel = _attention_style_2cta_data_partition_kernel[(triton.cdiv(n_ctx, block_m), )](
            q,
            k_desc,
            v_desc,
            o,
    ...
    ref = torch.matmul(torch.matmul(q.to(torch.float32), k.T.to(torch.float32)), v.to(torch.float32)).to(dtype)
    torch.testing.assert_close(o, ref, atol=2e-2, rtol=2e-2)

    ttgir = kernel.asm["ttgir"]
    assert "ttg.warp_specialize" in ttgir
    assert ttgir.count("ttng.tc_gen5_mma") >= 4
    assert ttgir.count("two_ctas") >= 4
