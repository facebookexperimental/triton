...
@pytest.mark.skipif(not is_hopper_or_newer(), reason="Need Hopper or newer")
def test_vote_ballot_sync_all_true(device):
    """Test vote_ballot_sync when all threads vote True."""

    @triton.jit
    def vote_ballot_all_true_kernel(output_ptr, ):
        tid = tlx.thread_id(0)
        # All threads vote True
        pred = tl.full([], 1, dtype=tl.int1)
        ballot_result = tlx.vote_ballot_sync(0xFFFFFFFF, pred)
    ...
    vote_ballot_all_true_kernel[(1, )](output, num_warps=1)
    torch.cuda.synchronize()

    # All 32 threads voted True, so all bits should be set
    expected_ballot = 0xFFFFFFFF
    # Note: Python int is signed, so we compare as unsigned
    assert (output.item() & 0xFFFFFFFF) == expected_ballot, (
        f"Expected {hex(expected_ballot)}, got {hex(output.item() & 0xFFFFFFFF)}")


@pytest.mark.skipif(not is_hopper_or_newer(), reason="Need Hopper or newer")
def test_vote_ballot_sync_all_false(device):
    """Test vote_ballot_sync when all threads vote False."""

    @triton.jit
    def vote_ballot_all_false_kernel(output_ptr, ):
        tid = tlx.thread_id(0)
        # All threads vote False
        pred = tl.full([], 0, dtype=tl.int1)
        ballot_result = tlx.vote_ballot_sync(0xFFFFFFFF, pred)
    ...
    vote_ballot_all_false_kernel[(1, )](output, num_warps=1)
    torch.cuda.synchronize()

    # All 32 threads voted False, so no bits should be set
    expected_ballot = 0x00000000
    assert output.item() == expected_ballot, f"Expected {hex(expected_ballot)}, got {hex(output.item())}"
