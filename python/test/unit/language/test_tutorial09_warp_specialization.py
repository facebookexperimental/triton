...
def test_tutorial09_matmul_tma_persistent_warp_specialize(
    ...
    if (DATA_PARTITION_FACTOR == 2 and SMEM_ALLOC_ALGO == 0 and BLOCK_SIZE_M == 256 and num_stages == 2
            and FLATTEN and not A_col_major and not B_col_major and EPILOGUE_SUBTILE == 1
            and use_early_tma_store_lowering):
    ...
    if (DATA_PARTITION_FACTOR == 2 and BLOCK_SIZE_M == 256 and BLOCK_SIZE_N == 256 and FLATTEN
            and SMEM_ALLOC_ALGO == 0):
        pytest.skip("Out of resources: tensor memory exceeded")

    if (DATA_PARTITION_FACTOR == 2 and BLOCK_SIZE_M == 256 and num_stages == 3 and FLATTEN
            and SMEM_ALLOC_ALGO == 0):
        pytest.skip("Out of resources: tensor memory exceeded")
