...
def test_tutorial09_matmul_tma_unified_persistent_while_loop_warp_specialize(
    ...
    with triton.knobs.nvidia.scope():
        ...
        if (SCHEDULE is tl.StaticPersistent1DScheduler and NUM_CTAS == 1 and BLOCK_SIZE_M == 128 and BLOCK_SIZE_N == 256
                and EPILOGUE_SUBTILE == 2):
            operand_staging = [
                line for line in ttgir.splitlines()
                if "ttg.local_alloc {buffer.copy = 3" in line and "x64xf16" in line and "buffer.tmaStaging" not in line
            ]
            assert len(operand_staging) == 2, "Expected exactly the complete three-deep A/B operand bundle"
            assert any("3x128x64xf16" in line for line in operand_staging), "Expected the three-deep A operand"
            assert any("3x256x64xf16" in line for line in operand_staging), "Expected the three-deep B operand"
