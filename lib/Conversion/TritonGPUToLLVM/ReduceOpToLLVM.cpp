...
private:
  ...
  bool isInnerTree(triton::ReduceOp op) const {
    auto attr = op->getAttrOfType<StringAttr>("reduction_ordering");
  ...
  void packResults(ReduceOpHelper &helper,
    ...
    // For inner tree, group accs by non-axis key to collect all partial
    // results for each output position.
    ...
    for (unsigned i = 0; i < op.getNumOperands(); ++i) {
      if (auto resultTy =
        ...
        for (int j = 0; j < resultElems; j++) {
          ...
          if (innerTree) {
            // Collect operand i from all partial results and tree-reduce
            auto &groups = groupedAccs[key];
            SmallVector<Value> groupVals;
            for (auto &group : groups)
              groupVals.push_back(group[i]);
            // Count-up tree-reduce
            while (groupVals.size() > 1) {
              SmallVector<Value> nextLevel;
              for (unsigned g = 0; g + 1 < groupVals.size(); g += 2) {
                SmallVector<Value> a = {groupVals[g]};
                SmallVector<Value> bVec = {groupVals[g + 1]};
                accumulate(op.getLoc(), rewriter, op.getCombineOp(), a, bVec);
                nextLevel.push_back(a[0]);
              }
              if (groupVals.size() % 2 == 1)
                nextLevel.push_back(groupVals.back());
              groupVals = std::move(nextLevel);
            }
            resultVals.push_back(groupVals[0]);
          } else {
        ...
        // 0d-tensor -> scalar
        if (innerTree) {
          auto &groups = groupedAccs.begin()->second;
          SmallVector<Value> groupVals;
          for (auto &group : groups)
            groupVals.push_back(group[i]);
          while (groupVals.size() > 1) {
  ...
  void storeWarpReduceToSharedMemory(
    ...
    // Track accumulator group index per output position for inner tree
    unsigned accGroupIdx = 0;
    SmallVector<unsigned> prevNonAxisKey;
    ...
    for (auto it : accs) {
      ...
      if (innerTree) {
        // Compute dense group index: keys are sorted, so consecutive keys
        // with the same non-axis coordinates get sequential group indices
        SmallVector<unsigned> nonAxisKey = key;
        nonAxisKey[axis] = 0;
        if (nonAxisKey != prevNonAxisKey) {
          accGroupIdx = 0;
          prevNonAxisKey = nonAxisKey;
        }
        writeIdx[axis] =
            b.add(b.i32_val(accGroupIdx * sizeInterWarps), warpIdAxis);
        accGroupIdx++;
      } else {
      ...
      if (auto resultTy =
        ...
        for (size_t j = 0; j < resultElems; ++j) {
          ...
          if (K == 1) {
            ...
            // INNER_TREE: load K values and tree-reduce
            SmallVector<Value> groupVals;
            for (unsigned g = 0; g < K; ++g) {
              ...
              groupVals.push_back(b.load(elemTy, readPtr));
            }
            // Tree-reduce K values
            while (groupVals.size() > 1) {
              SmallVector<Value> nextLevel;
              for (unsigned g = 0; g + 1 < groupVals.size(); g += 2) {
                SmallVector<Value> a = {groupVals[g]};
        ...
        if (K == 1) {
          ...
          // INNER_TREE: load K values and tree-reduce for scalar result
          SmallVector<Value> groupVals;
          for (unsigned g = 0; g < K; ++g) {
