...
private:
  ...
  // positions (register/lane/warp/slot) so the pow2 butterfly produces correct
  // results.
  ...
  // The reduction hardware (warp shuffle / register loop) needs a power-of-2
  // axis, so toLinearLayout rounds an NPOT axis up (e.g. 48 -> 64). The layout
  // then addresses pow2 slots, but only [0, dimSize) hold real data: a slot
  // whose raw axis position is >= dimSize is a "phantom" duplicate that must
  // not enter the reduction. We reconstruct each slot's raw position as the sum
  // of the set per-bit (register/lane/warp) position-bases -- they ADD because
  // the bits are independent -- flag position >= dimSize, and overwrite those
  // slots with the reduction identity (0 for sum, -inf for max) before
  // reducing. Pow2 axes skip all of this (no rounding -> the helpers below
  // return empty/null and emit no extra IR -> byte-identical to the pow2
  // reduction).
  //
  ...
  // For NPOT reduction dims, compute a predicate that is true for lanes/warps
