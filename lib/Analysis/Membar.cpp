...
static bool containsLocalBarrier(Operation *op) {
  ...
  // NOTE: ArriveBarrierOp is intentionally NOT treated as a local barrier here.
  // beta handles arrive_barrier specially in update() below (perThread arrives
  // skip the CTA-wide fence; regular arrives force a full-memory hazard so a
  // barrier is inserted before them). Short-circuiting here would bypass that
  // perThread-aware logic. See test-membar-ttng.mlir:
  // {barrier_before_regular_arrive, no_barrier_before_perthread_arrive}.
  return false;
}
