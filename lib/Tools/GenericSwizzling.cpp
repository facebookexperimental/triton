...
// Handle mixed pow2/NPOT layouts by splitting: swizzle pow2 dims, place each
// NPOT dim in segment (padded to next pow2) with Z/r mod-add coupling into its
// bases.
...
LinearLayout optimalSwizzlingLdSt(const LinearLayout &src,
  ...
  // Mixed pow2/NPOT: split, swizzle pow2 part, add Z/r mod-add coupling into
  // NPOT dims
  if (src.isModular()) {
