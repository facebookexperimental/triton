...
private:
  FailureOr<llvm::SmallBitVector> analyze(Value value) {
    ...
    // Reject ops whose result can differ across CTAs in ways this static
    // analysis cannot model: CLC (cluster launch control) hands out
    // work-stolen program ids at runtime, and atomics/loads read mutable
    // state. Treating any of these as pid-invariant would be unsound. The
    // `clc` substring match is intentional rather than an isa<> list: the CLC
    // op family spans many variants (clc_try_cancel, clc_advance, clc_read,
    // ...) that share no common trait or interface, and new variants are added
    // as Blackwell support evolves, so matching the shared mnemonic catches the
    // whole family without an enumeration that must be kept exhaustive. The
    // conservative allowlist below is the ultimate backstop -- any ttng op
    // reaches it and fails anyway.
