...
// (This is exactly the freedom `OptimizeThreadLocality` exploits for
// *unordered* reduces, which it must skip for ordered ones -- see its
// `hasDefinedOrdering` bail. This pass fills that gap for the ordered case.)
//
// STRATEGY. The cost of a reduction is dominated by the cross-warp
// shared-memory stage, which runs iff `warpsPerCTA[axis] != 1` (two barriers +
// smem round trip
...
// cross-warp stage) beats the cost of the `convert_layout` we must insert on
// the operand; if not, we leave the reduce alone.
//
// The reduction op, axis, extent and ordering are NEVER changed, so the numeric
// result is bit-identical (verified by the bitequiv eval hook: correctness
// stage must report 0 bit-changed). Scope: within-CTA blocked-layout
// reductions. The pass runs LAST in make_ttgir so its layout is not reverted by
// a later pass.
//
//===----------------------------------------------------------------------===//
...
static BlockedEncodingAttr makeIdeal(triton::ReduceOp red,
                                     BlockedEncodingAttr cur, int axis,
  ...
  if (laneBudget >
      0) // shape too small to hold a full warp: pile leftover on axis
    laneB[axis] += laneBudget;

  // 2. Warps: kept dims only (keep the axis warp-synchronous), by remaining
  // room.
  ...
  if (warpBudget >
      0) // kept dims cannot absorb all warps: fall back to axis (partial)
  ...
  auto cand = BlockedEncodingAttr::get(red.getContext(), S, T, W, O,
                                       cur.getCGALayout());
  if (cand == cur)
    return {};
  return cand;
...
static void rewriteOperandLayout(triton::ReduceOp red, BlockedEncodingAttr cand,
  ...
  Operation *redOp =
      red.getOperation(); // ReduceOp is variadic: index results via Operation*

  SmallVector<Value> converted;
  for (Value operand : red.getOperands()) {
    auto ty = cast<RankedTensorType>(operand.getType());
    auto newTy =
        RankedTensorType::get(ty.getShape(), ty.getElementType(), cand);
...
struct OptimizeReductionLayoutPass
  ...
  using Base =
      impl::TritonGPUOptimizeReductionLayoutBase<OptimizeReductionLayoutPass>;
  using Base::Base;
  void runOnOperation() override {
    // `strategy`, `minUnderparallel`, `maxElemsPerThread` are pass Options (see
    // Passes.td). The in-pipeline call sets them from `knobs.nvidia`
    // (compiler.py); LIT tests pass them as pass args. `strategy == "off"` ->
    // no-op (bit-safe).
    if (strategy == "off")
      return;
    ...
    for (triton::ReduceOp red : targets) {
      ...
      // contraction is layout-sensitive and NOT covered by inner_tree (which
      // only fixes the add tree), so relayouting would change the bits. Skip
      // it. Checking operand[0] is sufficient: FMA contraction only arises for
      // a single-operand sum(a*b); variadic reduces (e.g. welford) use non-add
      ...
      // Register-pressure guard: the reduce-friendly layout carries the axis
      // slice per thread, so if the tile has more elements per thread than the
      // register file holds (~256 for f32), the relayout spills and loses. Skip
      // those (they are the low-warp-count / very-large-tile corner).
      // Env-tunable.
      int64_t tileElems = 1;
      ...
      // Cost guard: the win is converting cross-warp / register reduction work
      // into cheap intra-warp shuffles, which only pays for the convert_layout
      // when the axis is BOTH gaining lanes AND currently badly
      // under-parallelized.
