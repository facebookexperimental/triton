...
ttg::CGAEncodingAttr
mlir::triton::getIdentityClusterBarrierLayout(Operation *op) {
  auto module = op->getParentOfType<ModuleOp>();
  auto dims = ttg::TritonGPUDialect::getClusterDims(module);
  assert(product(dims) > 1 &&
         "identity cluster barrier layout requires multiple CTAs");
  ...
  for (unsigned axis = 0; axis < 3; ++axis) {
    assert(dims[axis] > 0 && llvm::isPowerOf2_32(dims[axis]) &&
