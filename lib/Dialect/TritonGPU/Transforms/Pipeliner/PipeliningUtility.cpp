...
int mlir::triton::getNumStagesOrDefault(LoopLikeOpInterface loop,
                                        int defaultNumStages) {
  // Use the attribute attached to the loop if it exists otherwise use the
  // global control.
  auto helper = TritonDialect::getLoaded(loop)->getNumStagesAttrHelper();
  if (auto attr = helper.getAttr(loop))
    return attr.getInt();
