...
void createTMAAsyncLoad(scf::ForOp forOp, tt::DescriptorLoadOp loadOp,
  ...
  return createTMAAsyncCopy(
      ...
      [&](OpBuilderForStage &builder, Value desc, Value barrier, Value view,
          Value pred) {
        // Multicast copy: only the leader CTA issues the TMA and writes
        // directly into every peer's shared memory. Rendezvous all cluster
        // members first so no peer's target buffer is overwritten before that
        // peer has reached this pipeline stage.
